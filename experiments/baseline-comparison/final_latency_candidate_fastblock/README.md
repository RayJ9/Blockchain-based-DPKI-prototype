# Fastblock Final-Latency Candidate

This folder restores the previously edited `fastblock` latency version as a
standalone candidate snapshot, without modifying the current baseline-comparison
workflow.
...
`figure_latency_fastblock` version, while keeping the current experiment files
unchanged.

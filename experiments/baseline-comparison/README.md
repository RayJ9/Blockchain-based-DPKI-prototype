...
`experiment_artifacts/baseline-comparison`; the retained figure files in this folder are not
overwritten.
...
then draws the final `figure_latency.*` assets in this experiment directory.
...
The overhead table figure is rebuilt from the baseline-specific prototype benchmark:
...
this directory so the folder matches the final paper workflow.
...
The main `figure_latency.*` files now use this fastblock-based latency

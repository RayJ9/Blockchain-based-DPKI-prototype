# Availability-Timeout Experiment

Run the lightweight real-chain availability-tail probe:

```powershell
experiments\availability-timeout\run_experiment.ps1 -Requests 200
```

This executes a management-ratio-style DPKI/PKI workload and stores p90/p95/p99, timeout
availability, the largest observed samples, receipts, certificates, and chain
logs in the central experiment archive. It does not overwrite the retained
availability-timeout surface data or final figure.

Availability figure for failure probability and timeout threshold.

The current source data are retained under `source_data/`. They were generated
from Fig7-style service-time measurements using the same DPKI/PKI intra-domain
service-time definition as Fig7.

For this experiment, the PKI curve is plotted as a finite-sample conservative estimate
over 10000 requests at each grid point. The source CSV keeps both `PKI_Exact`
and `PKI_Observed` for checking.

Rebuild the figure data and redraw:

```powershell
python generate_data_fig9.py
& 'C:\Program Files\MATLAB\R2024a\bin\matlab.exe' -batch "cd('experiments/availability-timeout'); plot_fig9"
```

Outputs:

- `figure.png`
- `figure.pdf`
- `figure.eps`

The EPS is generated by `convert_png_to_pdf_eps.py` from the PNG to keep
transparent 3-D surfaces consistent across formats.

Kept data:

- `data_fig9.mat`
- `figure_summary.csv`
- `source_data/fig7_service_latency_samples_summary.csv`
- `source_data/surface_data_matrices_responding.npz`
- `source_data/surface_data_matrices_malicious.npz`

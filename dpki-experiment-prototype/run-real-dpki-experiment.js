...
const DEFAULTS = {
  ...
  powRuntime: path.join(ROOT, "omnilink-pow-4nodes", "runtime"),
  powWarmupMs: 65000,
...
function readOmnilinkPowRuntimeConfig(runtime) {
  const configPath = path.join(runtime, "configs", "node0.toml");
  const config = {
    path: configPath,
...
async function checkOmnilinkPowProcesses(web3, args) {
  ...
  if (activeMaintainers !== 4) {
    throw new Error(
      `Omnilink PoW requires 4 active maintainer processes, got ${activeMaintainers}/4. ` +
        `Start them with omnilink-pow-4nodes/scripts/start-omnilink-pow-4nodes.ps1. Runtime=${runtime}`
    );
  }
  const ready = await waitForOmnilinkPowReady(web3, args, runtime);
  return {
    backend: "omnilink-pow",
    configuredMaintainers: 4,
    activeMaintainers,
    rpc: args.rpc,
...
def parse_pow_runtime_config(runtime):
    config_path = runtime / 'configs' / 'node0.toml'
    result = {

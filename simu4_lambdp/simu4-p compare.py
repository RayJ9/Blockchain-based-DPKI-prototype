...
if __name__ == '__main__':
    # ===== 基线参数 =====
    P_MANAGE_SINGLE = 0.1                 # 仅在横轴不是 p 时生效
    Q_MANAGE_SINGLE = 0.3
    MU_BASE_SINGLE = 7.5
    GAMMA_HIGH_SECURITY_SINGLE = 0.1
    LAMBDA_P_BLOCK_RATE_SINGLE = 20
    ...
    # ===== 只选择一个数组作为横轴；其余保持标量 =====
    DISCRETE_SIM_LAMBDA_VALUES = 8             # 标量
    DISCRETE_SIM_M_VALUES = np.array([2,3,4,5,6,7,8])                         # 标量
    DISCRETE_SIM_P_VALUES = 0.1
    DISCRETE_SIM_EPSILON_VALUES = 0.1                # 标量
    ...
    elif isinstance(DISCRETE_SIM_P_VALUES, (list, np.ndarray)):     # 扫描 p
        x_param_name = 'p'
        x_param_label = '$p$ (mgmt request ratio)'
    ...
    if not (isinstance(discrete_sim_x_values, (list, np.ndarray)) and len(discrete_sim_x_values) > 0):
        raise ValueError("没有检测到数组形式的横轴参数。请确保四者中只有一个是数组。")
    ...
    print(f"固定 λ 值: {DISCRETE_SIM_LAMBDA_VALUES if not isinstance(DISCRETE_SIM_LAMBDA_VALUES, (list, np.ndarray)) else '随横轴变化'}")
    print(f"固定 M 值: {DISCRETE_SIM_M_VALUES if not isinstance(DISCRETE_SIM_M_VALUES, (list, np.ndarray)) else '随横轴变化'}")
    print(f"固定 p 值: {DISCRETE_SIM_P_VALUES if not isinstance(DISCRETE_SIM_P_VALUES, (list, np.ndarray)) else '随横轴变化'}")
    print(f"固定 epsilon 值: {DISCRETE_SIM_EPSILON_VALUES if not isinstance(DISCRETE_SIM_EPSILON_VALUES, (list, np.ndarray)) else '随横轴变化'}")
    ...
    for current_x_val in THEORY_X_VALUES_PLOT:
        ...
        p_val_current = current_x_val if x_param_name == 'p' else P_MANAGE_SINGLE

        fcfs_w_total, _ = theory_fcfs_values(
            m_val_current, lambda_val_current, p_val_current, MU_BASE_SINGLE, Q_MANAGE_SINGLE, epsilon_val_current
        )
        dpki_upper_theory_res = theoretical_values_dpki_upper_bound(
            lambda_val_current, p_val_current, Q_MANAGE_SINGLE, MU_BASE_SINGLE, m_val_current,
            GAMMA_HIGH_SECURITY_SINGLE, LAMBDA_P_BLOCK_RATE_SINGLE, epsilon_val_current
        )
        dpki_lower_theory_res = theoretical_values_dpki_lower_bound(
            lambda_val_current, p_val_current, Q_MANAGE_SINGLE, MU_BASE_SINGLE, m_val_current,
            GAMMA_HIGH_SECURITY_SINGLE, LAMBDA_P_BLOCK_RATE_SINGLE, epsilon_val_current
        )

        all_results.append({x_param_name: current_x_val, 'source': 'Theory', 'model': 'PKI FCFS',
                            'E_T_total': fcfs_w_total})
    ...
    for current_x_val in discrete_sim_x_values:
        ...
        # PKI FCFS Simulation
        try:
            sim_W_total_fcfs, sim_rho_fcfs = simulate_fcfs_queue(
                m_val_current, lambda_val_current, p_val_current, MU_BASE_SINGLE, Q_MANAGE_SINGLE,
                SIM_TIME, WARMUP_PROPORTION, COOLDOWN_PROPORTION, epsilon_val_current
            )
        ...
        # DPKI Simulation
        try:
            dpki_sim_res = run_simulation_dpki_model(
                lambda_val_current, p_val_current, Q_MANAGE_SINGLE, MU_BASE_SINGLE, m_val_current,
                GAMMA_HIGH_SECURITY_SINGLE, LAMBDA_P_BLOCK_RATE_SINGLE, SIM_TIME, WARMUP_TIME, epsilon_val_current
            )
    ...
    if len(discrete_sim_x_values) > 0:
        ...
        try:
            ...
            m_for_table = int(example_x_val) if x_param_name == 'M' else DISCRETE_SIM_M_VALUES
            epsilon_for_table = example_x_val if x_param_name == 'epsilon' else DISCRETE_SIM_EPSILON_VALUES
            p_for_table = example_x_val if x_param_name == 'p' else P_MANAGE_SINGLE

            dpki_upper_for_table = theoretical_values_dpki_upper_bound(
                lambda_for_table, p_for_table, Q_MANAGE_SINGLE, MU_BASE_SINGLE, m_for_table,
                GAMMA_HIGH_SECURITY_SINGLE, LAMBDA_P_BLOCK_RATE_SINGLE, epsilon_for_table
            )
            dpki_lower_for_table = theoretical_values_dpki_lower_bound(
                lambda_for_table, p_for_table, Q_MANAGE_SINGLE, MU_BASE_SINGLE, m_for_table,
                GAMMA_HIGH_SECURITY_SINGLE, LAMBDA_P_BLOCK_RATE_SINGLE, epsilon_for_table
            )
            dpki_sim_for_table = run_simulation_dpki_model(
                lambda_for_table, p_for_table, Q_MANAGE_SINGLE, MU_BASE_SINGLE, m_for_table,
                GAMMA_HIGH_SECURITY_SINGLE, LAMBDA_P_BLOCK_RATE_SINGLE, SIM_TIME, WARMUP_TIME, epsilon_for_table
            )
            pki_theory_w_total_for_table, _ = theory_fcfs_values(
                m_for_table, lambda_for_table, p_for_table, MU_BASE_SINGLE, Q_MANAGE_SINGLE, epsilon_for_table
            )
            ...
            print("{:<30} {:<15} {:<15} {:<15} {:<15}".format(
                "指标", "DPKI上界(理论)", "DPKI下界(理论)", "DPKI实际(仿真)", "PKI(理论)"
            ))
            print("-" * 100)
    ...
    plt.plot(dpki_upper_theory_df[x_param_name], dpki_upper_theory_df['E_T_total'],
             label='DPKI upper', linestyle='--', linewidth=2)
    plt.plot(dpki_lower_theory_df[x_param_name], dpki_lower_theory_df['E_T_total'],
             label='DPKI lower', linestyle=':', linewidth=2)
    plt.plot(dpki_real_situation_sim_df[x_param_name], dpki_real_situation_sim_df['E_T_total'],
             label='DPKI simu', marker='x', markersize=8, linestyle='-', linewidth=1.5)

    plt.plot(pki_fcfs_theory_df[x_param_name], pki_fcfs_theory_df['E_T_total'],
             label='PKI theory', linestyle='-', linewidth=2)
    plt.plot(pki_fcfs_sim_df[x_param_name], pki_fcfs_sim_df['E_T_total'],
             label='PKI comparison', marker='o', markersize=6, linestyle='None')

...
func TestCalcCommitMsgTxs(t *testing.T) {
	cfg := types.NewOmnilinkConfig(testnode.DefaultConfig)
...
//	omnilinkCfg := types.NewOmnilinkConfig(testnode.DefaultConfig)
//
//	api := new(apimocks.QueueProtocolAPI)
//	api.On("GetConfig", mock.Anything).Return(omnilinkCfg, nil)
//	para := &client{BaseClient: &drivers.BaseClient{}}
//
//	para.subCfg = new(subConfig)

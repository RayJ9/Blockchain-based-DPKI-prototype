...
type Proposal struct {
	tmtypes.PorsProposal
}

// NewProposal returns a new Proposal.
// If there is no POLRound, polRound should be -1.
func NewProposal(height int64, round int, blockhash []byte, polRound int, polBlockID tmtypes.PorsBlockID, seq int64) *Proposal {
	return &Proposal{tmtypes.PorsProposal{
		Height:     height,
		Round:      int32(round),
...
func (vote *Vote) Verify(chainID string, pubKey crypto.PubKey) error {
	...
	pub, err := ethCrypto.UnmarshalPubkey(pubKey.Bytes())
	if err != nil {
		panic(err)
	}
...
func (aggVote *AggVote) Verify(chainID string, valSet *ValidatorSet) error {
	...
	arr := &BitArray{PorsBitArray: aggVote.ValidatorArray}
	...
	origVote := &Vote{&tmtypes.PorsVote{
...
func (aggVote *AggVote) String() string {
	...
	bitArray := &BitArray{PorsBitArray: aggVote.ValidatorArray}

...
type VoteSet struct {
	...
	maj23         *tmtypes.PorsBlockID   // First 2/3 majority seen
...
func (voteSet *VoteSet) addVote(vote *Vote) (added bool, err error) {
	...
	blockKey := BlockID{PorsBlockID: vote.BlockID}.Key()
...
func (voteSet *VoteSet) addVerifiedVote(vote *Vote, blockKey string, votingPower int64) (added bool, conflicting *Vote) {
	...
	if origSum < quorum && quorum <= votesByBlock.sum {
		...
		if voteSet.maj23 == nil {
			voteSet.maj23 = &tmtypes.PorsBlockID{Hash: make([]byte, len(vote.BlockID.Hash))}
...
func (voteSet *VoteSet) AddAggVote(vote *AggVote) (bool, error) {
	...
	// Check maj32
	sum := int64(0)
	arr := &BitArray{PorsBitArray: vote.ValidatorArray}
...
func (voteSet *VoteSet) SetAggVote() error {
	...
	aggVote := &AggVote{&tmtypes.PorsAggVote{
		ValidatorAddress: voteSet.valSet.Proposer.Address,
		ValidatorArray:   bitArray.PorsBitArray,
...
// NOTE: VoteSet must not be nil
func (voteSet *VoteSet) SetPeerMaj23(peerID string, blockID *tmtypes.PorsBlockID) {
	if voteSet == nil {
...
// BitArrayByBlockID ...
func (voteSet *VoteSet) BitArrayByBlockID(blockID *tmtypes.PorsBlockID) *BitArray {
	if voteSet == nil {
		return nil
	}
	voteSet.mtx.Lock()
...
func (voteSet *VoteSet) TwoThirdsMajority() (tmtypes.PorsBlockID, bool) {
	if voteSet == nil {
		return tmtypes.PorsBlockID{}, false
	}
	voteSet.mtx.Lock()
	defer voteSet.mtx.Unlock()
	if voteSet.maj23 != nil {
		blockID := tmtypes.PorsBlockID{Hash: make([]byte, len(voteSet.maj23.Hash))}
...
// MakeCommit ...
func (voteSet *VoteSet) MakeCommit() *tmtypes.PorsCommit {
...
// MakeCommit ...
func (voteSet *VoteSet) MakeCommonCommit() *tmtypes.PorsCommit {
	voteSet.mtx.Lock()
	defer voteSet.mtx.Unlock()
	...
	votesCopy := make([]*tmtypes.PorsVote, len(voteSet.votes))
	for i, item := range voteSet.votes {
		if item != nil {
			votesCopy[i] = item.PorsVote
		} else {
			votesCopy[i] = &tmtypes.PorsVote{}
		}
	}
	var aggVote *tmtypes.PorsAggVote
	if voteSet.aggVote != nil {
		copy := voteSet.aggVote.Copy()
		aggVote = copy.PorsAggVote
	}

	if voteSet.voteType == VoteTypePrecommit {
		return &tmtypes.PorsCommit{
			BlockID:    voteSet.maj23,
...
func (voteSet *VoteSet) AddCommitVote(votes []*tmtypes.PorsVote, aggVote *tmtypes.PorsAggVote) error {

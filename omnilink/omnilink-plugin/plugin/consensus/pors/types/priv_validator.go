...
import (
	...
	types33 "code.corp.bcollie.net/omnilink/omnilink-base/types"
...
func GenPrivValidatorImp(filePath string, signType int) *PrivValidatorImp {
	...
	addr := []byte{}
	// workaround for eth
	if signType == types33.SECP256K1ETH {
		pub, err := ethCrypto.UnmarshalPubkey(privKey.PubKey().Bytes())
		if err != nil {
			panic(err)
		}
		addr1 := ethCrypto.PubkeyToAddress(*pub)
		addr = addr1.Bytes()

	} else {
		addr = GenAddressByPubKey(privKey.PubKey())
	}
...
func LoadOrGenPrivValidatorFS(filePath string, signType int) *PrivValidatorImp {
	var privVal *PrivValidatorImp
	if _, err := os.Stat(filePath); err == nil {
		privVal = LoadPrivValidatorFS(filePath)
	} else {
		privVal = GenPrivValidatorImp(filePath, signType)
		privVal.Save()
	}
	return privVal
}
...
func LoadPrivValidatorFSWithSigner(filePath string, signerFunc func(PrivValidator) Signer) *PrivValidatorImp {
	...
	addr, err := hex.DecodeString(strings.TrimPrefix(privVal.Address, "0x"))
	if err != nil {
...
func (pv *PrivValidatorImp) save() {
	...
	addr := fmt.Sprintf("0x%s", hex.EncodeToString(pv.Address))

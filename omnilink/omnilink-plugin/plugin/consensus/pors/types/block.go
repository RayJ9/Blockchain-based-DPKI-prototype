...
// IsZero returns true if this is the PorsBlockID for a nil-block
func (blockID BlockID) IsZero() bool {
	if blockID.PorsBlockID == nil {
		return true
	}
	return len(blockID.Hash) == 0
}

// Equals returns true if the PorsBlockID matches the given PorsBlockID
func (blockID BlockID) Equals(other BlockID) bool {
	return blockID.Key() == other.Key()
}

// Key returns a machine-readable string representation of the PorsBlockID
...
// String returns a human readable string representation of the PorsBlockID
...
func MakeBlock(height int64, round int64, pblock *types.Block, commit *tmtypes.PorsCommit) *PorsBlock {
	block := &PorsBlock{
		&tmtypes.PorsBlock{
			Header: &tmtypes.PorsBlockHeader{
				Height: height,
				Round:  round,
...
func (b *PorsBlock) ValidateBasic() error {
	if b == nil {
...
func (b *PorsBlock) String() string {
	return b.StringIndented("")
}

// StringIndented returns a string representation of the block
func (b *PorsBlock) StringIndented(indent string) string {
	if b == nil {
		return "nil-Block"
	}
	header := &Header{PorsBlockHeader: b.Header}
	lastCommit := &Commit{PorsCommit: b.LastCommit}
	return Fmt(`Block{
%s  %v
%s  %v
%s}#%v`,
...
func (b *PorsBlock) StringShort() string {
	if b == nil {
		return "nil-Block"
	}
...
// Header defines the structure of a Pors block header
// TODO: limit header size
// NOTE: changes to the Header should be duplicated in the abci Header
type Header struct {
	*tmtypes.PorsBlockHeader
}
...
type Commit struct {
	*tmtypes.PorsCommit
	hash      []byte
	bitArray  *BitArray
	firstVote *tmtypes.PorsVote
}

// FirstVote returns the first non-nil prevote/precommit in the commit
func (commit *Commit) FirstVote() *tmtypes.PorsVote {
...
func (commit *Commit) BitArray() *BitArray {
	if commit.AggVote != nil {
		bitArray := &BitArray{PorsBitArray: commit.AggVote.ValidatorArray}
	...
	if commit.bitArray == nil {
		if commit.VoteType == uint32(VoteTypePrecommit) {
			...
			for i, precommit := range commit.Precommits {
				// TODO: need to check the PorsBlockID otherwise we could be counting conflicts,
...
func (commit *Commit) GetByIndex(index int) *Vote {
	if commit.VoteType == uint32(VoteTypePrecommit) {
		return &Vote{PorsVote: commit.Precommits[index]}
	} else {
		return &Vote{PorsVote: commit.Prevotes[index]}
	}
}
...
func (commit *Commit) ValidateBasic() error {
	blockID := &BlockID{PorsBlockID: commit.BlockID}
	if blockID.IsZero() {
...
func (commit *Commit) Hash() []byte {
	if commit.hash == nil {
		if commit.AggVote != nil {
			aggVote := &AggVote{PorsAggVote: commit.AggVote}

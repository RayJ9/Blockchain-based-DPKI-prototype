...
func (pc *peerConn) SetID(newID ID) {
	pc.id = newID
}
...
func (pc *peerConn) CloseConn() {
	...
	if err != nil {
		porslog.Error("peerConn CloseConn failed", "err", err)
	}
}
...
func (pc *peerConn) HandshakeTimeout(
	...
	Parallel(
		func() {
			...
			if err1 != nil {
				porslog.Error("Peer handshake Marshal ourNodeInfo failed", "err", err1)
				return
			}
			frame := make([]byte, 4)
			...
			if err1 != nil {
				porslog.Error("Peer handshake write info size failed", "err", err1)
				return
			}
			_, err1 = pc.conn.Write(info[:])
			if err1 != nil {
				porslog.Error("Peer handshake write info failed", "err", err1)
				return
			}
		},
		func() {
			...
			if err2 != nil {
				porslog.Error("Peer handshake read info size failed", "err", err1)
				return
			}
			...
			_, err2 = io.ReadFull(pc.conn, readBuffer[:])
			if err2 != nil {
				porslog.Error("Peer handshake read info failed", "err", err1)
				return
			}
			err2 = json.Unmarshal(readBuffer, &peerNodeInfo)
			if err2 != nil {
				porslog.Error("Peer handshake Unmarshal failed", "err", err1)
				return
			}
			porslog.Info("Peer handshake", "peerNodeInfo", peerNodeInfo)
		},
	)
	if err1 != nil {
...
func (pc *peerConn) Send(msg MsgInfo) bool {
	...
	case <-time.After(defaultSendTimeout):
		if msg.TypeID == ttypes.ProposalBlockID {
			if propBlock, ok := msg.Msg.(*tmtypes.PorsBlock); ok {
				msg.Msg = &tmtypes.PorsBlock{
					Header:     propBlock.Header,
					LastCommit: propBlock.LastCommit,
				}
			}
		}
		porslog.Error("send msg timeout", "peerip", msg.PeerIP, "msg", msg)
		return false
	}
}
...
func (pc *peerConn) PickSendVote(votes ttypes.VoteSetReader) bool {
	if UseAggSig() {
		if votes.GetAggVote() != nil {
			if aggVote, ok := pc.state.PickAggVoteToSend(votes); ok {
				msg := MsgInfo{TypeID: ttypes.AggVoteID, Msg: aggVote.PorsAggVote, PeerID: pc.id, PeerIP: pc.ip.String()}
				porslog.Info("Sending aggregate vote message", "msg", msg)
				if pc.Send(msg) {
	...
	if vote, ok := pc.state.PickVoteToSend(votes); ok {
		if vote == nil {
			porslog.Warn("Pick nil vote", "aggVote", votes.GetAggVote(), "vote", votes)
			return false
		}
		msg := MsgInfo{TypeID: ttypes.VoteID, Msg: vote.PorsVote, PeerID: pc.id, PeerIP: pc.ip.String()}
		porslog.Info("Sending vote message", "msg", msg)
		if pc.Send(msg) {
...
func (pc *peerConn) Start() error {
	if atomic.CompareAndSwapUint32(&pc.started, 0, 1) {
		if atomic.LoadUint32(&pc.stopped) == 1 {
			porslog.Error("peerConn already stopped", "peerIP", pc.ip.String())
			return nil
		}
...
func (pc *peerConn) Stop() {
	if atomic.CompareAndSwapUint32(&pc.stopped, 0, 1) {
		pc.CloseConn()
		porslog.Info("peerConn close connection", "peerIP", pc.ip.String())
	}
}

func (pc *peerConn) stopForError(r interface{}) {
	porslog.Error("peerConn recovered panic", "error", r, "peer", pc.ip.String())
...
func encodeMsg(msg types.Message, typeID byte) []byte {
	...
	if len(raw) > MaxMsgPacketPayloadSize {
		...
		cmp = byte(1)
		porslog.Info("compress large message", "old", len(raw), "new", len(buf))
		raw = buf
	}
...
func decodeMsg(msg []byte, cmp byte) ([]byte, error) {
	...
	porslog.Info("uncompress large message", "old", len(msg), "new", len(buf))
	return buf, nil
}
...
FOR_LOOP:
	for {
		...
		case msg := <-pc.sendQueue:
			...
			if err != nil {
				porslog.Error("sendRoutine buffer write fail", "peer", pc, "err", err)
				pc.stopForError(err)
				break FOR_LOOP
			}
			err = pc.bufWriter.Flush()
			if err != nil {
				porslog.Error("sendRoutine buffer flush fail", "peer", pc, "err", err)
				pc.stopForError(err)
				break FOR_LOOP
			}
		}
	}
	porslog.Info("peerConn stop sendRoutine", "peerIP", pc.ip.String())
}
...
FOR_LOOP:
	for {
		...
		if err != nil {
			porslog.Error("recvRoutine read byte fail", "peer", pc, "err", err)
			pc.stopForError(err)
			break FOR_LOOP
		}
		...
		if msgLen <= 0 {
			porslog.Error("recvRoutine read invalid data", "msgLen", msgLen, "cmp", cmp, "peerIP", pc.ip.String())
			continue
		}
		buf2 := make([]byte, msgLen)
		_, err = io.ReadFull(pc.bufReader, buf2)
		if err != nil {
			porslog.Error("recvRoutine read data fail", "err", err, "peerIP", pc.ip.String())
			continue
		}
		buf3, err := decodeMsg(buf2, cmp)
		if err != nil {
			porslog.Error("recvRoutine decode msg fail", "err", err, "peerIP", pc.ip.String())
			continue
		}
		pkt.Bytes = buf3
		...
		if v, ok := ttypes.MsgMap[pkt.TypeID]; ok {
			...
			if err != nil {
				porslog.Error("recvRoutine Unmarshal data fail", "msgTy", pkt.TypeID, "msgLen", len(pkt.Bytes), "err", err, "peerIP", pc.ip.String())
				continue
			}
			if pc.transferChannel != nil && (pkt.TypeID == ttypes.ProposalID || pkt.TypeID == ttypes.VoteID ||
				...
				if pkt.TypeID == ttypes.ProposalID {
					proposal := realMsg.(*tmtypes.PorsProposal)
					porslog.Info("Receiving proposal", "proposal-height", proposal.Height, "peerip", pc.ip.String())
					pc.state.SetHasProposal(proposal)
				} else if pkt.TypeID == ttypes.VoteID {
					vote := &ttypes.Vote{PorsVote: realMsg.(*tmtypes.PorsVote)}
					porslog.Info("Receiving vote", "vote-height", vote.Height, "peerip", pc.ip.String())
					pc.state.SetHasVote(vote)
				} else if pkt.TypeID == ttypes.ProposalBlockID {
					block := &ttypes.PorsBlock{PorsBlock: realMsg.(*tmtypes.PorsBlock)}
					porslog.Info("Receiving proposal block", "block-height", block.Header.Height, "peerip", pc.ip.String())
					pc.state.SetHasProposalBlock(block)
				} else if pkt.TypeID == ttypes.AggVoteID {
					aggVote := &ttypes.AggVote{PorsAggVote: realMsg.(*tmtypes.PorsAggVote)}
					porslog.Info("Receiving aggregate vote", "aggVote-height", aggVote.Height, "peerip", pc.ip.String())
					pc.state.SetHasAggVote(aggVote)
				}
			} else if pkt.TypeID == ttypes.ProposalHeartbeatID {
				pc.heartbeatQueue <- realMsg.(*tmtypes.PorsHeartbeat)
			} else {
				pc.updateStateQueue <- MsgInfo{pkt.TypeID, realMsg.(proto.Message), pc.ID(), pc.ip.String()}
			}
		} else {
			porslog.Error("receive unknown message type", "type", pkt.TypeID, "peerIP", pc.ip.String())
		}
	}
	pc.quitUpdate <- struct{}{}
	pc.quitBeat <- struct{}{}
	porslog.Info("peerConn stop recvRoutine", "peerIP", pc.ip.String())
}
	...
	for {
		...
		case msg := <-pc.updateStateQueue:
			...
			if typeID == ttypes.NewRoundStepID {
				pc.state.ApplyNewRoundStepMessage(msg.Msg.(*tmtypes.PorsNewRoundStepMsg))
			} else if typeID == ttypes.ValidBlockID {
				pc.state.ApplyValidBlockMessage(msg.Msg.(*tmtypes.PorsValidBlockMsg))
			} else if typeID == ttypes.HasVoteID {
				pc.state.ApplyHasVoteMessage(msg.Msg.(*tmtypes.PorsHasVoteMsg))
			} else if typeID == ttypes.VoteSetMaj23ID {
				tmp := msg.Msg.(*tmtypes.PorsVoteSetMaj23Msg)
				...
				default:
					porslog.Error("Bad VoteSetBitsMessage field Type", "type", byte(tmp.Type))
					return
				}
				if myVotes != nil && myVotes.PorsBitArray != nil {
					voteSetBitMsg := &tmtypes.PorsVoteSetBitsMsg{
						Height:  tmp.Height,
						Round:   tmp.Round,
						Type:    tmp.Type,
						BlockID: tmp.BlockID,
						Votes:   myVotes.PorsBitArray,
					}
				...
				pc.state.ApplyProposalPOLMessage(msg.Msg.(*tmtypes.PorsProposalPOLMsg))
			} else if typeID == ttypes.VoteSetBitsID {
				tmp := msg.Msg.(*tmtypes.PorsVoteSetBitsMsg)
				...
				porslog.Error("Unknown message type in updateStateRoutine", "msg", msg)
			}
		}
	}
	close(pc.updateStateQueue)
	porslog.Info("peerConn stop updateStateRoutine", "peerIP", pc.ip.String())
}
...
FOR_LOOP:
	for {
		...
		case heartbeat := <-pc.heartbeatQueue:
			msg, ok := heartbeat.(*tmtypes.PorsHeartbeat)
			if ok {
				porslog.Info("Received proposal heartbeat message",
	...
	porslog.Info("peerConn stop heartbeatRoutine", "peerIP", pc.ip.String())
}
...
OUTER_LOOP:
	for {
		...
		if !pc.IsRunning() {
			porslog.Info("peerConn stop gossipDataRoutine", "peerIP", pc.ip.String())
			return
		}
		...
		if (0 < prs.Height) && (prs.Height < rs.Height) {
			if prs.ProposalBlockHash != nil && !prs.ProposalBlock {
				...
				if proposalBlock == nil {
					porslog.Error("load proposal block fail", "selfHeight", rs.Height,
						"blockHeight", pc.myState.client.GetCurrentHeight())
					time.Sleep(pc.myState.PeerGossipSleep())
					continue OUTER_LOOP
				}
				newBlock := &ttypes.PorsBlock{PorsBlock: proposalBlock}
				if !newBlock.HashesTo(prs.ProposalBlockHash) {
					porslog.Error(fmt.Sprintf("Wrong proposal block hash. Expected %X, got %X", prs.ProposalBlockHash,
				...
				porslog.Info("Sending block for catchup", "peerIP", pc.ip.String(),
		...
		if rs.Proposal != nil && !prs.Proposal {
			// Proposal: share the proposal metadata with peer.
			{
				...
				porslog.Info(fmt.Sprintf("Sending proposal. Self state: %v/%v/%v", rs.Height, rs.Round, rs.Step),
			...
			if 0 <= rs.Proposal.POLRound {
				msg := MsgInfo{TypeID: ttypes.ProposalPOLID, Msg: &tmtypes.PorsProposalPOLMsg{
					Height:           rs.Height,
					ProposalPOLRound: rs.Proposal.POLRound,
					ProposalPOL:      rs.Votes.Prevotes(int(rs.Proposal.POLRound)).BitArray().PorsBitArray,
				}, PeerID: pc.id, PeerIP: pc.ip.String()}
				porslog.Info("Sending POL", "height", prs.Height, "round", prs.Round)
				pc.Send(msg)
			}
			continue OUTER_LOOP
		}
		...
		if rs.Proposal != nil && prs.ProposalBlockHash != nil && bytes.Equal(rs.Proposal.Blockhash, prs.ProposalBlockHash) {
			if rs.ProposalBlock != nil && !prs.ProposalBlock {
				msg := MsgInfo{TypeID: ttypes.ProposalBlockID, Msg: rs.ProposalBlock.PorsBlock, PeerID: pc.id, PeerIP: pc.ip.String()}
				porslog.Info(fmt.Sprintf("Sending proposal block. Self state: %v/%v/%v", rs.Height, rs.Round, rs.Step),
...
OUTER_LOOP:
	for {
		// Manage disconnects from self or peer.
		if !pc.IsRunning() {
			porslog.Info("peerConn stop gossipVotesRoutine", "peerIP", pc.ip.String())
			return
		}
		...
		if prs.Height != 0 && rs.Height == prs.Height+1 {
			if pc.PickSendVote(rs.LastCommit) {
				porslog.Info("Picked rs.LastCommit to send", "peerip", pc.ip.String(), "height", prs.Height)
				continue OUTER_LOOP
			}
		}
		...
		if prs.Height != 0 && rs.Height >= prs.Height+2 {
			...
			commitObj := &ttypes.Commit{PorsCommit: commit}
			if pc.PickSendVote(commitObj) {
				porslog.Info("Picked Catchup commit to send",
		...
		if sleeping == 0 {
			...
			sleeping = 1
			porslog.Info("No votes to send, sleeping", "peerip", pc.ip.String(), "rs.Height", rs.Height, "prs.Height", prs.Height,
...
func (pc *peerConn) gossipVotesForHeight(rs *ttypes.RoundState, prs *ttypes.PeerRoundState) bool {
	...
	if prs.Step == ttypes.RoundStepNewHeight {
		if pc.PickSendVote(rs.LastCommit) {
			porslog.Info("Picked rs.LastCommit to send", "peerip", pc.ip.String(),
	...
	if prs.Step <= ttypes.RoundStepPropose && prs.Round != -1 && prs.Round <= rs.Round && prs.ProposalPOLRound != -1 {
		if polPrevotes := rs.Votes.Prevotes(prs.ProposalPOLRound); polPrevotes != nil {
			if pc.PickSendVote(polPrevotes) {
				porslog.Info("Picked rs.Prevotes(prs.ProposalPOLRound) to send",
	...
	if prs.Step <= ttypes.RoundStepPrevoteWait && prs.Round != -1 && prs.Round <= rs.Round {
		if pc.PickSendVote(rs.Votes.Prevotes(prs.Round)) {
			porslog.Info("Picked rs.Prevotes(prs.Round) to send",
				"peerip", pc.ip.String(), "peer(H/R)", fmt.Sprintf("%v/%v", prs.Height, prs.Round))
			return true
		}
	}
	...
	if prs.Step <= ttypes.RoundStepPrecommitWait && prs.Round != -1 && prs.Round <= rs.Round {
		if pc.PickSendVote(rs.Votes.Precommits(prs.Round)) {
			porslog.Info("Picked rs.Precommits(prs.Round) to send",
				"peerip", pc.ip.String(), "peer(H/R)", fmt.Sprintf("%v/%v", prs.Height, prs.Round))
			return true
		}
	}
...
OUTER_LOOP:
	for {
		// Manage disconnects from self or peer.
		if !pc.IsRunning() {
			porslog.Info("peerConn stop queryMaj23Routine", "peerIP", pc.ip.String())
			return
		}
		...
		// Maybe send Height/Round/Prevotes
		{
			...
			if rs.Height == prs.Height {
				if maj23, ok := rs.Votes.Prevotes(prs.Round).TwoThirdsMajority(); ok {
					msg := MsgInfo{TypeID: ttypes.VoteSetMaj23ID, Msg: &tmtypes.PorsVoteSetMaj23Msg{
						Height:  prs.Height,
		...
		// Maybe send Height/CatchupCommitRound/CatchupCommit.
		{
			...
			if prs.CatchupCommitRound != -1 && 0 < prs.Height && prs.Height <= pc.myState.client.csStore.LoadStateHeight() {
				...
				if commit.VoteType == uint32(ttypes.VoteTypePrecommit) {
					commitTmp := ttypes.Commit{PorsCommit: commit}
					msg := MsgInfo{TypeID: ttypes.VoteSetMaj23ID, Msg: &tmtypes.PorsVoteSetMaj23Msg{
						Height:  prs.Height,
...
func (ps *PeerConnState) SetHasProposal(proposal *tmtypes.PorsProposal) {
	ps.mtx.Lock()
	defer ps.mtx.Unlock()
	...
	porslog.Info("Peer set proposal", "peerIP", ps.ip.String(),
...
func (ps *PeerConnState) SetHasProposalBlock(block *ttypes.PorsBlock) {
	ps.mtx.Lock()
	defer ps.mtx.Unlock()
	...
	porslog.Info("Peer set proposal block", "peerIP", ps.ip.String(),
...
func (ps *PeerConnState) SetHasAggVote(aggVote *ttypes.AggVote) {
	...
	porslog.Info("Peer set aggregate vote", "peerIP", ps.ip.String(),
		"peer-state", fmt.Sprintf("%v/%v(%v)/%v", ps.Height, ps.Round, ps.CatchupCommitRound, ps.Step),
...
func (ps *PeerConnState) PickVoteToSend(votes ttypes.VoteSetReader) (vote *ttypes.Vote, ok bool) {
	...
	if index, ok := votes.BitArray().Sub(psVotes).PickRandom(); ok {
		porslog.Info("PickVoteToSend", "peer(H/R)", fmt.Sprintf("%v/%v", ps.Height, ps.Round),
...
func (ps *PeerConnState) ensureCatchupCommitRound(height int64, round int, numValidators int) {
	...
	*/
	...
	porslog.Info("ensureCatchupCommitRound", "height", height, "round", round, "ps.CatchupCommitRound", ps.CatchupCommitRound,
...
func (ps *PeerConnState) setHasVote(height int64, round int, voteType byte, index int) {
	...
	psVotes := ps.getVoteBitArray(height, round, voteType)
	porslog.Info("setHasVote before", "height", height, "psVotes", psVotes.String(), "peerip", ps.ip.String())
	if psVotes != nil {
		psVotes.SetIndex(index, true)
	}
	porslog.Info("setHasVote after", "height", height, "index", index, "type", voteType,
		"peerVotes", psVotes.String(), "peerip", ps.ip.String())
}

// ApplyNewRoundStepMessage updates the peer state for the new round.
func (ps *PeerConnState) ApplyNewRoundStepMessage(msg *tmtypes.PorsNewRoundStepMsg) {
	ps.mtx.Lock()
	defer ps.mtx.Unlock()
	...
	porslog.Info("ApplyNewRoundStepMessage", "peerip", ps.ip.String(),
	...
	if psHeight != msg.Height || psRound != int(msg.Round) {
		porslog.Info("Reset PorsProposal, Prevotes, Precommits", "peerip", ps.ip.String(),
			"peer(H/R)", fmt.Sprintf("%v/%v", psHeight, psRound))
		ps.Proposal = false
	...
	if psHeight == msg.Height && psRound != int(msg.Round) && int(msg.Round) == psCatchupCommitRound {
		...
		porslog.Info("Reset Precommits to CatchupCommit", "peerip", ps.ip.String(),
			"peer(H/R)", fmt.Sprintf("%v/%v", psHeight, psRound))
		ps.Precommits = psCatchupCommit
	}
	if psHeight != msg.Height {
		porslog.Info("Reset LastCommit, CatchupCommit", "peerip", ps.ip.String(),
			"peer(H/R)", fmt.Sprintf("%v/%v", psHeight, psRound))
...
func (ps *PeerConnState) ApplyValidBlockMessage(msg *tmtypes.PorsValidBlockMsg) {
	ps.mtx.Lock()
	defer ps.mtx.Unlock()
	...
	porslog.Info("ApplyValidBlockMessage", "peerIP", ps.ip.String(),
	...
	if ps.CatchupCommitRound == int(msg.Round) && msg.IsCommit {
		porslog.Info("Set ProposalBlockHash for catchup", "peerIP", ps.ip.String(),
			"peer(H/R/S)", fmt.Sprintf("%v/%v/%v", ps.Height, ps.Round, ps.Step),
...
func (ps *PeerConnState) ApplyProposalPOLMessage(msg *tmtypes.PorsProposalPOLMsg) {
	ps.mtx.Lock()
	defer ps.mtx.Unlock()
	...
	ps.ProposalPOL = &ttypes.BitArray{PorsBitArray: msg.ProposalPOL}
}

// ApplyHasVoteMessage updates the peer state for the new vote.
func (ps *PeerConnState) ApplyHasVoteMessage(msg *tmtypes.PorsHasVoteMsg) {
	ps.mtx.Lock()
	defer ps.mtx.Unlock()
	...
	porslog.Info("ApplyHasVoteMessage", "msg(H/R)", fmt.Sprintf("%v/%v", msg.Height, msg.Round),
		"peerip", ps.ip.String())
...
func (ps *PeerConnState) ApplyVoteSetBitsMessage(msg *tmtypes.PorsVoteSetBitsMsg, ourVotes *ttypes.BitArray) {
	ps.mtx.Lock()
	defer ps.mtx.Unlock()
	...
	if votes != nil {
		if ourVotes == nil {
			bitarray := &ttypes.BitArray{PorsBitArray: msg.Votes}

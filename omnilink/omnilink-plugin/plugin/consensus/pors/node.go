...
func (node *Node) Start() {
	if atomic.CompareAndSwapUint32(&node.started, 0, 1) {
		...
		porslog.Info("Local listener", "ip", listenerIP, "port", listenerPort)
		...
		for i := 0; i < len(node.seeds); i++ {
			go func(i int) {
				...
				if ok {
					porslog.Info("find our ip ", "ourIP", ip)
					node.IP = ip
					return
				}
				...
				if err != nil {
					porslog.Error("Error dialing peer", "err", err)
				}
			}(i)
		}
...
func (node *Node) addOutboundPeerWithConfig(addr string) error {
	porslog.Info("Dialing peer", "address", addr)
...
func (node *Node) Stop() {
	...
	if err != nil {
		porslog.Error("Close listener failed", "err", err)
	}
	// Stop peers
...
func (node *Node) BroadcastRoutine() {
	for {
		...
		if !ok {
			porslog.Info("broadcastChannel closed")
			return
		}
		node.Broadcast(msg)
	}
...
func (node *Node) UnicastRoutine() {
	for {
		...
		if !ok {
			porslog.Info("unicastChannel closed")
			return
		}
		for _, peer := range node.peerSet.List() {
			if peer.ID() == msg.PeerID {
				...
				if !success {
					porslog.Error("send failure in UnicastRoutine", "peerID", peer.ID())
				}
				break
			}
		}
...
func (node *Node) connectComming(inConn net.Conn) {
	...
	if maxPeers <= node.peerSet.Size() {
		porslog.Info("Ignoring inbound connection: already have enough peers", "address", inConn.RemoteAddr().String(), "numPeers", node.peerSet.Size(), "max", maxPeers)
		return
	}
	...
	if err != nil {
		porslog.Info("Ignoring inbound connection: error while adding peer", "address", inConn.RemoteAddr().String(), "err", err)
		return
	}
}
...
func (node *Node) StopPeerForError(peer Peer, reason interface{}) {
	porslog.Error("Stopping peer for error", "peer", peer, "err", reason)
...
func (node *Node) addInboundPeer(conn net.Conn) error {
	...
	if err != nil {
		if er := conn.Close(); er != nil {
			porslog.Error("addInboundPeer close conn failed", "er", er)
		}
		return err
	}
	if err = node.addPeer(peerConn); err != nil {
...
func (node *Node) addPeer(pc *peerConn) error {
	...
	porslog.Info("Successful handshake with peer", "peerNodeInfo", peerNodeInfo)
	...
	porslog.Info("Added peer", "ip", pc.ip, "peer", pc)
	...
	pc.Send(stateMsg)
	porslog.Info("Send state msg", "msg", stateMsg, "ourIP", node.IP, "ourID", node.ID)
	return nil
}
...
func (node *Node) Broadcast(msg MsgInfo) chan bool {
	...
	porslog.Info("Broadcast", "msgtype", msg.TypeID)
	var wg sync.WaitGroup
	for _, peer := range node.peerSet.List() {
		wg.Add(1)
		go func(peer Peer) {
...
func (node *Node) startInitPeer(peer *peerConn) error {
	...
	if err != nil {
		// Should never happen
		porslog.Error("Error starting peer", "peer", peer, "err", err)
		return err
	}
...
func (node *Node) reconnectToPeer(addr string) {
	...
	start := time.Now()
	porslog.Info("Reconnecting to peer", "addr", addr)
	for i := 0; i < reconnectAttempts; i++ {
		...
		if err != nil {
			porslog.Info("LookupIP failed", "host", host)
			continue
		}
		if node.peerSet.HasIP(ips[0]) {
			porslog.Info("Reconnecting to peer exit, already connect to the peer", "peer", host)
			return
		}
		...
		porslog.Info("Error reconnecting to peer. Trying again", "tries", i, "err", err, "addr", addr)
		// sleep a set amount
		randomSleep(reconnectInterval)
		continue
	}

	porslog.Error("Failed to reconnect to peer. Beginning exponential backoff",
	...
	porslog.Error("Failed to reconnect to peer. Giving up", "addr", addr, "elapsed", time.Since(start))
}
...
func newOutboundPeerConn(addr string, ourNodePrivKey crypto.PrivKey, onPeerError func(Peer, interface{}), state *ConsensusState) (*peerConn, error) {
	conn, err := dial(addr)
	if err != nil {
		porslog.Error("Failed to dail in newOutboundPeerConn", "addr", addr, "error", err)
	...
	if err != nil {
		porslog.Error("Failed to newPeerConn in newOutboundPeerConn", "addr", addr, "error", err)

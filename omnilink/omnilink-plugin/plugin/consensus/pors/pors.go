...
const (
	...
	// DefaultPorsPort 默认端口
	DefaultPorsPort = 34001
)

var (
	porslog = log15.New("module", "pors")
	...
	dbPath                             = fmt.Sprintf("datadir%spors", string(os.PathSeparator))
	port                  int32        = DefaultPorsPort
...
func init() {
	drivers.Reg("pors", New)
	drivers.QueryData.Register("pors", &Client{})
	random = rand.New(rand.NewSource(time.Now().UnixNano()))
}

// Client pors implementation
type Client struct {
	//config
	*drivers.BaseClient
...
func New(cfg *types.Consensus, sub []byte) queue.Module {
	porslog.Info("create pors client")
	genesis = cfg.Genesis
	genesisBlockTime = cfg.GenesisBlockTime
	applyConfig(cfg, sub)
	if !cfg.Minerstart {
		porslog.Info("This node only sync block")
	...
	if !ok {
		porslog.Error("invalid sign name")
		return nil
	}
	...
	if err != nil {
		porslog.Error("load pors crypto fail", "err", err)
		return nil
	}
	...
	if UseAggSig() {
		...
		if err != nil {
			porslog.Error("pors crypto not support aggregate signature", "name", ttypes.CryptoName)
			return nil
		}
	}
	...
	if err != nil {
		porslog.Error("load genesis file fail", "error", err)
		return nil
	}
	privValidator := ttypes.LoadPrivValidatorFS(privFile)
	if privValidator == nil {
		porslog.Error("load priv_validator file fail")
		return nil
	}

	porslog.Info("show pors info", "version", porsVersion, "sign", ttypes.CryptoName, "useAggSig", UseAggSig(),
...
func (client *Client) GenesisState() *State {
	...
	if err != nil {
		porslog.Error("GenesisState", "err", err)
		return nil
	}
	return &state
}
...
func (client *Client) Close() {
	...
	porslog.Info("consensus pors closed")
}
...
OuterLoop:
	for fastSync {
		...
		case <-client.ctx.Done():
			porslog.Info("StartConsensus quit")
			return
		case <-hint.C:
			porslog.Info("Still catching up max height......", "Height", client.GetCurrentHeight(), "cost", time.Since(beg))
		default:
			if client.IsCaughtUp() {
				porslog.Info("This node has caught up max height")
				break OuterLoop
			}
	...
	if client.GetCurrentHeight() == 0 {
		...
		porslog.Info("load state from store")
	} else {
		...
		porslog.Info("load state from block")
		...
		porslog.Info("save state from block")
	}

	// start
	porslog.Info("show state info",
	...
	node := NewNode(validatorNodes, protocol, listeningAddress, client.privKey, state.ChainID, porsVersion, csState)
...
func (client *Client) getBlockInfoTx(current *types.Block) (*tmtypes.PorsExecAction, error) {
	//检查第一笔交易
	...
	var valAction tmtypes.PorsExecAction
	err := types.Decode(baseTx.GetPayload(), &valAction)
	if err != nil {
		return nil, err
	}
	//检查交易类型
	if valAction.GetTy() != tmtypes.PorsActionBlockInfo {
...
func (client *Client) CheckBlock(parent *types.Block, current *types.BlockDetail) error {
	...
	if current.Block.Height > 1 {
		...
		lastProposalBlock := &ttypes.PorsBlock{PorsBlock: lastInfo.GetBlock()}
...
func (client *Client) CreateBlock() {
	...
	for {
		select {
		case <-client.ctx.Done():
			porslog.Info("CreateBlock quit")
			return
		case <-ticker.C:
			if !client.csState.IsRunning() {
				porslog.Info("consensus not running")
				break
			}

			height, err := client.getLastHeight()
			if err != nil {
				porslog.Info("getLastHeight fail", "err", err)
				break
			}
...
func (client *Client) CommitBlock(block *types.Block) {
	...
	if err != nil {
		porslog.Warn("CommitBlock fail", "err", err)
...
func (client *Client) WaitBlock(height int64) *types.Block {
	...
	for {
		select {
		case <-client.ctx.Done():
			porslog.Info("WaitBlock quit")
			return nil
		case <-ticker.C:
			porslog.Info("Still waiting block......", "height", height, "cost", time.Since(beg))
		default:
...
func (client *Client) QueryValidatorsByHeight(height int64) (*tmtypes.PorsNodes, error) {
	if height < 1 {
		return nil, ttypes.ErrHeightLessThanOne
	}
	req := &tmtypes.ReqPorsNodes{Height: height}
	param, err := proto.Marshal(req)
	if err != nil {
		porslog.Error("QueryValidatorsByHeight marshal", "err", err)
		return nil, types.ErrInvalidParam
	}
	msg := client.GetQueueClient().NewMessage("execs", types.EventBlockChainQuery,
		&types.ChainExecutor{Driver: "pors", FuncName: "GetPorsNodeUpdateByHeight", StateHash: zeroHash[:], Param: param})
	err = client.GetQueueClient().Send(msg, true)
	if err != nil {
		porslog.Error("QueryValidatorsByHeight send", "err", err)
		return nil, err
	}
	msg, err = client.GetQueueClient().Wait(msg)
	if err != nil {
		porslog.Info("QueryValidatorsByHeight result", "err", err)
		return nil, err
	}
	return msg.GetData().(types.Message).(*tmtypes.PorsNodes), nil
}

// QueryBlockInfoByHeight get blockInfo and block by height
func (client *Client) QueryBlockInfoByHeight(height int64) (*tmtypes.PorsBlockInfo, *types.Block, error) {
	if height < 1 {
...
func (client *Client) LoadBlockCommit(height int64) *tmtypes.PorsCommit {
	blockInfo, _, err := client.QueryBlockInfoByHeight(height)
	if err != nil {
		porslog.Error("LoadBlockCommit GetBlockInfo fail", "err", err)
		return nil
	}
	if height > 1 {
		...
		if (seq == 0 && voteType != uint32(ttypes.VoteTypePrecommit)) ||
			...
			porslog.Error("LoadBlockCommit wrong VoteType", "seq", seq, "voteType", voteType)
			return nil
		}
	}
...
func (client *Client) LoadBlockState(height int64) *tmtypes.PorsState {
	blockInfo, _, err := client.QueryBlockInfoByHeight(height)
	if err != nil {
		porslog.Error("LoadBlockState GetBlockInfo fail", "err", err)
		return nil
	}
...
func (client *Client) LoadProposalBlock(height int64) *tmtypes.PorsBlock {
	blockInfo, block, err := client.QueryBlockInfoByHeight(height)
	if err != nil {
		porslog.Error("LoadProposal GetBlockInfo fail", "err", err)
		return nil
	}
...
func (client *Client) Query_IsHealthy(req *types.ReqNil) (types.Message, error) {
	...
	return &tmtypes.PorsIsHealthy{IsHealthy: isHealthy}, nil
}
...
func (client *Client) Query_NodeInfo(req *types.ReqNil) (types.Message, error) {
	...
	nodes := make([]*tmtypes.PorsNodeInfo, 0)
	for _, val := range vals {
		if val == nil {
			nodes = append(nodes, &tmtypes.PorsNodeInfo{})
		} else {
			ipstr, idstr := "UNKOWN", "UNKOWN"
			pub, err := ttypes.ConsensusCrypto.PubKeyFromBytes(val.PubKey)
			if err != nil {
				porslog.Error("Query_NodeInfo invalid pubkey", "err", err)
			} else {
				...
				if id == client.node.ID {
					...
					if ip == nil {
						porslog.Error("Query_NodeInfo nil ip", "id", idstr)
					} else {
						ipstr = ip.String()
					}
				}
	...
	return &tmtypes.PorsNodeInfoSet{Nodes: nodes}, nil
}

...
type ConsensusState struct {
	...
	ttypes.RoundState
	state State // PorsState until height-1.
	...
	setProposal    func(proposal *tmtypes.PorsProposal) error
...
func (cs *ConsensusState) LoadCommit(height int64) *tmtypes.PorsCommit {
	cs.mtx.Lock()
	defer cs.mtx.Unlock()
...
func (cs *ConsensusState) Stop() {
	...
	cs.quit <- struct{}{}
	porslog.Info("consensus state closed")
}
...
func (cs *ConsensusState) sendInternalMessage(mi MsgInfo) {
	...
	default:
		...
		porslog.Info("Internal msg queue is full. Using a go-routine")
...
func (cs *ConsensusState) reconstructLastCommit(state State) {
	...
	commit := ttypes.Commit{PorsCommit: seenCommit}
	...
	if round > 0 {
		...
		porslog.Info("reconstructLastCommit validator change", "round", round,
...
func (cs *ConsensusState) updateToState(state State) {
	...
	if !cs.state.IsEmpty() && (state.LastBlockHeight <= cs.state.LastBlockHeight) {
		porslog.Info("Ignoring updateToState()", "newHeight", state.LastBlockHeight+1, "oldHeight", cs.state.LastBlockHeight+1)
		return
	}
	...
	if state.Sequence > 0 && round > 0 {
		validators.IncrementAccum(round)
		porslog.Info("updateToState validator change", "round", round,
			"proposer", fmt.Sprintf("%X", ttypes.Fingerprint(validators.Proposer.Address)))
	}
...
func (cs *ConsensusState) receiveRoutine(maxSteps int) {
	defer func() {
		if r := recover(); r != nil {
			porslog.Error("CONSENSUS FAILURE!!!", "err", r, "stack", string(debug.Stack()))
		}
	}()
	...
	for {
		if maxSteps > 0 {
			if cs.nSteps >= maxSteps {
				porslog.Info("reached max steps. exiting receive routine")
				cs.nSteps = 0
				return
			}
		}
...
func (cs *ConsensusState) handleMsg(mi MsgInfo) {
	...
	switch msg := msg.(type) {
	case *tmtypes.PorsProposal:
		// will not cause transition.
		// once proposal is set, we can receive block parts
		err = cs.setProposal(msg)
		if err != nil {
			porslog.Error("handleMsg proposalMsg failed", "msg", msg, "error", err)
		}
	case *tmtypes.PorsBlock:
		...
		if err != nil && cs.Round != int(msg.Header.Round) {
			porslog.Info("Received block from wrong round", "height", cs.Height, "csRound", cs.Round, "blockRound", msg.Header.Round)
			err = nil
		}
	case *tmtypes.PorsVote:
	...
	case *tmtypes.PorsAggVote:
		err = cs.tryAddAggVote(msg, peerID)
	default:
		porslog.Error("Unknown msg type", msg.String(), "peerid", peerID, "peerip", peerIP)
	}
	if err != nil {
		//porslog.Error("Error with msg", "type", reflect.TypeOf(msg), "peerid", peerID, "peerip", peerIP, "err", err, "msg", msg)
	}
}

func (cs *ConsensusState) handleTimeout(ti timeoutInfo, rs ttypes.RoundState) {
	porslog.Info("Received tock", "timeout", ti.String())

	// timeouts must be for current height, round, step
	if ti.Height != rs.Height || ti.Round < rs.Round || (ti.Round == rs.Round && ti.Step < rs.Step) {
		porslog.Info("Ignoring tock because we're ahead", "height", rs.Height, "round", rs.Round, "step", rs.Step)
		return
	}
...
func (cs *ConsensusState) checkTxsAvailable() {
	for {
		...
		case height := <-cs.client.TxsAvailable():
			rs := cs.GetRoundState()
			if rs.Height != height {
				porslog.Info(fmt.Sprintf("blockchain(H: %v) and consensus(H: %v) are not sync", height, rs.Height))
				break
			}
			cs.txsAvailable <- height
		case <-cs.client.ctx.Done():
			porslog.Info("checkTxsAvailable quit")
			return
		}
	}
}
...
func (cs *ConsensusState) enterNewRound(height int64, round int) {
	if cs.Height != height || round < cs.Round || (cs.Round == round && cs.Step != ttypes.RoundStepNewHeight) {
		porslog.Info(fmt.Sprintf("enterNewRound(%v/%v): Invalid args. Current step: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step))
		return
	}

	if now := time.Now(); cs.StartTime.After(now) {
		porslog.Info("Need to set a buffer and log message here for sanity.", "startTime", cs.StartTime, "now", now)
	}

	porslog.Info(fmt.Sprintf("enterNewRound(%v/%v). Current: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step))
	...
	if cs.Round < round {
		...
		porslog.Info("enterNewRound validator change", "cs.Round", cs.Round, "round", round,
			"proposer", fmt.Sprintf("%X", ttypes.Fingerprint(validators.Proposer.Address)))
	}
	// Setup new round
	...
	if round == 0 || round == int(cs.state.LastCommitRound) {
		...
		porslog.Info("Resetting PorsProposal info")
		cs.Proposal = nil
		cs.ProposalBlock = nil
		cs.ProposalBlockHash = nil
	}
...
func (cs *ConsensusState) proposalHeartbeat(height int64, round int) {
	...
	for {
		...
		heartbeat := &tmtypes.PorsHeartbeat{
			Height:           rs.Height,
		...
		heartbeatMsg := &ttypes.Heartbeat{PorsHeartbeat: heartbeat}
		err := cs.GetPrivValidator().SignHeartbeat(chainID, heartbeatMsg)
		if err != nil {
			porslog.Error("SignHeartbeat failed", "err", err)
			continue
		}
...
func (cs *ConsensusState) enterPropose(height int64, round int) {
	if cs.Height != height || round < cs.Round || (cs.Round == round && ttypes.RoundStepPropose <= cs.Step) {
		porslog.Info(fmt.Sprintf("enterPropose(%v/%v): Invalid args. Current step: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step))
		return
	}
	porslog.Info(fmt.Sprintf("enterPropose(%v/%v). Current: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step))
	...
	if cs.privValidator == nil {
		porslog.Info("This node is not a validator")
		return
	}
	...
	porslog.Info("This node is a validator")

	if cs.isProposer() {
		porslog.Info("enterPropose: Our turn to propose",
			"proposer", fmt.Sprintf("%X", ttypes.Fingerprint(cs.Validators.GetProposer().Address)),
			"privValidator", fmt.Sprintf("%X", ttypes.Fingerprint(cs.privValidator.GetAddress())))
		cs.decideProposal(height, round)
	} else {
		porslog.Info("enterPropose: Not our turn to propose",
			"proposer", fmt.Sprintf("%X", ttypes.Fingerprint(cs.Validators.GetProposer().Address)),
			"privValidator", fmt.Sprintf("%X", ttypes.Fingerprint(cs.privValidator.GetAddress())))
	}
}
...
func (cs *ConsensusState) getProposerID() ID {
	...
	if err != nil {
		porslog.Error("getProposerID fail", "err", err)
	}
...
func (cs *ConsensusState) defaultDecideProposal(height int64, round int) {
	...
	if cs.ValidBlock != nil {
		if preExec.Load().(bool) {
			...
			if pblockNew == nil {
				porslog.Error("defaultDecideProposal PreExecBlock fail")
				return
			}
	...
	// Make proposal
	propBlockID := tmtypes.PorsBlockID{Hash: block.Hash()}
	...
	if err := cs.privValidator.SignProposal(cs.state.ChainID, proposal); err == nil {
		...
		cs.sendInternalMessage(MsgInfo{ttypes.ProposalID, &proposal.PorsProposal, cs.ourID, ""})
		cs.sendInternalMessage(MsgInfo{ttypes.ProposalBlockID, block.PorsBlock, cs.ourID, ""})
		porslog.Info("Signed proposal", "height", height, "round", round, "proposal", proposal)
	} else {
		porslog.Error("enterPropose: Error signing proposal", "height", height, "round", round, "err", err)
	}
}
...
func (cs *ConsensusState) createProposalBlock() (block *ttypes.PorsBlock) {
	var commit *tmtypes.PorsCommit
	if cs.Height == 1 {
		// We're creating a proposal for the first block.
		// The commit is empty, but not nil.
		commit = &tmtypes.PorsCommit{}
	} else if cs.LastCommit.HasTwoThirdsMajority() {
		// Make the commit from LastCommit
		commit = cs.LastCommit.MakeCommonCommit()
	} else {
		// This shouldn't happen.
		porslog.Error("enterPropose: Cannot propose anything: No commit for the previous block.")
		return nil
	}
	...
	if pblock == nil {
		porslog.Error("createProposalBlock BuildBlock fail")
		return nil
	}
	porslog.Info(fmt.Sprintf("createProposalBlock BuildBlock. Current: %v/%v/%v", cs.Height, cs.Round, cs.Step),
		"height", pblock.Height, "txs-len", len(pblock.Txs), "cost", types.Since(beg))

	if pblock.Height != cs.Height {
		porslog.Error("pblock.Height is not equal to cs.Height")
		return nil
	}

	proposerAddr := cs.privValidator.GetAddress()
	block = cs.state.MakeBlock(cs.Height, int64(cs.Round), pblock, commit, proposerAddr)
	baseTx := cs.createBaseTx(block.PorsBlock)
	if baseTx == nil {
		porslog.Error("createProposalBlock createBaseTx fail")
		return nil
	}
	...
	if preExec.Load().(bool) {
		...
		if pblockNew == nil {
			porslog.Error("createProposalBlock PreExecBlock fail")
			return nil
		}
...
func (cs *ConsensusState) createBaseTx(block *tmtypes.PorsBlock) *types.Transaction {
	var state *tmtypes.PorsState
	if cs.Height == 1 {
...
func (cs *ConsensusState) enterPrevote(height int64, round int) {
	if cs.Height != height || round < cs.Round || (cs.Round == round && ttypes.RoundStepPrevote <= cs.Step) {
		porslog.Info(fmt.Sprintf("enterPrevote(%v/%v): Invalid args. Current step: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step))
		return
	}
	...
	porslog.Info(fmt.Sprintf("enterPrevote(%v/%v). Current: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step), "cost", types.Since(cs.begCons))
...
func (cs *ConsensusState) defaultDoPrevote(height int64, round int) {
	...
	if cs.LockedBlock != nil {
		porslog.Info("enterPrevote: Block was locked", "hash", fmt.Sprintf("%X", cs.LockedBlock.Hash()))
	...
	if cs.ProposalBlock == nil {
		porslog.Info("enterPrevote: ProposalBlock is nil")
	...
	if err != nil {
		...
		porslog.Error("enterPrevote: ProposalBlock is invalid", "err", err)
		cs.signAddVote(ttypes.VoteTypePrevote, nil)
		return
	}
	...
	if preExec.Load().(bool) && !cs.isProposer() {
		...
		if blockNew == nil {
			porslog.Error("enterPrevote: PreExec ProposalBlock fail")
			cs.signAddVote(ttypes.VoteTypePrevote, nil)
			return
		}
		cfg := cs.client.GetQueueClient().GetConfig()
		if !bytes.Equal(blockNew.Hash(cfg), cs.ProposalBlock.Data.Hash(cfg)) {
			porslog.Error("enterPrevote: PreExec ProposalBlock has change")
			cs.signAddVote(ttypes.VoteTypePrevote, nil)
			return
		}
	}
	...
	porslog.Info("enterPrevote: ProposalBlock is valid")
...
func (cs *ConsensusState) enterAggPrevoteWait(height int64, round int) {
	if cs.Height != height || round < cs.Round || (cs.Round == round && ttypes.RoundStepAggPrevoteWait <= cs.Step) {
		porslog.Info(fmt.Sprintf("enterAggPrevoteWait(%v/%v): Invalid args. Current step: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step))
		return
	}
	porslog.Info(fmt.Sprintf("enterAggPrevoteWait(%v/%v). Current: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step))
...
func (cs *ConsensusState) enterPrevoteWait(height int64, round int) {
	if cs.Height != height || round < cs.Round || (cs.Round == round && ttypes.RoundStepPrevoteWait <= cs.Step) {
		porslog.Info(fmt.Sprintf("enterPrevoteWait(%v/%v): Invalid args. Current step: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step))
		return
	}
	if !cs.Votes.Prevotes(round).HasTwoThirdsAny() {
		panic(fmt.Sprintf("enterPrevoteWait(%v/%v), but Prevotes does not have any +2/3 votes", height, round))
	}
	porslog.Info(fmt.Sprintf("enterPrevoteWait(%v/%v). Current: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step))
...
func (cs *ConsensusState) enterPrecommit(height int64, round int) {
	if cs.Height != height || round < cs.Round || (cs.Round == round && ttypes.RoundStepPrecommit <= cs.Step) {
		porslog.Info(fmt.Sprintf("enterPrecommit(%v/%v): Invalid args. Current step: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step))
		return
	}

	porslog.Info(fmt.Sprintf("enterPrecommit(%v/%v). Current: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step), "cost", types.Since(cs.begCons))
	...
	if !ok {
		if cs.LockedBlock != nil {
			porslog.Info("enterPrecommit: No +2/3 prevotes during enterPrecommit while we're locked. Precommitting nil")
		} else {
			porslog.Info("enterPrecommit: No +2/3 prevotes during enterPrecommit. Precommitting nil")
		}
	...
	if len(blockID.Hash) == 0 {
		if cs.LockedBlock == nil {
			porslog.Info("enterPrecommit: +2/3 prevoted for nil.")
		} else {
			porslog.Info("enterPrecommit: +2/3 prevoted for nil. Unlocking")
			cs.LockedRound = -1
			cs.LockedBlock = nil
		}
		cs.signAddVote(ttypes.VoteTypePrecommit, nil)
	...
	if cs.LockedBlock.HashesTo(blockID.Hash) {
		porslog.Info("enterPrecommit: +2/3 prevoted locked block. Relocking", "hash", fmt.Sprintf("%X", blockID.Hash))
	...
	if cs.ProposalBlock.HashesTo(blockID.Hash) {
		porslog.Info("enterPrecommit: +2/3 prevoted proposal block. Locking", "hash", fmt.Sprintf("%X", blockID.Hash))
		// Validate the block.
	...
	porslog.Info("enterPrecommit: +2/3 prevotes for a block we do not have. Precommitting nil", "hash", fmt.Sprintf("%X", blockID.Hash))
	cs.LockedRound = -1
	cs.LockedBlock = nil
...
func (cs *ConsensusState) enterAggPrecommitWait(height int64, round int) {
	if cs.Height != height || round < cs.Round || (cs.Round == round && ttypes.RoundStepAggPrecommitWait < cs.Step) {
		porslog.Info(fmt.Sprintf("enterAggPrecommitWait(%v/%v): Invalid args. Current step: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step))
		return
	}
	porslog.Info(fmt.Sprintf("enterAggPrecommitWait(%v/%v). Current: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step))
	...
	//cs.broadcastChannel <- MsgInfo{TypeID: ttypes.VoteID, Msg: vote.PorsVote, PeerID: cs.ourID, PeerIP: ""}
}

// Enter: any +2/3 precommits for next round.
func (cs *ConsensusState) enterPrecommitWait(height int64, round int) {
	if cs.Height != height || round < cs.Round || (cs.Round == round && ttypes.RoundStepPrecommitWait <= cs.Step) {
		porslog.Info(fmt.Sprintf("enterPrecommitWait(%v/%v): Invalid args. Current step: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step))
		return
	}
	if !cs.Votes.Precommits(round).HasTwoThirdsAny() {
		panic(fmt.Sprintf("enterPrecommitWait(%v/%v), but Precommits does not have any +2/3 votes", height, round))
	}
	porslog.Info(fmt.Sprintf("enterPrecommitWait(%v/%v). Current: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step))
...
func (cs *ConsensusState) enterCommit(height int64, commitRound int) {
	if cs.Height != height || ttypes.RoundStepCommit <= cs.Step {
		porslog.Info(fmt.Sprintf("enterCommit(%v/%v): Invalid args. Current step: %v/%v/%v", height, commitRound, cs.Height, cs.Round, cs.Step))
		return
	}
	porslog.Info(fmt.Sprintf("enterCommit(%v/%v). Current: %v/%v/%v", height, commitRound, cs.Height, cs.Round, cs.Step), "cost", types.Since(cs.begCons))
	...
	var blockID tmtypes.PorsBlockID
	var ok bool
	...
	if cs.LockedBlock.HashesTo(blockID.Hash) {
		porslog.Info("Commit is for locked block. Set ProposalBlock=LockedBlock", "LockedBlock-hash", fmt.Sprintf("%X", blockID.Hash))
	...
	if !cs.ProposalBlock.HashesTo(blockID.Hash) {
		porslog.Info("Commit is for a block we don't know about. Set ProposalBlock=nil",
		...
		validBlockMsg := &tmtypes.PorsValidBlockMsg{
			Height:    cs.Height,
...
func (cs *ConsensusState) tryFinalizeCommit(height int64) {
	...
	if !ok || len(blockID.Hash) == 0 {
		porslog.Error("Attempt to finalize failed. There was no +2/3 majority, or +2/3 was for <nil>.")
		porslog.Info(fmt.Sprintf("Continue consensus. Current: %v/%v/%v", cs.Height, cs.CommitRound, cs.Step), "cost", types.Since(cs.begCons))
		return
	}
	if !cs.ProposalBlock.HashesTo(blockID.Hash) {
		// TODO: this happens every time if we're not a validator (ugly logs)
		// TODO: ^^ wait, why does it matter that we're a validator?
		porslog.Error("Attempt to finalize failed. We don't have the commit block.",
...
func (cs *ConsensusState) finalizeCommit(height int64) {
	if cs.Height != height || cs.Step != ttypes.RoundStepCommit {
		porslog.Info(fmt.Sprintf("finalizeCommit(%v): Invalid args. Current step: %v/%v/%v", height, cs.Height, cs.Round, cs.Step))
		return
	}

	block := cs.ProposalBlock
	stateCopy := cs.state.Copy()
	porslog.Info("First finalizeCommit validators of statecopy", "validators", stateCopy.Validators)
	// NOTE: the block.AppHash wont reflect these txs until the next block
	var err error
	stateCopy, err = cs.blockExec.ApplyBlock(stateCopy, ttypes.BlockID{PorsBlockID: &tmtypes.PorsBlockID{Hash: block.Hash()}}, block)
	if err != nil {
	...
	if bytes.Equal(cs.privValidator.GetAddress(), block.PorsBlock.Header.ProposerAddr) {
		porslog.Info(fmt.Sprintf("Proposer reach consensus. Current: %v/%v/%v", cs.Height, cs.Round, cs.Step),
			"CommitRound", cs.CommitRound, "tx-len", len(commitBlock.Txs), "cost", types.Since(cs.begCons),
			"proposer-addr", fmt.Sprintf("%X", ttypes.Fingerprint(block.PorsBlock.Header.ProposerAddr)),
			"seq", block.Header.Sequence)
	} else {
		porslog.Info(fmt.Sprintf("Not-Proposer reach consensus. Current: %v/%v/%v", cs.Height, cs.Round, cs.Step),
			"CommitRound", cs.CommitRound, "tx-len", len(commitBlock.Txs), "cost", types.Since(cs.begCons),
			"proposer-addr", fmt.Sprintf("%X", ttypes.Fingerprint(block.PorsBlock.Header.ProposerAddr)),
			"seq", block.Header.Sequence)
	}
	...
	porslog.Info("After Apply finalizeCommit validators of statecopy", "validators", stateCopy.Validators)
	...
	if err == nil && qbftNodes != nil {
		if len(qbftNodes.Nodes) > 0 {
			porslog.Warn("finalizeCommit validators of statecopy update", "update-qbftNodes", qbftNodes)
			...
			if err != nil {
				porslog.Error("Error changing validator set", "error", err)
			}
			porslog.Info("nextVal finalizeCommit validators of statecopy", "nextValSet", nextValSet)
			// change results from this height but only applies to the next height
			stateCopy.LastHeightValidatorsChanged = block.Header.Height + 1
			porslog.Info("before update to nextVal finalizeCommit validators of statecopy", "validators", stateCopy.Validators.String())
			stateCopy.Validators = nextValSet

		}
	}
	porslog.Info("Last finalizeCommit validators of statecopy", "validators", stateCopy.Validators.String())

	// save local state and seen commit
	var seenCommit *tmtypes.PorsCommit
	...
	porslog.Info(fmt.Sprintf("Save consensus state. Current: %v/%v/%v", cs.Height, cs.CommitRound, cs.Step), "cost", types.Since(cs.begCons))
...
func (cs *ConsensusState) defaultSetProposal(proposal *tmtypes.PorsProposal) error {
	porslog.Info(fmt.Sprintf("Consensus receive proposal. Current: %v/%v/%v", cs.Height, cs.Round, cs.Step),
		"proposal", fmt.Sprintf("%v/%v", proposal.Height, proposal.Round))
	// Already have one
	// TODO: possibly catch double proposals
	if cs.Proposal != nil {
		porslog.Info("defaultSetProposal: already has proposal")
		return nil
	}

	// Does not apply
	if proposal.Height != cs.Height || int(proposal.Round) != cs.Round {
		porslog.Info("defaultSetProposal: height is not equal or round is not equal", "proposal-height", proposal.Height, "cs-height", cs.Height,
	...
	if proposal.Sequence != cs.state.Sequence {
		porslog.Error("invalid proposal sequence", "proposal-seq", proposal.Sequence,
	...
	if proposal.POLRound < -1 ||
		...
		porslog.Error("Invalid POLRound", "POLRound", proposal.POLRound, "Round", proposal.Round)
	...
	proposalTmp := &ttypes.Proposal{PorsProposal: *proposal}
	...
	porslog.Info(fmt.Sprintf("Consensus set proposal. Current: %v/%v/%v", cs.Height, cs.Round, cs.Step),
...
func (cs *ConsensusState) addProposalBlock(proposalBlock *tmtypes.PorsBlock) (err error) {
	block := &ttypes.PorsBlock{PorsBlock: proposalBlock}
	height, round := block.Header.Height, block.Header.Round
	porslog.Info(fmt.Sprintf("Consensus receive proposal block. Current: %v/%v/%v", cs.Height, cs.Round, cs.Step),
		"block(H/R/hash)", fmt.Sprintf("%v/%v/%X", height, round, block.Hash()))

	// Blocks might be reused, so round mismatch is OK
	if cs.Height != height {
		porslog.Info("Received block from wrong height", "height", height, "round", round)
		return nil
	}
	...
	if block.Header.Sequence != cs.state.Sequence || block.Header.LastSequence != cs.state.LastSequence {
		porslog.Error("invalid proposalBlock sequence", "proposalBlock-seq", block.Header.Sequence,
	...
	if block.Header.Sequence > 0 && !bytes.Equal(block.Header.ProposerAddr, cs.Validators.GetProposer().Address) {
		porslog.Error("invalid proposalBlock proposer",
	...
	if !block.HashesTo(cs.ProposalBlockHash) {
		...
		porslog.Info("Received block when we're not expecting any", "ProposalBlockHash", fmt.Sprintf("%X", cs.ProposalBlockHash),
	...
	if block.HashesTo(cs.ProposalBlock.Hash()) {
		porslog.Info("addProposalBlock: already has proposal block")
		return nil
	}
	...
	porslog.Info(fmt.Sprintf("Consensus set proposal block. Current: %v/%v/%v", cs.Height, cs.Round, cs.Step),
		"ProposalBlockHash", fmt.Sprintf("%X", cs.ProposalBlockHash), "cost", types.Since(cs.begCons))

	if DetachExec() {
		porslog.Info("write proposal block in advance")
	...
	if hasTwoThirds && len(blockID.Hash) == 0 && (cs.ValidRound < cs.Round) {
		if cs.ProposalBlock.HashesTo(blockID.Hash) {
			porslog.Info("Updating valid block to new proposal block",
...
func (cs *ConsensusState) tryAddAggVote(aggVoteRaw *tmtypes.PorsAggVote, peerID string) error {
	aggVote := &ttypes.AggVote{PorsAggVote: aggVoteRaw}
	if aggVote.Height != cs.Height {
		porslog.Info("PorsAggVote ignored and not added", "voteHeight", aggVote.Height, "csHeight", cs.Height, "peerID", peerID)
	...
	if err != nil {
		porslog.Error("Error attempting to add aggregate vote", "err", err)
	...
	case uint32(ttypes.VoteTypePrevote):
		...
		porslog.Info("Added aggregate prevote", "aggVote", aggVote, "prevotes", prevotes.StringShort())
		...
		if blockID, ok := prevotes.TwoThirdsMajority(); ok {
			...
			if (cs.LockedBlock != nil) &&
				...
				porslog.Info("Unlocking because of POL.", "lockedRound", cs.LockedRound, "POLRound", aggVote.Round)
				cs.LockedRound = -1
				cs.LockedBlock = nil
			}
			...
			if len(blockID.Hash) != 0 && (cs.ValidRound < int(aggVote.Round)) && (int(aggVote.Round) == cs.Round) {
				...
				if cs.ProposalBlock.HashesTo(blockID.Hash) {
					porslog.Info("Updating ValidBlock because of POL.", "validRound", cs.ValidRound, "POLRound", aggVote.Round)
	...
	case uint32(ttypes.VoteTypePrecommit):
		...
		porslog.Info("Added aggregate precommit", "aggVote", aggVote, "precommits", precommits.StringShort())
...
func (cs *ConsensusState) tryAddVote(voteRaw *tmtypes.PorsVote, peerID string, peerIP string) error {
	vote := &ttypes.Vote{PorsVote: voteRaw}
	...
	if err != nil {
		...
		if err == ErrVoteHeightMismatch {
			...
			if bytes.Equal(vote.ValidatorAddress, cs.privValidator.GetAddress()) {
				porslog.Error("Found conflicting vote from ourselves. Did you unsafe_reset a validator?",
			...
			porslog.Error("Error attempting to add vote", "err", err)
...
func (cs *ConsensusState) addVote(vote *ttypes.Vote, peerID string, peerIP string) (added bool, err error) {
	porslog.Info(fmt.Sprintf("Consensus receive vote. Current: %v/%v/%v", cs.Height, cs.Round, cs.Step),
	...
	if vote.Height+1 == cs.Height {
		...
		porslog.Info(fmt.Sprintf("Added to lastPrecommits: %v", cs.LastCommit.StringShort()))

		hasVoteMsg := &tmtypes.PorsHasVoteMsg{
			Height: vote.Height,
			Round:  vote.Round,
	...
	if vote.Height != cs.Height {
		...
		porslog.Info("PorsVote ignored and not added", "voteHeight", vote.Height, "csHeight", cs.Height, "peerID", peerID)
		return
	}
	...
	case uint32(ttypes.VoteTypePrevote):
		...
		porslog.Info("Added to prevote", "vote", vote, "prevotes", prevotes.StringShort())

		// If +2/3 prevotes for a block or nil for *any* round:
		if blockID, ok := prevotes.TwoThirdsMajority(); ok {
			porslog.Info("prevote enters twothirds majority")
			// There was a polka!
			// If we're locked but this is a recent polka, unlock.
			// If it matches our ProposalBlock, update the ValidBlock
			...
			if (cs.LockedBlock != nil) &&
				...
				porslog.Info("Unlocking because of POL.", "lockedRound", cs.LockedRound, "POLRound", vote.Round)
				cs.LockedRound = -1
				cs.LockedBlock = nil
			}
			...
			if len(blockID.Hash) != 0 && (cs.ValidRound < int(vote.Round)) && (int(vote.Round) == cs.Round) {
				...
				if cs.ProposalBlock.HashesTo(blockID.Hash) {
					porslog.Info("Updating ValidBlock because of POL.", "validRound", cs.ValidRound, "POLRound", vote.Round)
			...
			if ok {
				if ttypes.RoundStepPrevote <= cs.Step && (cs.isProposalComplete() || len(blockID.Hash) == 0) {
					if UseAggSig() && cs.isProposer() && prevotes.GetAggVote() == nil {
						...
						if err != nil {
							porslog.Error("prevotes SetAggVote fail", "err", err)
							break
						}
						aggVoteMsg := prevotes.GetAggVote().PorsAggVote
						cs.broadcastChannel <- MsgInfo{TypeID: ttypes.AggVoteID, Msg: aggVoteMsg, PeerID: "", PeerIP: ""}
						porslog.Info("Send aggregate prevote", "aggVote", prevotes.GetAggVote())
					}
					if cs.state.Sequence == 0 || len(blockID.Hash) == 0 {
	...
	case uint32(ttypes.VoteTypePrecommit):
		...
		porslog.Info("Added to precommit", "vote", vote, "precommits", precommits.StringShort())
		...
		if ok {
			...
			if UseAggSig() && cs.isProposer() && precommits.GetAggVote() == nil {
				...
				if err != nil {
					porslog.Error("precommits SetAggVote fail", "err", err)
					break
				}
				aggVoteMsg := precommits.GetAggVote().PorsAggVote
				cs.broadcastChannel <- MsgInfo{TypeID: ttypes.AggVoteID, Msg: aggVoteMsg, PeerID: "", PeerIP: ""}
				porslog.Info("Send aggregate precommit", "aggVote", precommits.GetAggVote())
			}
			if len(blockID.Hash) != 0 {
				cs.enterCommit(height, int(vote.Round))
				if skipTimeoutCommit.Load().(bool) && precommits.HasAll() {
...
func (cs *ConsensusState) signVote(voteType byte, hash []byte) (*ttypes.Vote, error) {
	addr := cs.privValidator.GetAddress()
	valIndex, _ := cs.Validators.GetByAddress(addr)
	tVote := &tmtypes.PorsVote{}
	tVote.BlockID = &tmtypes.PorsBlockID{Hash: hash}
	vote := &ttypes.Vote{
		PorsVote: &tmtypes.PorsVote{
			ValidatorAddress: addr,
			ValidatorIndex:   int32(valIndex),
			Height:           cs.Height,
			Round:            int32(cs.Round),
			...
			BlockID:          &tmtypes.PorsBlockID{Hash: hash},
			Signature:        nil,
			UseAggSig:        UseAggSig(),
		},
	}
...
func (cs *ConsensusState) signAddVote(voteType byte, hash []byte) *ttypes.Vote {
	...
	if cs.privValidator == nil || !cs.Validators.HasAddress(cs.privValidator.GetAddress()) {
		porslog.Error("invalid privValidator, check whether privkey file is overridden", "height", cs.Height, "round", cs.Round)
		return nil
	}
	vote, err := cs.signVote(voteType, hash)
	if err == nil {
		// send to self
		cs.sendInternalMessage(MsgInfo{TypeID: ttypes.VoteID, Msg: vote.PorsVote, PeerID: cs.ourID, PeerIP: ""})
		if UseAggSig() {
			// send to proposer
			cs.unicastChannel <- MsgInfo{TypeID: ttypes.VoteID, Msg: vote.PorsVote, PeerID: cs.getProposerID(), PeerIP: ""}
		...
		porslog.Info("Sign and send vote", "height", cs.Height, "round", cs.Round, "vote", vote)
		return vote
	}

	porslog.Error("Error signing vote", "height", cs.Height, "round", cs.Round, "vote", vote, "err", err)
	return nil
}
...
func (cs *ConsensusState) GetPrevotesState(height int64, round int, blockID *tmtypes.PorsBlockID) *ttypes.BitArray {
	if height != cs.Height {
		return nil
	}
...
func (cs *ConsensusState) GetPrecommitsState(height int64, round int, blockID *tmtypes.PorsBlockID) *ttypes.BitArray {
	if height != cs.Height {
		return nil
	}
...
func (cs *ConsensusState) SetPeerMaj23(height int64, round int, voteType byte, peerID ID, blockID *tmtypes.PorsBlockID) {
	if height == cs.Height {

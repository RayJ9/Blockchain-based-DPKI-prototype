...
import (
	...
	vrf "code.corp.bcollie.net/omnilink/omnilink-base/common/vrf/secp256k1"
	"code.corp.bcollie.net/omnilink/omnilink-base/queue"
	drivers "code.corp.bcollie.net/omnilink/omnilink-base/system/consensus"
	driver "code.corp.bcollie.net/omnilink/omnilink-base/system/dapp"
...
//316190000 coins
func createTicket(cfg *types.OmnilinkConfig, minerAddr, returnAddr string, count int32, height int64) (ret []*types.Transaction) {
...
func (client *Client) CheckBlock(parent *types.Block, current *types.BlockDetail) error {
	omnilinkCfg := client.GetAPI().GetConfig()
	cfg := ty.GetTicketMinerParam(omnilinkCfg, current.Block.Height)
	...
	//vrf verify
	if omnilinkCfg.IsDappFork(current.Block.Height, ty.TicketX, "ForkTicketVrf") {
		var input []byte
...
func (client *Client) getNextRequiredDifficulty(block *types.Block, bits uint32) (uint32, []byte, error) {
	...
	// Genesis block.
	if block == nil {
		return omnilinkCfg.GetP(0).PowLimitBits, defaultModify, nil
	}
	powLimitBits := omnilinkCfg.GetP(block.Height).PowLimitBits
	cfg := ty.GetTicketMinerParam(omnilinkCfg, block.Height)
...
func (client *Client) updateBlock(block *types.Block, txHashList [][]byte) (txList [][]byte) {
	...
	block.ParentHash = lastBlock.Hash(omnilinkCfg)
	block.Height = lastBlock.Height + 1
	cfg := omnilinkCfg.GetP(block.Height)

...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-base/system/crypto/none"
	"code.corp.bcollie.net/omnilink/omnilink-base/types"
	ty "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/valnode/types"
...
func main() {
	...
	if strings.Contains(os.Args[0], "para") {
		...
		var err error
		execAddr, err = address.GetExecAddress(testExec, 2)
		if err != nil {
			panic(err.Error())
		}
	}
			...
			for {
				...
				default:
					...
					for txs := 0; txs < batchNum; txs++ {
						...
						tx.Payload = RandStringBytes(sizeInt)
						//交易签名
						//tx.Sign(types.SECP256K1ETH, priv)
...
func Put(ip string, size string, privkey string) {
	...
	tx.Sign(types.SECP256K1ETH, getprivkey(privkey))
	poststr := fmt.Sprintf(`{"jsonrpc":"2.0","id":2,"method":"Omnilink.SendTransaction","params":[{"data":"%v"}]}`,
...
func Get(ip string, hash string) {
	...
	poststr := fmt.Sprintf(`{"jsonrpc":"2.0","id":2,"method":"Omnilink.QueryTransaction","params":[{"hash":"%s"}]}`, hash)
	resp, err := http.Post(url, "application/json", bytes.NewBufferString(poststr))
	if err != nil {
		fmt.Println(err)
		return
...
func setTxHeight(ip string) {
	url := "http://" + ip + ":8801"
	poststr := fmt.Sprintf(`{"jsonrpc":"2.0","id":2,"method":"Omnilink.GetLastHeader","params":[]}`)
	resp, err := http.Post(url, "application/json", bytes.NewBufferString(poststr))
	if err != nil {
		fmt.Println(err)
		return
...
func getprivkey(key string) crypto.PrivKey {
	cr, err := crypto.Load(types.GetSignName("", types.SECP256K1ETH), -1)
	if err != nil {
		panic(err)
	}
...
func genaddress() (string, crypto.PrivKey) {
	...
	addrto := address.PubKeyToAddr(2, privto.PubKey().Bytes())

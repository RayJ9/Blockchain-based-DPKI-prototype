...
CLI="docker exec ${NODE1} /root/omnilink-cli"
...
function para_init() {
    ...
    for auth in "${authAccount[@]}"; do
        tomlfile="omnilink.para.$index.toml"
...
function para_import_wallet() {
    ...
    ./$OMNILINK_CLI --rpc_laddr "http://localhost:$port" seed save -p 1314fuzamei -s "tortoise main civil member grace happy century convince father cage beach hip maid merry rib"

    echo "=========== # unlock wallet ============="
    ./$OMNILINK_CLI --rpc_laddr "http://localhost:$port" wallet unlock -p 1314fuzamei -t 0

    echo "=========== # import private key ============="
    echo "key: ${key}"
    ./$OMNILINK_CLI --rpc_laddr "http://localhost:$port" account import_key -k "${key}" -l "paraAuthAccount"

    echo "=========== # close auto mining ============="
    ./$OMNILINK_CLI --rpc_laddr "http://localhost:$port" wallet auto_mine -f 0

    echo "=========== # wallet status ============="
    ./$OMNILINK_CLI --rpc_laddr "http://localhost:$port" wallet status
}

function para_unlock_wallet() {
    for ((i = 0; i < ${#authPort[@]}; i++)); do
        echo "=========== # para unlock wallet ${authPort[$i]}============="
        ./$OMNILINK_CLI --rpc_laddr "http://localhost:${authPort[$i]}" wallet unlock -p 1314fuzamei -t 0
    done
...
function start() {
    ...
    #    ./omnilink-cli --rpc_laddr http://localhost:18901 block last_header
...
function check_docker_status() {
    ...
    if [ "${status}" == "Exit" ] || [ "${statusPara}" == "Exit" ]; then
        echo "=========== omnilink service Exit logs ========== "
        docker-compose logs parachain1
        echo "=========== omnilink service Exit logs End========== "
    fi
...
EOF
  ...
  parachain$i:
    ...
    environment:
      PARAFILE: "/root/omnilink.para.$i.toml"
    ports:
     - "1890$i:8901"
    volumes:
...
function create_build() {
    rm -rf $buildpath
    mkdir -p $buildpath
    cp omnilink* Dockerfile ./*.sh "$buildpath"/
    cd $buildpath
    create_yml
}

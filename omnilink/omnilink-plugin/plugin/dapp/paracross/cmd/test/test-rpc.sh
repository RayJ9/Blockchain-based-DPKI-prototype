...
paracross_GetBlock2MainInfo() {
    omnilink_Http '{"method":"Omnilink.Query","params":[{ "execer":"paracross", "funcName": "GetBlock2MainInfo", "payload" : {"start":1,"end":3}}]}' ${UNIT_HTTP} '(.result.items[1].height == "2")' "$FUNCNAME"
}
...
function paracross_QueryParaBalance() {
    ...
    req='{"method":"Omnilink.GetBalance", "params":[{"addresses" : ["'"$1"'"], "execer" : "'"${exec}"'","asset_exec":"paracross","asset_symbol":"'"${symbol}"'"}]}'
...
function paracross_QueryMainBalance() {
    ...
    req='{"method":"Omnilink.GetBalance", "params":[{"addresses" : ["'"$1"'"], "execer" : "paracross"}]}'
...
function paracross_Transfer_Withdraw_Inner() {
    ...
    #2  存钱到合约地址
    tx_hash=$(curl -ksd '{"method":"Omnilink.CreateRawTransaction","params":[{"to":"'"$paracross_addr"'","amount":'$amount_save'}]}' ${UNIT_HTTP} | jq -r ".result")
    omnilink_SignAndSendTx "$tx_hash" "$privkey" ${UNIT_HTTP}
    ...
    #3  资产从主链转移到平行链
    tx_hash=$(curl -ksd '{"method":"Omnilink.CreateTransaction","params":[{"execer":"'"$execer_name"'","actionName":"ParacrossAssetTransfer","payload":{"execName":"'"$execer_name"'","to":"'"$from_addr"'","amount":'$amount_should'}}]}' ${UNIT_HTTP} | jq -r ".result")
    omnilink_SignAndSendTx "$tx_hash" "$privkey" ${UNIT_HTTP}
    ...
    while true; do
        ...
        if [ "$para_amount_real" != "$amount_should" ] || [ "$main_amount_real" != "$amount_should" ]; then
            omnilink_BlockWait 2 ${UNIT_HTTP}
            times=$((times - 1))
            if [ $times -le 0 ]; then
    ...
    #5 取钱
    tx_hash=$(curl -ksd '{"method":"Omnilink.CreateTransaction","params":[{"execer":"'"$execer_name"'","actionName":"ParacrossAssetWithdraw","payload":{"IsWithdraw":true,"execName":"'"$execer_name"'","to":"'"$from_addr"'","amount":'$withdraw_should'}}]}' ${UNIT_HTTP} | jq -r ".result")
    omnilink_SignAndSendTx "$tx_hash" "$privkey" ${UNIT_HTTP}
...
function paracross_IsSync() {
    if [ "$IS_PARA" == "true" ]; then
        ...
        req='{"method":"Omnilink.IsSync","params":[]}'
    fi
    omnilink_Http "$req" ${UNIT_HTTP} '(.error|not)' "$FUNCNAME"
}

function paracross_ListTitles() {
    local main_ip=${UNIT_HTTP//8901/8801}
    omnilink_Http '{"method":"Omnilink.Query","params":[{ "execer":"paracross", "funcName": "ListTitles", "payload" : {}}]}' ${main_ip} '(.error|not) and (.result| [has("titles"),true])' "$FUNCNAME"
}

function paracross_GetHeight() {
    if [ "$IS_PARA" == "true" ]; then
        omnilink_Http '{"method":"Omnilink.Query","params":[{ "execer":"paracross", "funcName": "GetHeight", "payload" : {}}]}' ${UNIT_HTTP} '(.error|not) and (.result| [has("consensHeight"),true])' "$FUNCNAME"
    fi
}

function paracross_GetNodeGroupAddrs() {
    omnilink_Http '{"method":"Omnilink.Query","params":[{ "execer":"paracross", "funcName":"GetNodeGroupAddrs","payload":{"title":"user.p.para."}}]}' ${UNIT_HTTP} '(.error|not) and (.result| [has("key","value"),true])' "$FUNCNAME"
}

function paracross_GetNodeGroupStatus() {
    omnilink_Http '{"method":"Omnilink.Query","params":[{ "execer":"paracross", "funcName":"GetNodeGroupStatus","payload":{"title":"user.p.para."}}]}' ${UNIT_HTTP} '(.error|not) and (.result| [has("status"),true])' "$FUNCNAME"
}

function paracross_ListNodeGroupStatus() {
    omnilink_Http '{"method":"Omnilink.Query","params":[{ "execer":"paracross", "funcName":"ListNodeGroupStatus","payload":{"title":"user.p.para.","status":2}}]}' ${UNIT_HTTP} '(.error|not) and (.result| [has("status"),true])' "$FUNCNAME"
}

function paracross_ListNodeStatus() {
    omnilink_Http '{"method":"Omnilink.Query","params":[{ "execer":"paracross", "funcName":"ListNodeStatusInfo","payload":{"title":"user.p.para.","status":3}}]}' ${UNIT_HTTP} '(.error|not) and (.result| [has("status"),true])' "$FUNCNAME"
}

function paracross_GetSupervisionInfo() {
    omnilink_Http '{"method":"Omnilink.Query","params":[{ "execer":"paracross", "funcName":"GetSupervisionNodeGroupAddrs","payload":{"title":"user.p.para."}}]}' ${UNIT_HTTP} '(.error|not) and (.result| [has("key","value"),true])' "GetSupervisionNodeGroupAddrs"
    omnilink_Http '{"method":"Omnilink.Query","params":[{ "execer":"paracross", "funcName":"ListSupervisionNodeStatusInfo","payload":{"title":"user.p.para.","status":0}}]}' ${UNIT_HTTP} '(.error|not) and (.result| [has("status"),true])' "ListSupervisionNodeStatusInfo status:0"
}
...
function paracross_txgroupex() {
    ...
    #  资产从主链转移到平行链
    req='"method":"Omnilink.CreateTransaction","params":[{"execer":"'"${paracross_execer_name}"'","actionName":"CrossAssetTransfer","payload":{"assetExec":"'"${coins_exec}"'","assetSymbol":"'"${bty_symbol}"'","toAddr":"'"${para_test_addr}"'","amount":'${amount_transfer}'}}]'
    echo "$req"
    ...
    #  资产从平行链转移到平行链合约
    req='"method":"Omnilink.CreateTransaction","params":[{"execer":"'"${paracross_execer_name}"'","actionName":"TransferToExec","payload":{"execName":"'"${paracross_execer_name}"'","to":"'"${trade_exec_addr}"'","amount":'${amount_trade}', "cointoken":"coins.bty"}}]'
    echo "$req"
    resp=$(curl -ksd "{$req}" "${para_ip}")
    echo "$resp"
    err=$(jq '(.error)' <<<"$resp")
    ...
    req='"method":"Omnilink.CreateNoBlanaceTxs","params":[{"txHexs":["'"${tx_hash_asset}"'","'"${tx_hash_transferExec}"'"],"privkey":"'"${para_test_prikey}"'","expire":"120s"}]'
    resp=$(curl -ksd "{$req}" "${para_ip}")
    err=$(jq '(.error)' <<<"$resp")
    if [ "$err" != null ]; then
        echo "$resp"
    ...
    #sign 1
    tx_sign=$(curl -ksd '{"method":"Omnilink.SignRawTx","params":[{"privkey":"'"$para_test_prikey"'","txHex":"'"$tx_hash_group"'","index":2,"expire":"120s"}]}' "${para_ip}" | jq -r ".result")
    #sign 2
    tx_sign2=$(curl -ksd '{"method":"Omnilink.SignRawTx","params":[{"privkey":"'"$para_test_prikey"'","txHex":"'"$tx_sign"'","index":3,"expire":"120s"}]}' "${para_ip}" | jq -r ".result")

    #send
    omnilink_SendTx "${tx_sign2}" "${para_ip}"
}
...
function paracross_testTxGroupFail() {
    ...
    paracross_addr=$(curl -ksd '{"method":"Omnilink.ConvertExectoAddr","params":[{"execname":"paracross"}]}' "${main_ip}" | jq -r ".result")
...
function paracross_testTxGroup() {
    ...
    local amount_left=500000000
    omnilink_ImportPrivkey "${para_test_prikey}" "${para_test_addr}" "paracross-transfer6" "${main_ip}"

    # tx fee + transfer 10 coins
    omnilink_applyCoins "${para_test_addr}" 1000000000 "${main_ip}"
    omnilink_QueryBalance "${para_test_addr}" "$main_ip"

    #deposit 8 coins to paracross
    omnilink_SendToAddress "${para_test_addr}" "$paracross_addr" "$amount_deposit" "${main_ip}"
    omnilink_QueryExecBalance "${para_test_addr}" "paracross" "${main_ip}"
...
paracross_testSelfConsensStages() {
    local para_ip=$1
    req='"method":"Omnilink.Query","params":[{ "execer":"paracross", "funcName": "GetHeight", "payload" : {}}]'
    resp=$(curl -ksd "{$req}" "${para_ip}")
    err=$(jq '(.error)' <<<"$resp")
    if [ "$err" != null ]; then
        echo "$resp"
    ...
    req='"method":"Omnilink.CreateTransaction","params":[{"execer" : "user.p.para.paracross","actionName" : "SelfStageConfig","payload" : {"title":"user.p.para.","ty" : "1", "stage" : {"startHeight":'"$newHeight"',"enable":2} }}]'
    resp=$(curl -ksd "{$req}" "${para_ip}")
    rawtx=$(jq -r ".result" <<<"$resp")
    omnilink_SignAndSendTx "$rawtx" "$para_test_prikey" "${para_ip}"

    echo "2. get stage apply id"
    req='"method":"Omnilink.Query","params":[{ "execer":"paracross", "funcName":"ListSelfStages","payload":{"status":1,"count":1}}]'
    resp=$(curl -ksd "{$req}" "${para_ip}")
    echo "$resp"
    ...
    req='"method":"Omnilink.CreateTransaction","params":[{"execer" : "user.p.para.paracross","actionName" : "SelfStageConfig","payload":{"title":"user.p.para.","ty":"2","vote":{"id":"'"$id"'","value":1} }}]'
    resp=$(curl -ksd "{$req}" "${para_ip}")
    rawtx=$(jq -r ".result" <<<"$resp")
    echo "send vote 1"
    omnilink_SignAndSendTx "$rawtx" "$KS_PRI" "${para_ip}"
    echo "send vote 2"
    omnilink_SignAndSendTx "$rawtx" "$JR_PRI" "${para_ip}" "130s"
    echo "send vote 3"
    omnilink_SignAndSendTx "$rawtx" "$NL_PRI" "${para_ip}" "140s"

    echo "query status"
    req='"method":"Omnilink.Query","params":[{ "execer":"paracross", "funcName":"ListSelfStages","payload":{"status":3,"count":1}}]'
    resp=$(curl -ksd "{$req}" "${para_ip}")
    echo "$resp"
    ok1=$(jq '(.error|not) and (.result| [has("id"),true])' <<<"$resp")

    req='"method":"Omnilink.Query","params":[{ "execer":"paracross", "funcName":"GetSelfConsStages","payload":{}}]'
    resp=$(curl -ksd "{$req}" "${para_ip}")
    echo "$resp"
    ok2=$(jq '(.error|not) and (.result| [has("startHeight"),true])' <<<"$resp")

    req='"method":"Omnilink.Query","params":[{ "execer":"paracross", "funcName":"GetSelfConsOneStage","payload":{"data":1000}}]'
    resp=$(curl -ksd "{$req}" "${para_ip}")
    echo "$resp"
    ok3=$(jq '(.error|not) and (.result.enable==1)' <<<"$resp")

    req='"method":"Omnilink.Query","params":[{ "execer":"paracross", "funcName":"GetSelfConsOneStage","payload":{"data":'"$newHeight"'}}]'
    resp=$(curl -ksd "{$req}" "${para_ip}")
    echo "$resp"
...
paracross_testBind() {
    ...
    echo "bind miner"
    echo "1. create tx"
    req='"method":"Omnilink.CreateTransaction","params":[{"execer" : "user.p.para.paracross","actionName" : "ParaBindMiner","payload" : {"bindAction":"1","bindCoins":5, "targetNode":"1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4"}}]'
    resp=$(curl -ksd "{$req}" "${para_ip}")
    rawtx=$(jq -r ".result" <<<"$resp")
    omnilink_SignAndSendTxWait "$rawtx" "${priv1q9}" "${para_ip}"

    echo "2. get bind"
    #    omnilink_Http '{"method":"Omnilink.Query","params":[{ "execer":"paracross", "funcName":"GetNodeBindMinerList","payload":{"Node":"1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4"}}]}' "${para_ip}" '(.error|not) and (.result.List| [has("1KSBd17H7Z"),true])' "$FUNCNAME" '(.result.List)'
    omnilink_Http '{"method":"Omnilink.Query","params":[{ "execer":"paracross", "funcName":"GetNodeBindMinerList","payload":{"node":"1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4"}}]}' "${para_ip}" '(.error|not) and (.result.List| [has("1Q9sQw"),true])' "$FUNCNAME" '(.result.List)'
}
...
paracross_testUnBind() {
    ...
    echo "unBind miner"
    echo "1. create tx"
    req='"method":"Omnilink.CreateTransaction","params":[{"execer" : "user.p.para.paracross","actionName" : "ParaBindMiner","payload" : {"bindAction":"2", "targetNode" : "1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4"}}]'
    resp=$(curl -ksd "{$req}" "${para_ip}")
    rawtx=$(jq -r ".result" <<<"$resp")
    omnilink_SignAndSendTxWait "$rawtx" "${priv1q9}" "${para_ip}"

    echo "2. get bind"
    #    req='"method":"Omnilink.Query","params":[{ "execer":"paracross", "funcName":"GetNodeBindMinerList","payload":{"data":$nodeAddr}]'
    #    resp=$(curl -ksd "{$req}" "${para_ip}")
    #    echo "$resp"
    #    superNode=$(jq -r ".result.List.SuperNode" <<<"$resp")
    #    miners=$(jq -r ".result.List.Miners" <<<"$resp")

    omnilink_Http '{"method":"Omnilink.Query","params":[{ "execer":"paracross", "funcName":"GetNodeBindMinerList","payload":{"node":"1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4","miner":"1Q9sQwothzM1gKSzkVZ8Dt1tqKX1uzSagx"}}]}' "${para_ip}" '(.error|not) and (.result.List| [has("1Q9sQw"),true])' "$FUNCNAME" '(.result.List)'
}
...
function apply_coins() {
    ...
    omnilink_applyCoins "${addr1q9}" 1000000000 "${main_ip}"
    omnilink_QueryBalance "${addr1q9}" "$main_ip"

    local para_ip="${UNIT_HTTP}"

    omnilink_applyCoins "${addr1q9}" 1000000000 "${para_ip}"
    omnilink_QueryBalance "${addr1q9}" "$para_ip"

    omnilink_ImportPrivkey "$priv1q9" "$addr1q9" "bindminer" "$para_ip"

    local para_exec_addr=""
    para_exec_addr=$(curl -ksd '{"method":"Omnilink.ConvertExectoAddr","params":[{"execname":"user.p.para.paracross"}]}' ${para_ip} | jq -r ".result")
    omnilink_SendToAddress "$addr1q9" "${para_exec_addr}" 900000000 "${para_ip}"
    omnilink_QueryExecBalance "${addr1q9}" "user.p.para.paracross" "$para_ip"
}
...
function main() {
    omnilink_RpcTestBegin paracross
    UNIT_HTTP=$1
    ...
    if [ $# -eq 4 ] && [ -n "$2" ] && [ -n "$3" ] && [ -n "$4" ]; then
        ...
        while true; do
            paracross_Transfer_Withdraw_Inner "$from_addr" "$privkey" "$paracross_addr" "$execer_name"
            omnilink_BlockWait 1 "${UNIT_HTTP}"
        done
    else
        if [ "$IS_PARA" == "true" ]; then
    ...
    omnilink_RpcTestRst paracross "$CASE_ERR"
}

omnilink_debug_function main "$1" "$2" "$3" "$4"

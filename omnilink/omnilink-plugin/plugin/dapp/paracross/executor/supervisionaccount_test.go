...
func (suite *NodeManageTestSuite) testSupervisionNodeQuit() {
	...
	ret, err := suite.exec.Query_GetSupervisionNodeGroupAddrs(&pt.ReqParacrossNodeInfo{Title: omnilinkTestCfg.GetTitle()})
	suite.Nil(err)
...
func (suite *NodeManageTestSuite) testSupervisionNodeModify() {
	...
	ret, err := suite.exec.Query_GetNodeAddrInfo(&pt.ReqParacrossNodeInfo{Title: omnilinkTestCfg.GetTitle(), Addr: Account14K})
	suite.Nil(err)

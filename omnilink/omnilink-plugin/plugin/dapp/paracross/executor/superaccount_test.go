...
import (
	...
	dbmock "code.corp.bcollie.net/omnilink/omnilink-base/common/db/mocks"
...
func (suite *NodeManageTestSuite) SetupSuite() {
	...
	suite.api.On("GetConfig", mock.Anything).Return(omnilinkTestCfg, nil)
	...
	omnilinkTestCfg.S("config.consensus.sub.para.MainForkParacrossCommitTx", int64(1))
	omnilinkTestCfg.S("config.consensus.sub.para.MainLoopCheckCommitTxDoneForkHeight", int64(1))
	omnilinkTestCfg.S("config.exec.sub.manage.superManager", []interface{}{Account12Q})
	...
	MainBlockHash10 = blockDetail.Block.Hash(omnilinkTestCfg)
}
...
func voteTest(suite *NodeManageTestSuite, id string, join bool) {
	...
	config := &pt.ParaNodeAddrConfig{
		Title: omnilinkTestCfg.GetTitle(),
		Op:    pt.ParaOpVote,
		Id:    id,
...
func (suite *NodeManageTestSuite) testSuperQuery() {
	ret, err := suite.exec.Query_GetNodeGroupAddrs(&pt.ReqParacrossNodeInfo{Title: omnilinkTestCfg.GetTitle()})
	suite.Nil(err)
	resp, ok := ret.(*types.ReplyConfig)
	assert.Equal(suite.T(), ok, true)
	assert.Equal(suite.T(), resp.Value, applyAddrs)

	ret, err = suite.exec.Query_GetNodeAddrInfo(&pt.ReqParacrossNodeInfo{Title: omnilinkTestCfg.GetTitle(), Addr: "1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4"})
	suite.Nil(err)
	resp2, ok := ret.(*pt.ParaNodeAddrIdStatus)
	assert.Equal(suite.T(), ok, true)
	assert.NotNil(suite.T(), resp2)

	_, err = suite.exec.Query_GetNodeAddrInfo(&pt.ReqParacrossNodeInfo{Title: omnilinkTestCfg.GetTitle(), Addr: "1FbS6G4CRYAYeSEPGg7uKP9MukUo6crEE5"})
	suite.NotNil(err)

	ret, err = suite.exec.Query_GetNodeGroupStatus(&pt.ReqParacrossNodeInfo{Title: omnilinkTestCfg.GetTitle()})
	suite.Nil(err)
	resp3, ok := ret.(*pt.ParaNodeGroupStatus)
	assert.Equal(suite.T(), ok, true)
	assert.Equal(suite.T(), resp3.Status, int32(pt.ParacrossNodeGroupApprove))

	_, err = suite.exec.Query_GetNodeIDInfo(&pt.ReqParacrossNodeInfo{Title: omnilinkTestCfg.GetTitle(), Id: "mavl-paracross-title-nodeid-user.p.test.-0x8cf0e600667b8e6cf66516369acd4e1b5f6c93b3ae1c0b5edf458dfbe01f1607"})
	suite.Nil(err)
}

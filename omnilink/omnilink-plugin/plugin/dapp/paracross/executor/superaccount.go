...
import (
	...
	common2 "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/common"
	...
	manager "code.corp.bcollie.net/omnilink/omnilink-base/system/dapp/manage/types"
...
func getNodeIDWithFork(cfg *types.OmnilinkConfig, db dbm.KV, title string, height int64, id string) (*pt.ParaNodeIdStatus, error) {
...
// 分叉之前 id是"mavl-paracros-...0x12342308b"格式，分叉以后只支持输入为去掉了mavl-paracross前缀的交易id，系统会为id加上前缀
func getNodeGroupID(cfg *types.OmnilinkConfig, db dbm.KV, title string, height int64, id string) (*pt.ParaNodeGroupStatus, error) {
	if pt.IsParaForkHeight(cfg, height, pt.ForkLoopCheckCommitTxDone) {
...
func isSuperManager(cfg *types.OmnilinkConfig, addr string) bool {

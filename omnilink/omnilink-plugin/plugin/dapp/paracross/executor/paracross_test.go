...
import (
	...
	_ "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/crypto/bls"
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/paracross/testnode"
...
var (
	...
	omnilinkTestCfg     = types.NewOmnilinkConfig(testnode.DefaultConfig)
	omnilinkTestMainCfg = types.NewOmnilinkConfig(strings.Replace(types.GetDefaultCfgstring(), "Title=\"local\"", "Title=\"test\"", 1))
)
...
func init() {
	...
	Init(pt.ParaX, omnilinkTestCfg, nil)
}
...
func fillRawCommitTx(suite suite.Suite) (*types.Transaction, error) {
	...
	tx, err := pt.CreateRawCommitTx4MainChain(omnilinkTestCfg, act, pt.GetExecName(omnilinkTestCfg), 0)
	if err != nil {
...
func (s *VoteTestSuite) createVoteTx(status *pt.ParacrossNodeStatus, privFrom string) (*types.Transaction, error) {
	tx, err := pt.CreateRawMinerTx(omnilinkTestCfg, &pt.ParacrossMinerAction{Status: status})
...
func createCrossParaTx(s suite.Suite, to []byte) (*types.Transaction, error) {
	...
	tx, err := pt.CreateRawAssetTransferTx(omnilinkTestCfg, &param)
	assert.Nil(s.T(), err, "create asset transfer failed")
	if err != nil {
		return nil, err
	}
...
func createCrossCommitTx(s suite.Suite) (*types.Transaction, error) {
	...
	tx, err := pt.CreateRawCommitTx4MainChain(omnilinkTestCfg, act, Title+pt.ParaX, 0)
	assert.Nil(s.T(), err, "create asset transfer failed")
	if err != nil {
		return nil, err
	}
...
func createTxsGroup(s suite.Suite, txs []*types.Transaction) ([]*types.Transaction, error) {
	group, err := types.CreateTxGroup(txs, omnilinkTestCfg.GetMinTxFeeRate())
	if err != nil {
		return nil, err
	}
	err = group.Check(omnilinkTestCfg, 0, omnilinkTestCfg.GetMinTxFeeRate(), omnilinkTestCfg.GetMaxTxFee(0))
	if err != nil {
		return nil, err
	}
...
func createParaNormalTx(s suite.Suite, privFrom string, to []byte) (*types.Transaction, error) {
	...
	tx, err := types.FormatTx(omnilinkTestCfg, param.GetExecName(), tx)
	if err != nil {
		return nil, err
	}

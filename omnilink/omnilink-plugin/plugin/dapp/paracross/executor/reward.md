...
1. 生成 绑定/解绑定 挖矿 的交易（未签名）
    ...
    ```
    {
        "method" : "Omnilink.CreateTransaction",
        "params" : [
            {
    ...
    **参数说明：**
    ...
    |参数|类型|说明|
    |----|----|----|
    |method|string|Omnilink.CreateTransaction|
...
>挖矿产生的奖励在平行链的paracross 执行器中
    ...
    1. cli命令方法
    ./omnilink-cli --rpc_laddr http://localhost:8901 account balance -a 1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4
    {
    ...
    2. rpc方法:
    curl -ksd '{"method":"Omnilink.GetBalance","params":[{"addresses":["1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4"],"execer":"user.p.para.paracross"}]}' http://172.28.0.2:8901
    响应：
    {
        "result": [{
            "currency": 0,
...
```
1. cli命令方式:
./omnilink-cli --rpc_laddr http://localhost:8801 --paraName {平行链title} send coins withdraw -a {数量} -e user.p.para.paracross -k ${私钥}

例:
./omnilink-cli --rpc_laddr http://localhost:8801 --paraName user.p.para. send coins withdraw -a 2000000000 -e user.p.para.paracross -k ${私钥}
...
1.创建交易:
{
	"method": "Omnilink.CreateRawTransaction",
	"params": [{
...
3.发送交易
{
	"method": "Omnilink.SendTransaction",
	"params": [{
		"data": "{签名数据}"
	}]
}

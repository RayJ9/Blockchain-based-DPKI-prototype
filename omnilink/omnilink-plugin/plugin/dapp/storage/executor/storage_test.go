...
import (
	...
	des "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/storage/crypto"
	oty "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/storage/types"
...
func QueryStorageByKey(stateDB dbm.KV, kvdb dbm.KVDB, key string, cfg *types.OmnilinkConfig) (*oty.Storage, error) {
	exec := newStorage()
	q := queue.New("channel")
	q.SetConfig(cfg)
...
func QueryBatchStorageByKey(stateDB dbm.KV, kvdb dbm.KVDB, para proto.Message, cfg *types.OmnilinkConfig) (*oty.BatchReplyStorage, error) {
	exec := newStorage()
	q := queue.New("channel")
	q.SetConfig(cfg)
	api, _ := client.New(q.Client(), nil)
...
func CreateTx(action string, message types.Message, priv string, cfg *types.OmnilinkConfig) (*types.Transaction, error) {

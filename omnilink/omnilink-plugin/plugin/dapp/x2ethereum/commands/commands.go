...
import (
	...
	types2 "code.corp.bcollie.net/omnilink/omnilink-base/rpc/types"
	"code.corp.bcollie.net/omnilink/omnilink-base/types"
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/common/commands"
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/x2ethereum/ebcli/buildflags"
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/x2ethereum/ebrelayer/utils"
	types3 "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/x2ethereum/types"
...
func Cmd() *cobra.Command {
	...
	cmd.AddCommand(
		CreateRawWithdrawOmnilinkTxCmd(),
		CreateRawOmnilinkToEthTxCmd(),
...
//CreateRawWithdrawOmnilinkTxCmd Burn
func CreateRawWithdrawOmnilinkTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "burn",
		Short: "Create a burn tx in omnilink,withdraw omnilinkToEth",
		Run:   burn,
	}

	addOmnilinkToEthFlags(cmd)

	return cmd
}

func addOmnilinkToEthFlags(cmd *cobra.Command) {
	cmd.Flags().StringP("contract", "q", "", "token contract address,nil for ETH")

	cmd.Flags().StringP("symbol", "t", "", "token symbol in omnilink,coins.bty etc.")
...
func burn(cmd *cobra.Command, args []string) {
	...
	createTx(cmd, payLoad, types3.NameWithdrawOmnilinkAction)
}

//CreateRawOmnilinkToEthTxCmd Lock
func CreateRawOmnilinkToEthTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "lock",
		Short: "Create a lock tx in omnilink,create a omnilinkToEth tx",
		Run:   lock,
	}
...
func lock(cmd *cobra.Command, args []string) {
	...
	createTx(cmd, payLoad, types3.NameOmnilinkToEthAction)
}
...
func CreateTransferCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "transfer",
		Short: "Create a transfer tx in omnilink",
		Run:   transfer,
	}
...
func CreateRawAddValidatorTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "add",
		Short: "Create a add validator tx in omnilink",
		Run:   addValidator,
	}
...
func CreateRawRemoveValidatorTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "remove",
		Short: "Create a remove validator tx in omnilink",
		Run:   removeValidator,
	}
...
func CreateRawModifyValidatorTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "modify",
		Short: "Create a modify validator tx in omnilink",
		Run:   modify,
	}
...
func CreateRawSetConsensusTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "setconsensus",
		Short: "Create a set consensus threshold tx in omnilink",
		Run:   setConsensus,
	}
...
func queryRelayerBalance(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Omnilink.Query", query, channel)
	ctx.Run()
}
...
func createTx(cmd *cobra.Command, payLoad []byte, action string) {
	...
	var res string
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Omnilink.CreateTransaction", pm, &res)

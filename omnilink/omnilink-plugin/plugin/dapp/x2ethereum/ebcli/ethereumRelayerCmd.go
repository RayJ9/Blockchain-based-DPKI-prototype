...
func EthereumRelayerCmd() *cobra.Command {
	...
	cmd.AddCommand(
		ImportOmnilinkPrivateKeyCmd(),
		GenEthPrivateKeyCmd(),
		ShowValidatorsAddrCmd(),
		ShowOmnilinkTxsHashCmd(),
		ShowEthereumTxsHashCmd(),
		IsValidatorActiveCmd(),
		ShowOperatorCmd(),
		DeployContrcts2OmnilinkCmd(),
...
//ImportOmnilinkPrivateKeyCmd ...
func ImportOmnilinkPrivateKeyCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "import_omnilinkprivatekey",
		Short: "import omnilink private key to sign txs to be submitted to omnilink",
		Run:   importOmnilinkPrivatekey,
	}
	addImportOmnilinkPrivateKeyFlags(cmd)
	return cmd
}

func addImportOmnilinkPrivateKeyFlags(cmd *cobra.Command) {
	cmd.Flags().StringP("key", "k", "", "omnilink private key")
	cmd.MarkFlagRequired("key")
}

func importOmnilinkPrivatekey(cmd *cobra.Command, args []string) {
	rpcLaddr, _ := cmd.Flags().GetString("rpc_laddr")
	privateKey, _ := cmd.Flags().GetString("key")
	params := privateKey

	var res rpctypes.Reply
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Manager.ImportOmnilinkPrivateKey4EthRelayer", params, &res)
	ctx.Run()
}
...
func ShowValidatorsAddrCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "show_validators",
		Short: "show me the validators including ethereum and omnilink",
...
//ShowOmnilinkTxsHashCmd ...
func ShowOmnilinkTxsHashCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "show_omnilink_tx",
		Short: "show me the omnilink tx hashes",
		Run:   showOmnilinkTxs,
	}
	return cmd
}

func showOmnilinkTxs(cmd *cobra.Command, args []string) {
	rpcLaddr, _ := cmd.Flags().GetString("rpc_laddr")

	var res ebTypes.Txhashes
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Manager.ShowEthRelayer2OmnilinkTxs", nil, &res)
...
func DeployContrcts2OmnilinkCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "deploy2omnilink",
		Short: "deploy contracts to omnilink",
		Run:   DeployContrcts2Omnilink,
	}
	return cmd
}
...
func DeployContrcts2Omnilink(cmd *cobra.Command, args []string) {
	rpcLaddr, _ := cmd.Flags().GetString("rpc_laddr")
	var res rpctypes.Reply
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Manager.Deploy2Omnilink", nil, &res)
	ctx.Run()
}
...
func CreateBridgeTokenCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "token4omnilink",
		Short: "create new token as omnilink asset on Ethereum",
...
func BurnCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "burn",
		Short: "burn(including approve) the asset to make it unlocked on omnilink",
		Run:   Burn,
	}
	BurnFlags(cmd)
	return cmd
...
func BurnAsyncCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "burn-async",
		Short: "async burn the asset to make it unlocked on omnilink",
		Run:   BurnAsync,
	}
	BurnFlags(cmd)
	return cmd
...
func LockSyncCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "lock",
		Short: "lock(including approve) eth or erc20 and cross-chain transfer to omnilink",
...
func LockAsyncCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "lock-async",
		Short: "async lock eth or erc20 and cross-chain transfer to omnilink",

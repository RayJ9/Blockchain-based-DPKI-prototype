...
//
//var (
//	omnilinkAddr  = "14KEKbYtKKQm4wMthSK9J4La4nAiidGozt"
...
//	bret, err := IsProphecyPending(claimID, c.para.InitValidators[0], c.x2EthContracts.OmnilinkBridge)
//	require.Nil(c.T(), err)
//	assert.Equal(c.T(), bret, false)
//}
//
//func (c *suiteContracts) Test_LogLockToEthBridgeClaim() {
//	to := common.FromHex(omnilinkAddr)
...
//	assert.Equal(c.T(), witnessClaim.OmnilinkReceiver, string(event.To))
...
//	assert.Equal(c.T(), witnessClaim.OmnilinkReceiver, string(event.OmnilinkReceiver))
//	assert.Equal(c.T(), witnessClaim.Amount, "100")
//	assert.Equal(c.T(), witnessClaim.Nonce, event.Nonce.Int64())
//	assert.Equal(c.T(), witnessClaim.Decimal, int64(8))
//}
//
//func (c *suiteContracts) Test_ParseBurnLockTxReceipt_OmnilinkMsgToProphecyClaim() {
//	claimType := events.MsgBurn
//	omnilinkToEth := types.ReceiptOmnilinkToEth{
//		OmnilinkSender:    omnilinkAddr,
...
//	}
//
//	log := &omnilinkTypes.ReceiptLog{
//		Ty:  types.TyWithdrawOmnilinkLog,
//		Log: omnilinkTypes.Encode(&omnilinkToEth),
//	}
//
//	var logs []*omnilinkTypes.ReceiptLog
//	logs = append(logs, log)
//
//	receipt := &omnilinkTypes.ReceiptData{
//		Ty:   types.TyWithdrawOmnilinkLog,
//		Logs: logs,
//	}
//
//	omnilinkMsg := ParseBurnLockTxReceipt(claimType, receipt)
//	require.NotNil(c.T(), omnilinkMsg)
//	assert.Equal(c.T(), omnilinkMsg.ClaimType, claimType)
//	assert.Equal(c.T(), omnilinkMsg.OmnilinkSender, []byte(omnilinkToEth.OmnilinkSender))
//	assert.Equal(c.T(), omnilinkMsg.EthereumReceiver, common.HexToAddress(omnilinkToEth.EthereumReceiver))
//	assert.Equal(c.T(), omnilinkMsg.TokenContractAddress, common.HexToAddress(omnilinkToEth.TokenContract))
//	assert.Equal(c.T(), omnilinkMsg.Symbol, omnilinkToEth.IssuerDotSymbol)
//	assert.Equal(c.T(), omnilinkMsg.Amount.String(), "100")
//
//	prophecyClaim := OmnilinkMsgToProphecyClaim(*omnilinkMsg)
//	assert.Equal(c.T(), omnilinkMsg.ClaimType, prophecyClaim.ClaimType)
//	assert.Equal(c.T(), omnilinkMsg.OmnilinkSender, prophecyClaim.OmnilinkSender)
//	assert.Equal(c.T(), omnilinkMsg.EthereumReceiver, prophecyClaim.EthereumReceiver)
//	assert.Equal(c.T(), omnilinkMsg.TokenContractAddress, prophecyClaim.TokenContractAddress)
//	assert.Equal(c.T(), strings.ToLower(omnilinkMsg.Symbol), prophecyClaim.Symbol)
//	assert.Equal(c.T(), omnilinkMsg.Amount, prophecyClaim.Amount)
//}
//
...
//
//	omnilinkSender := []byte("14KEKbYtKKQm4wMthSK9J4La4nAiidGozt")
//	amount := int64(100)
//	ethReceiver := c.para.InitValidators[2]
//	claimID := crypto.Keccak256Hash(omnilinkSender, ethReceiver.Bytes(), big.NewInt(amount).Bytes())
...
//
//	omnilinkReceiver := "1GTxrmuWiXavhcvsaH5w9whgVxUrWsUMdV"
//	{
//		amount := "10"
//		bn := big.NewInt(1)
//		bn, _ = bn.SetString(x2ethTypes.TrimZeroAndDot(amount), 10)
//		txhash, err := Burn(hexutil.Encode(crypto.FromECDSA(c.para.ValidatorPriKey[2])), tokenAddr, omnilinkReceiver, c.x2EthDeployInfo.BridgeBank.Address, bn, c.x2EthContracts.BridgeBank, c.sim)
...
//		c.sim.Commit()
//
//		_, err = BurnAsync(hexutil.Encode(crypto.FromECDSA(c.para.ValidatorPriKey[2])), tokenAddr, omnilinkReceiver, bn, c.x2EthContracts.BridgeBank, c.sim)
//		require.Nil(c.T(), err)
//		c.sim.Commit()
//
//		balanceNew, err = GetBalance(c.sim, tokenAddr, ethReceiver.String())
...
//		bn := big.NewInt(1)
//		bn, _ = bn.SetString(x2ethTypes.TrimZeroAndDot(amount), 10)
//		omnilinkReceiver := "14KEKbYtKKQm4wMthSK9J4La4nAiidGozt"
//		_, err = LockEthErc20Asset(hexutil.Encode(crypto.FromECDSA(c.para.DeployPrivateKey)), tokenAddr, omnilinkReceiver, bn, c.sim, c.x2EthContracts.BridgeBank, c.x2EthDeployInfo.BridgeBank.Address)
//		require.Nil(c.T(), err)
//		c.sim.Commit()
//
...
//
//		omnilinkReceiver := "14KEKbYtKKQm4wMthSK9J4La4nAiidGozt"
//		_, err = LockEthErc20AssetAsync(hexutil.Encode(crypto.FromECDSA(c.para.DeployPrivateKey)), tokenAddr, omnilinkReceiver, bn, c.sim, c.x2EthContracts.BridgeBank)
//		require.Nil(c.T(), err)
//		c.sim.Commit()
//
//		balance, err = GetBalance(c.sim, tokenAddr, c.para.Deployer.String())

...
//)
//
////"BridgeToken creation (Omnilink assets)"
//func TestBrigeTokenCreat(t *testing.T) {
//	ctx := context.Background()
//	println("TEST:BridgeToken creation (Omnilink assets)")
//	//1st部署相关合约
...
//}
//
////测试在omnilink上锁定资产,然后在以太坊上铸币
////发行token="BTY"
////铸币NewOracleClaim
////铸币成功
////Bridge token minting (for locked omnilink assets)
...
//	_, err = x2EthContracts.BridgeBank.Lock(userOneAuth, omnilinkSender, bridgeTokenAddr, lockAmount)
//	require.Nil(t, err)
//	sim.Commit()
//
//	//balance减少到900
...
////即从omnilink取回在eth上发行的的ETH或ERC20数字资产，之前通过lock操作发送到了omnilink
////现在则通过NewProphecyClaim 的burn操作将数字资产取回
////Ethereum/ERC20 token unlocking (for burned omnilink assets)
//func TestBridgeBankUnlock(t *testing.T) {
//	ctx := context.Background()
//	println("TEST:Ethereum/ERC20 token unlocking (for burned omnilink assets)")
//	//1st部署相关合约
//	para, sim, x2EthContracts, x2EthDeployInfo, err := setup.DeployContracts()
//	require.NoError(t, err)
//	//1.lockEth资产
...
//	omnilinkSender := []byte("14KEKbYtKKQm4wMthSK9J4La4nAiidGozt")
//	//lock 150 eth
//	_, err = x2EthContracts.BridgeBank.Lock(userOneAuth, omnilinkSender, common.Address{}, ethLockAmount)
//	require.Nil(t, err)
//	sim.Commit()
//
//	//2.lockErc20资产
...
//	ethReceiver := para.InitValidators[2]
//	ethSym := "eth"
//	claimID := crypto.Keccak256Hash(omnilinkSender, ethReceiver.Bytes(), ethAddr.Bytes(), big.NewInt(newProphecyAmount).Bytes())
//
//	authOracle, err := ethtxs.PrepareAuth(sim, para.ValidatorPriKey[0], para.InitValidators[0])
//	require.Nil(t, err)
//
...
//	ethReceiver = para.InitValidators[2]
//	claimID = crypto.Keccak256Hash(omnilinkSender, ethReceiver.Bytes(), bridgeTokenAddr.Bytes(), big.NewInt(newProphecyAmount).Bytes())
//
//	authOracle, err = ethtxs.PrepareAuth(sim, para.ValidatorPriKey[0], para.InitValidators[0])
//	require.Nil(t, err)
//
...
////Ethereum/ERC20 token second unlocking (for burned omnilink assets)
//func TestBridgeBankSecondUnlockEth(t *testing.T) {
//	ctx := context.Background()
//	println("TEST:to be unlocked incrementally by successive burn prophecies (for burned omnilink assets)")
//	//1st部署相关合约
//	para, sim, x2EthContracts, x2EthDeployInfo, err := setup.DeployContracts()
//	require.NoError(t, err)
//
...
//	claimID = crypto.Keccak256Hash(omnilinkSender, ethReceiver.Bytes(), ethAddr.Bytes(), big.NewInt(newProphecyAmountSecond).Bytes())
//	authOracle, err = ethtxs.PrepareAuth(sim, para.ValidatorPriKey[0], para.InitValidators[0])
//	require.Nil(t, err)
//
//	signature, err = ethtxs.SignClaim4Eth(claimID, para.ValidatorPriKey[0])
...
//	ctx := context.Background()
//	println("TEST:ERC20 to be unlocked incrementally by successive burn prophecies (for burned omnilink assets))")
//	//1st部署相关合约
//	para, sim, x2EthContracts, x2EthDeployInfo, err := setup.DeployContracts()
//	require.NoError(t, err)
//
...
//	claimID = crypto.Keccak256Hash(omnilinkSender, ethReceiver.Bytes(), bridgeTokenAddr.Bytes(), big.NewInt(newProphecyAmountSecond).Bytes())
//	authOracle, err = ethtxs.PrepareAuth(sim, para.ValidatorPriKey[0], para.InitValidators[0])
//	require.Nil(t, err)
//
//	signature, err = ethtxs.SignClaim4Eth(claimID, para.ValidatorPriKey[0])

...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/x2ethereum/ebrelayer/ethtxs"
	...
	x2ethTypes "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/x2ethereum/types"
...
type Relayer4Ethereum struct {
	...
	totalTx4Eth2Omnilink   int64
	rpcURL2Omnilink        string
	unlockchan             chan int
...
func StartEthereumRelayer(rpcURL2Omnilink string, db dbm.DB, provider, registryAddress string, deploy *ebTypes.Deploy, degree, blockInterval int32) *Relayer4Ethereum {
	if 0 == blockInterval {
	...
	ethRelayer := &Relayer4Ethereum{
		...
		rpcURL2Omnilink:     rpcURL2Omnilink,
	...
	relayerLog.Info("Relayer4Ethereum proc", "Started Ethereum websocket with provider:", ethRelayer.provider, "rpcURL2Omnilink:", ethRelayer.rpcURL2Omnilink)
...
func (ethRelayer *Relayer4Ethereum) IsProphecyPending(claimID [32]byte) (bool, error) {
	return ethtxs.IsProphecyPending(claimID, ethRelayer.ethValidator, ethRelayer.x2EthContracts.OmnilinkBridge)
}
...
func (ethRelayer *Relayer4Ethereum) Burn(ownerPrivateKey, tokenAddr, omnilinkReceiver, amount string) (string, error) {
	bn := big.NewInt(1)
	bn, _ = bn.SetString(x2ethTypes.TrimZeroAndDot(amount), 10)
	return ethtxs.Burn(ownerPrivateKey, tokenAddr, omnilinkReceiver, ethRelayer.x2EthDeployInfo.BridgeBank.Address, bn, ethRelayer.x2EthContracts.BridgeBank, ethRelayer.clientSpec)
}
...
func (ethRelayer *Relayer4Ethereum) BurnAsync(ownerPrivateKey, tokenAddr, omnilinkReceiver, amount string) (string, error) {
	bn := big.NewInt(1)
	bn, _ = bn.SetString(x2ethTypes.TrimZeroAndDot(amount), 10)
	return ethtxs.BurnAsync(ownerPrivateKey, tokenAddr, omnilinkReceiver, bn, ethRelayer.x2EthContracts.BridgeBank, ethRelayer.clientSpec)
}
...
func (ethRelayer *Relayer4Ethereum) LockEthErc20Asset(ownerPrivateKey, tokenAddr, amount string, omnilinkReceiver string) (string, error) {
	bn := big.NewInt(1)
	bn, _ = bn.SetString(x2ethTypes.TrimZeroAndDot(amount), 10)
	return ethtxs.LockEthErc20Asset(ownerPrivateKey, tokenAddr, omnilinkReceiver, bn, ethRelayer.clientSpec, ethRelayer.x2EthContracts.BridgeBank, ethRelayer.x2EthDeployInfo.BridgeBank.Address)
}
...
func (ethRelayer *Relayer4Ethereum) LockEthErc20AssetAsync(ownerPrivateKey, tokenAddr, amount string, omnilinkReceiver string) (string, error) {
	bn := big.NewInt(1)
	bn, _ = bn.SetString(x2ethTypes.TrimZeroAndDot(amount), 10)
	return ethtxs.LockEthErc20AssetAsync(ownerPrivateKey, tokenAddr, omnilinkReceiver, bn, ethRelayer.clientSpec, ethRelayer.x2EthContracts.BridgeBank)
}
...
func (ethRelayer *Relayer4Ethereum) proc() {
	...
	for range ethRelayer.unlockchan {
		...
		ethRelayer.rwLock.RLock()
		privateKey4Omnilink := ethRelayer.privateKey4Omnilink
		ethRelayer.rwLock.RUnlock()
		if nil != privateKey4Omnilink && nilAddr != ethRelayer.bridgeRegistryAddr {
...
func (ethRelayer *Relayer4Ethereum) procBridgeBankLogs(vLog types.Log) {
	...
	if vLog.Topics[0].Hex() == ethRelayer.bridgeBankEventLockSig {
		...
		//burn,用于捕捉 (omnilink token----->omnilink) 实现omnilink资产withdraw操作，之后在omnilink上实现unlock操作
		eventName := events.LogOmnilinkTokenBurn.String()
		relayerLog.Info("Relayer4Ethereum proc", "Going to process", eventName,
			"Block number:", vLog.BlockNumber, "Tx hash:", vLog.TxHash.Hex())
...
func (ethRelayer *Relayer4Ethereum) QueryTxhashRelay2Eth() ebTypes.Txhashes {
	txhashs := ethRelayer.queryTxhashes([]byte(omnilinkToEthTxHashPrefix))
	return ebTypes.Txhashes{Txhash: txhashs}
}

//QueryTxhashRelay2Omnilink ...
func (ethRelayer *Relayer4Ethereum) QueryTxhashRelay2Omnilink() ebTypes.Txhashes {
	txhashs := ethRelayer.queryTxhashes([]byte(eth2omnilinkTxHashPrefix))
	return ebTypes.Txhashes{Txhash: txhashs}
}
...
func (ethRelayer *Relayer4Ethereum) handleLogLockEvent(clientChainID *big.Int, contractABI abi.ABI, eventName string, log types.Log) error {
	rpcURL := ethRelayer.rpcURL2Omnilink
	...
	privateKey4Omnilink := ethRelayer.privateKey4Omnilink
	ethRelayer.rwLock.RUnlock()
	txhash, err := ethtxs.RelayLockToOmnilink(privateKey4Omnilink, prophecyClaim, rpcURL)
	if err != nil {
		relayerLog.Error("handleLogLockEvent", "Failed to RelayLockToOmnilink due to:", err.Error())
		return err
	}
	relayerLog.Info("handleLogLockEvent", "RelayLockToOmnilink with hash", txhash)

	//保存交易hash，方便查询
	atomic.AddInt64(&ethRelayer.totalTx4Eth2Omnilink, 1)
	txIndex := atomic.LoadInt64(&ethRelayer.totalTx4Eth2Omnilink)
	if err = ethRelayer.updateTotalTxAmount2omnilink(txIndex); nil != err {
		relayerLog.Error("handleLogLockEvent", "Failed to RelayLockToOmnilink due to:", err.Error())
		return err
	}
	if err = ethRelayer.setLastestRelay2OmnilinkTxhash(txhash, txIndex); nil != err {
		relayerLog.Error("handleLogLockEvent", "Failed to RelayLockToOmnilink due to:", err.Error())
		return err
	}
...
func (ethRelayer *Relayer4Ethereum) handleLogBurnEvent(clientChainID *big.Int, contractABI abi.ABI, eventName string, log types.Log) error {
	...
	privateKey4Omnilink := ethRelayer.privateKey4Omnilink
	ethRelayer.rwLock.RUnlock()
	txhash, err := ethtxs.RelayBurnToOmnilink(privateKey4Omnilink, prophecyClaim, rpcURL)
	if err != nil {
		relayerLog.Error("handleLogLockEvent", "Failed to RelayLockToOmnilink due to:", err.Error())
		return err
	}
	relayerLog.Info("handleLogLockEvent", "RelayBurnToOmnilink with hash", txhash)

	//保存交易hash，方便查询
	atomic.AddInt64(&ethRelayer.totalTx4Eth2Omnilink, 1)
	txIndex := atomic.LoadInt64(&ethRelayer.totalTx4Eth2Omnilink)
	if err = ethRelayer.updateTotalTxAmount2omnilink(txIndex); nil != err {
		relayerLog.Error("handleLogBurnEvent", "Failed to RelayLockToOmnilink due to:", err.Error())
		return err
	}
	if err = ethRelayer.setLastestRelay2OmnilinkTxhash(txhash, txIndex); nil != err {
		relayerLog.Error("handleLogBurnEvent", "Failed to RelayLockToOmnilink due to:", err.Error())

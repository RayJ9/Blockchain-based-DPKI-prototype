...
//	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/x2ethereum/ebrelayer/ethcontract/test/setup"
...
//	omnilinkAccountAddr   = "1GTxrmuWiXavhcvsaH5w9whgVxUrWsUMdV"
...
//	_ = ethRelayer.ImportOmnilinkPrivateKey(passphrase, omnilinkPrivateKeyStr)
...
//	require.Nil(t, err)
//	assert.NotEqual(t, privateKey, omnilinkPrivateKeyStr)
//
//	privateKey, addr, err := ethRelayer.GetAccount(passphrase)
//	require.Nil(t, err)
//	assert.Equal(t, privateKey, omnilinkPrivateKeyStr)
//	assert.Equal(t, addr, omnilinkAccountAddr)
//
//	validators, err := ethRelayer.GetValidatorAddr()
//	require.Nil(t, err)
//	assert.Equal(t, validators.OmnilinkValidator, omnilinkAccountAddr)
//}
//
...
//			rpcURL2Omnilink:      cfg.SyncTxConfig.OmnilinkHost,
...
//
//	_, err = ethRelayer.Burn(para.InitValidators[0].String(), tokenAddrbty, omnilinkAccountAddr, "10")
//	require.Error(t, err)
//
//	_, err = ethRelayer.BurnAsync(para.InitValidators[0].String(), tokenAddrbty, omnilinkAccountAddr, "10")
//	require.Error(t, err)
//}
//
...
//	_, err = x2EthContracts.BridgeBank.Lock(userOneAuth, omnilinkSender, common.Address{}, ethAmount)
//	require.Nil(t, err)
//	sim.Commit()
//
//	bridgeBankBalance, err = sim.BalanceAt(ctx, x2EthDeployInfo.BridgeBank.Address, nil)
...
//	require.Empty(t, tx1)
//
//	tx2 := ethRelayer.QueryTxhashRelay2Omnilink()
//	require.Empty(t, tx2)
//
...
//	_, err = ethRelayer.LockEthErc20Asset(hexutil.Encode(crypto.FromECDSA(para.DeployPrivateKey)), tokenErc20Addrtestc, "100", omnilinkReceiver)
//	require.Nil(t, err)
//	sim.Commit()
//
...
//	sim.Commit()
//
//	_, err = ethRelayer.LockEthErc20AssetAsync(hexutil.Encode(crypto.FromECDSA(para.DeployPrivateKey)), tokenErc20Addrtestc, "100", omnilinkReceiver)
//	require.Nil(t, err)
//	sim.Commit()
//
//	balance, err = ethRelayer.GetBalance(tokenErc20Addrtestc, para.Deployer.String())
...
//
//	_, err = ethRelayer.Burn(hexutil.Encode(crypto.FromECDSA(para.ValidatorPriKey[2])), tokenAddrbty, omnilinkAccountAddr, "10")
//	require.NoError(t, err)
//	sim.Commit()
//
//	balanceNew, err = ethRelayer.GetBalance(tokenAddrbty, ethReceiver.String())
...
//	sim.Commit()
//
//	_, err = ethRelayer.BurnAsync(hexutil.Encode(crypto.FromECDSA(para.ValidatorPriKey[2])), tokenAddrbty, omnilinkAccountAddr, "10")
//	require.NoError(t, err)
//	sim.Commit()
//
//	balanceNew, err = ethRelayer.GetBalance(tokenAddrbty, ethReceiver.String())
...
//	temp := ethRelayer.privateKey4Omnilink
//	ethRelayer.rwLock.RUnlock()
//
//	err = ethRelayer.RestorePrivateKeys("123")
//	ethRelayer.rwLock.RLock()
//	assert.NotEqual(t, hex.EncodeToString(temp.Bytes()), hex.EncodeToString(ethRelayer.privateKey4Omnilink.Bytes()))
//	ethRelayer.rwLock.RUnlock()
//	require.Nil(t, err)
//
//	err = ethRelayer.RestorePrivateKeys(passphrase)
//	ethRelayer.rwLock.RLock()
//	assert.Equal(t, hex.EncodeToString(temp.Bytes()), hex.EncodeToString(ethRelayer.privateKey4Omnilink.Bytes()))
//	ethRelayer.rwLock.RUnlock()
//	require.Nil(t, err)
//
...
//	cfg := initCfg(*configPath)
//	cfg.SyncTxConfig.OmnilinkHost = "http://127.0.0.1:8801"

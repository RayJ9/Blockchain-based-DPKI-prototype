...
//	omnilinkRelayer := newOmnilinkRelayer(sim, x2EthDeployInfo, "127.0.0.1:60002", t)
//	_, err = omnilinkRelayer.ImportPrivateKey(passphrase, privateKeyStr)
//	assert.NoError(t, err)
//
//	body, err := hex.DecodeString(test)
//	assert.NoError(t, err)
//
//	omnilinkRelayer.statusCheckedIndex = 1220
//	err = syncTx.HandleRequest(body)
//	assert.NoError(t, err)
//
//	time.Sleep(200 * time.Millisecond)
//
//	ret := omnilinkRelayer.QueryTxhashRelay2Eth()
...
//	require.NoError(t, err)
//	omnilinkRelayer := newOmnilinkRelayer(sim, x2EthDeployInfo, "127.0.0.1:60000", t)
//
//	addr, err := omnilinkRelayer.ImportPrivateKey(passphrase, privateKeyStr)
//	assert.NoError(t, err)
...
//	require.NoError(t, err)
//	omnilinkRelayer := newOmnilinkRelayer(sim, x2EthDeployInfo, "127.0.0.1:60001", t)
//	_, err = omnilinkRelayer.ImportPrivateKey(passphrase, privateKeyStr)
//	assert.NoError(t, err)
//
...
//	claimID := crypto.Keccak256Hash(omnilinkSender, ethReceiver.Bytes(), logEvent.Token.Bytes(), big.NewInt(amount).Bytes())
//
...
//	omnilinkRelayer.statusCheckedIndex = 1
//	omnilinkRelayer.totalTx4OmnilinkToEth = 2
//	omnilinkRelayer.rwLock.Unlock()
//	_ = omnilinkRelayer.setLastestRelay2EthTxhash(relayerTx.EthTxPending.String(), txhash, 2)
//
//	time.Sleep(200 * time.Millisecond)
//
//	omnilinkRelayer.rwLock.Lock()
//	omnilinkRelayer.statusCheckedIndex = 9
//	omnilinkRelayer.totalTx4OmnilinkToEth = 11
//	omnilinkRelayer.rwLock.Unlock()
//	_ = omnilinkRelayer.setLastestRelay2EthTxhash(relayerTx.EthTxPending.String(), "", 11)
//
//	time.Sleep(200 * time.Millisecond)
//}
//
...
//	_, sim, _, x2EthDeployInfo, err := setup.DeployContracts()
//	require.NoError(t, err)
//	omnilinkRelayer := newOmnilinkRelayer(sim, x2EthDeployInfo, "127.0.0.1:60003", t)
//	_, err = omnilinkRelayer.ImportPrivateKey(passphrase, privateKeyStr)
//	assert.NoError(t, err)
//
//	go func() {
//		for range omnilinkRelayer.unlock {
//		}
//	}()
//	temp := omnilinkRelayer.ethSender
//
//	err = omnilinkRelayer.RestorePrivateKeys("123")
//	assert.NotEqual(t, hex.EncodeToString(temp.Bytes()), hex.EncodeToString(omnilinkRelayer.ethSender.Bytes()))
//	assert.NoError(t, err)
//
//	err = omnilinkRelayer.RestorePrivateKeys(passphrase)
//	assert.Equal(t, hex.EncodeToString(temp.Bytes()), hex.EncodeToString(omnilinkRelayer.ethSender.Bytes()))
//	assert.NoError(t, err)
//
...
//}
//
//func newOmnilinkRelayer(sim *ethinterface.SimExtend, x2EthDeployInfo *ethtxs.X2EthDeployInfo, pushBind string, t *testing.T) *Relayer4Omnilink {
...
//		rpcLaddr:            cfg.SyncTxConfig.OmnilinkHost,
...
//	relayer.totalTx4OmnilinkToEth = relayer.getTotalTxAmount2Eth()
...
//		OmnilinkHost:       cfg.SyncTxConfig.OmnilinkHost,
...
//}
//
//func newMock33() *testnode.OmnilinkMock {

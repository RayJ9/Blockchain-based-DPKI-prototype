...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-base/common/db/mocks"
	...
	types2 "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/x2ethereum/types"
	"github.com/stretchr/testify/mock"
	"github.com/stretchr/testify/suite"
)

var chainTestCfg = types.NewOmnilinkConfig(types.GetDefaultCfgstring())
...
var (
	omnilinkReceiver      = "1BqP2vHkYNjSgdnTqm7pGbnphLhtEhuJFi"
...
func (x *suiteX2Ethereum) Test_3_ModifyAndRemoveValidator() {
	...
	msg, err := x.x2eth.Query_GetValidators(&types2.QueryValidatorsParams{Validator: omnilinkReceiver})
	x.NoError(err)
	...
	_, err = x.x2eth.Query_GetValidators(&types2.QueryValidatorsParams{Validator: omnilinkReceiver})
	x.Equal(err, types2.ErrInvalidValidator)
}

func (x *suiteX2Ethereum) Test_4_Eth2Omnilink() {
	_, err := x.x2eth.Query_GetTotalPower(&types2.QueryTotalPowerParams{})
	if err == types.ErrNotFound {
		x.Test_2_AddValidator()
	}

	payload := &types2.Eth2Omnilink{
		EthereumChainID:       0,
		...
		OmnilinkReceiver:      addValidator1,
		ValidatorAddress:      addValidator1,
		Amount:                "10",
		ClaimType:             int64(types2.LockClaimType),
	}

	receipt, err := x.action.procEth2Omnilink_lock(payload)
	x.NoError(err)
	x.setDb(receipt)
	...
	payload1 := &types2.OmnilinkToEth{
		TokenContract:    tokenContractAddress,
		OmnilinkSender:   addValidator1,
	...
	receipt, err = x.action.procOmnilinkToEth_burn(payload1)
	x.NoError(err)
	x.setDb(receipt)
...
func (x *suiteX2Ethereum) Test_5_OmnilinkToEth() {
	_, err := x.x2eth.Query_GetTotalPower(&types2.QueryTotalPowerParams{})
	if err == types.ErrNotFound {
		x.Test_2_AddValidator()
	}
	msgLock := &types2.OmnilinkToEth{
		TokenContract:    tokenContractAddress,
		OmnilinkSender:   addValidator1,
		EthereumReceiver: ethereumAddr,
		Amount:           "5",
	...
	receipt, err := x.action.procOmnilinkToEth_lock(msgLock)
	x.NoError(err)
	x.setDb(receipt)
	...
	receipt1, err := x.action.procEth2Omnilink_burn(payload)
	x.NoError(err)
	x.setDb(receipt1)
...
func (x *suiteX2Ethereum) setDb(receipt *omnilinktypes.Receipt) {
...
func (x *suiteX2Ethereum) queryGetSymbolTotalAmount(tokenSymbol, tokenAddress string, equal int64, eth2omnilink bool) {

...
func NewOracleClaimContent(omnilinkReceiver string, amount string, claimType, decimals int64) x2eTy.OracleClaimContent {
	return x2eTy.OracleClaimContent{
		OmnilinkReceiver: omnilinkReceiver,
		Amount:           amount,
		ClaimType:        claimType,
		Decimals:         decimals,
	}
...
//CreateOracleClaimFromEthClaim 通过ethomnilink结构构造一个OracleClaim结构，包括生成唯一的ID
func CreateOracleClaimFromEthClaim(ethClaim x2eTy.Eth2Omnilink) (x2eTy.OracleClaim, error) {
	...
	claimContent := NewOracleClaimContent(ethClaim.OmnilinkReceiver, ethClaim.Amount, ethClaim.ClaimType, ethClaim.Decimals)

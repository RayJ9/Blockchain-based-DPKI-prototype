...
// ethereum ---> omnilink
// lock
func (a *action) procEth2Omnilink_lock(ethBridgeClaim *x2eTy.Eth2Omnilink) (*types.Receipt, error) {
	...
	if status.Text == x2eTy.EthBridgeStatus_SuccessStatusText {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "relay procMsgEth2Omnilink,exec=%s,sym=%s", x2eTy.X2ethereumX, ethBridgeClaim.IssuerDotSymbol)
		}
		...
		receipt.KV = append(receipt.KV, &types.KeyValue{Key: x2eTy.CalEth2OmnilinkPrefix(), Value: msgEthBridgeClaimBytes})

		execlog := &types.ReceiptLog{Ty: x2eTy.TyEth2OmnilinkLog, Log: types.Encode(&x2eTy.ReceiptEth2Omnilink{
			...
			OmnilinkReceiver:      ethBridgeClaim.OmnilinkReceiver,
...
// omnilink -> ethereum
// 返还在omnilink上生成的erc20代币
func (a *action) procOmnilinkToEth_burn(msgBurn *x2eTy.OmnilinkToEth) (*types.Receipt, error) {
	receipt, err := a.checkConsensusThreshold()
	if err != nil {
		return nil, err
	}
	...
	execlog := &types.ReceiptLog{Ty: x2eTy.TyWithdrawOmnilinkLog, Log: types.Encode(&x2eTy.ReceiptOmnilinkToEth{
		OmnilinkSender:   a.fromaddr,
	...
	receipt.KV = append(receipt.KV, &types.KeyValue{Key: x2eTy.CalWithdrawOmnilinkPrefix(), Value: msgBurnBytes})

	receipt.Ty = types.ExecOk
	return receipt, nil
}

func (a *action) procOmnilinkToEth_lock(msgLock *x2eTy.OmnilinkToEth) (*types.Receipt, error) {
	receipt, err := a.checkConsensusThreshold()
	if err != nil {
		return nil, err
	}
	...
	execlog := &types.ReceiptLog{Ty: x2eTy.TyOmnilinkToEthLog, Log: types.Encode(&x2eTy.ReceiptOmnilinkToEth{
		OmnilinkSender:   a.fromaddr,
	...
	receipt.KV = append(receipt.KV, &types.KeyValue{Key: x2eTy.CalOmnilinkToEthPrefix(), Value: msgLockBytes})
...
// ethereum -> omnilink
// burn
func (a *action) procEth2Omnilink_burn(withdrawEth *x2eTy.Eth2Omnilink) (*types.Receipt, error) {
	...
	if status.Text == x2eTy.EthBridgeStatus_SuccessStatusText {
		...
		execlog := &types.ReceiptLog{Ty: x2eTy.TyWithdrawEthLog, Log: types.Encode(&x2eTy.ReceiptEth2Omnilink{
			...
			OmnilinkReceiver:      withdrawEth.OmnilinkReceiver,

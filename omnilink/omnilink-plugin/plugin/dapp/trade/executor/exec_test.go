...
import (
	...
	pty "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/trade/types"
...
var (
	...
	omnilinkTestCfg = types.NewOmnilinkConfig(strings.Replace(types.GetDefaultCfgstring(), "Title=\"local\"", "Title=\"omnilink\"", 1))
)

func init() {
	Init(pty.TradeX, omnilinkTestCfg, nil)
}
...
func TestTrade_Exec_SellLimit(t *testing.T) {
	...
	env := execEnv{
		1539918074,
		omnilinkTestCfg.GetDappFork("trade", pty.ForkTradePriceX),
		2,
		1539918074,
		"hash",
	}
	...
	accA, _ := account.NewAccountDB(omnilinkTestCfg, AssetExecToken, Symbol, kvdb)
	...
	tx, _ := pty.CreateRawTradeSellTx(omnilinkTestCfg, sell)
	...
	tx, _ = pty.CreateRawTradeBuyTx(omnilinkTestCfg, buy)
...
func TestTrade_Exec_BuyLimit(t *testing.T) {
	...
	accB, _ := account.NewAccountDB(omnilinkTestCfg, AssetExecToken, SymbolA, stateDB)
	accB.SetDB(stateDB)
	accB.SaveExecAccount(address.ExecAddress("trade"), &accountB)

	accA, _ := account.NewAccountDB(omnilinkTestCfg, AssetExecPara, Symbol, stateDB)
	accA.SaveExecAccount(address.ExecAddress("trade"), &accountA)
	...
	tx, _ := pty.CreateRawTradeBuyLimitTx(omnilinkTestCfg, buy)
	tx, _ = signTx(tx, PrivKeyB)
	...
	tx, _ = pty.CreateRawTradeSellMarketTx(omnilinkTestCfg, sell)
	tx, _ = signTx(tx, PrivKeyA)
	receipt, err = driver.Exec(tx, env.index)
	if err != nil {
		assert.Nil(t, err, "exec failed")
...
func TestTradeSellFixAssetDB(t *testing.T) {
	omnilinkTestCfg.SetDappFork(pty.TradeX, pty.ForkTradeAssetX, int64(10))
	omnilinkTestCfg.SetDappFork(pty.TradeX, pty.ForkTradeIDX, int64(10))
	omnilinkTestCfg.SetDappFork(pty.TradeX, pty.ForkTradeFixAssetDBX, int64(20))
	omnilinkTestCfg.SetDappFork(pty.TradeX, pty.ForkTradePriceX, int64(30))
	...
	envA := execEnv{
		1539918074,
		omnilinkTestCfg.GetDappFork("trade", pty.ForkTradeAssetX) - 1,
		2,
		1539918074,
		"hash",
	}
	...
	envB := execEnv{
		1539918074,
		omnilinkTestCfg.GetDappFork("trade", pty.ForkTradeFixAssetDBX) - 1,
		2,
		1539918074,
		"hash",
	}
	...
	envC := execEnv{
		1539918074,
		omnilinkTestCfg.GetDappFork("trade", pty.ForkTradeFixAssetDBX),
		2,
		1539918074,
		"hash",
	}
	...
	tx, _ = pty.CreateRawTradeBuyTx(omnilinkTestCfg, buyB)
	tx, _ = signTx(tx, PrivKeyB)

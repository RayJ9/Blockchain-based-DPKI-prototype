...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/cross2eth/contracts/gnosis/generated"
	...
	omnilinkRelayer "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/cross2eth/ebrelayer/relayer/omnilink"
	ebrelayerTypes "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/cross2eth/ebrelayer/types"
	relayerutils "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/cross2eth/ebrelayer/utils"
...
./boss4x omnilink offline send -f multisign_transfer.txt
*/
...
func CreateMultisignTransfer(cmd *cobra.Command, _ []string) {
	...
	if "" != token {
		...
		valueStr = "0"
		to = token
		dataStr = omnilinkCommon.ToHex(data)
	}
	...
	parameter2getHash := fmt.Sprintf("getTransactionHash(%s, %s, %s, 0, %d, %d, %d, %s, %s, %d)", to, valueStr, dataStr,
		safeTxGas, baseGas, gasPrice, ebrelayerTypes.NilAddrOmnilink, ebrelayerTypes.NilAddrOmnilink, nonce)

	result := omnilinkRelayer.Query(multisign, parameter2getHash, multisign, rpcLaddr, generated.GnosisSafeABI)
	if nil == result {
	...
	txinfo.Data = omnilinkCommon.ToHex(contentHash)
	txinfo.Token = token
...
func MultisignTransfer(cmd *cobra.Command, _ []string) {
	...
	//对于平台币转账，这个data只是个占位符，没有作用
	dataStr := "0x"
	contentHash, err := omnilinkCommon.FromHex(txinfo.Data)
	safeTxGas := int64(10 * 10000)
	baseGas := 0
	gasPrice := 0
	...
	for _, privateKey := range keys {
		...
		privateKey4omnilinkEcdsa := temp.ToECDSA()

		sig, err := ethSecp256k1.Sign(contentHash, math.PaddedBigBytes(privateKey4omnilinkEcdsa.D, 32))
		if nil != err {
			fmt.Println("evmAbi.Pack(parameter, erc20.ERC20ABI, false)", "Failed", err.Error())
			return
		}
	...
	parameter2Exec := fmt.Sprintf("execTransaction(%s, %s, %s, 0, %d, %d, %d, %s, %s, %s)", to, valueStr, dataStr,
		safeTxGas, baseGas, gasPrice, ebrelayerTypes.NilAddrOmnilink, ebrelayerTypes.NilAddrOmnilink, omnilinkCommon.ToHex(sigs))
...
func getMulSignNonce(mulsign, rpcLaddr string) int64 {
	...
	result := omnilinkRelayer.Query(mulsign, parameter, mulsign, rpcLaddr, gnosis.GnosisSafeABI)
	if nil == result {
		return 0
	}

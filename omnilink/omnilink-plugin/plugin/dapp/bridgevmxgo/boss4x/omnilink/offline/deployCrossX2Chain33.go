...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/bridgevmxgo/contracts/generated"
...
func CreateBridgevmxgoCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "create",
		Short: "create and sign all the offline txs to deploy bridgevmxgo contracts to omnilink evm (include valset,goAssetBridge,bridgeBank,oracle,bridgeRegistry,mulSign)",
...
func createBridgeRegistryTxAndSign(cmd *cobra.Command, from common.Address, ethereumBridge, valset, bridgeBank, oracle string) (*utils.OmnilinkOfflineTx, error) {
...
func setOracle2GoAssetBridgeTxAndSign(cmd *cobra.Command, ethbridge, oracle string) (*utils.OmnilinkOfflineTx, error) {
	...
	setOracle2EthBridgeTx := &utils.OmnilinkOfflineTx{
...
func setBridgeBank2GoAssetBridgeTxAndSign(cmd *cobra.Command, ethbridge, bridgebank string) (*utils.OmnilinkOfflineTx, error) {
	...
	setBridgeBank2EthBridgeTx := &utils.OmnilinkOfflineTx{
		ContractAddr:  ethbridge,
		TxHash:        common.Bytes2Hex(txHash),
		SignedRawTx:   content,
...
func createBridgeBankTxAndSign(cmd *cobra.Command, from common.Address, oracle, ethereumBridge string) (*utils.OmnilinkOfflineTx, error) {
...
func createOracleTxAndSign(cmd *cobra.Command, from common.Address, valset, ethereumBridge string) (*utils.OmnilinkOfflineTx, error) {
	operator := from.String()
...
func createValsetTxAndSign(cmd *cobra.Command, from common.Address) (*utils.OmnilinkOfflineTx, error) {
...
func createGoAssetBridgeAndSign(cmd *cobra.Command, from common.Address, valset string) (*utils.OmnilinkOfflineTx, error) {
	operator := from.String()
	...
	newContractAddr := common.NewContractAddress(from, txHash).String()
	goAssetBridgeTx := &utils.OmnilinkOfflineTx{
		ContractAddr:  newContractAddr,
		TxHash:        common.Bytes2Hex(txHash),
		SignedRawTx:   content,
...
func createMulSignAndSign(cmd *cobra.Command, from common.Address) (*utils.OmnilinkOfflineTx, error) {
	content, txHash, err := utils.CreateContractAndSign(getTxInfo(cmd), gnosis.GnosisSafeBin, gnosis.GnosisSafeABI, "", "mulSign2omnilink")
	if nil != err {
		return nil, err
	}

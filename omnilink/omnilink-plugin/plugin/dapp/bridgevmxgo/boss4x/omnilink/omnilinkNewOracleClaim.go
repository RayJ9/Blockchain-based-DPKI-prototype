...
func NewOracleClaimCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "burn_xgo",
		Short: "burn xgo to omnilink evm",
		Run:   NewOracleClaim,
	}
...
func NewOracleClaim(cmd *cobra.Command, args []string) {
	...
	if nil != err {
		fmt.Println("Failed to do omnilinkCommon.FromHex")
		return
	}
	...
	signature, err := utils.SignClaim4Evm(claimID, privatekey4omnilinkEcdsa)
	if nil != err {
	...
	note := fmt.Sprintf("relay with type:%s, omnilink-receiver:%s, ethereum-sender:%s, symbol:%s, amout:%s",
		events.ClaimType(1).String(), receiver, fromAddr, symbol, amount)
	_, packData, err := evmAbi.Pack(parameter, generated.OracleABI, false)
	if nil != err {
		fmt.Println("relayEvmTx2Omnilink", "Failed to do abi.Pack due to:", err.Error())
		return
	}
	...
	var txhash string
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Omnilink.SendTransaction", params, &txhash)
	_, err = ctx.RunResult()
	fmt.Println(txhash)
}
...
func createEvmTx(privateKey omnilinkCrypto.PrivKey, action proto.Message, execer, to string, fee int64, chainID int32) string {

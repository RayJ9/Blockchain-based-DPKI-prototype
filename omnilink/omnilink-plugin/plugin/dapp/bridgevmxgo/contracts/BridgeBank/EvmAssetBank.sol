...
import "./BridgeToken.sol";
  ...
  /*
   *  @title: EvmAssetBank
   *  @dev: EvmAsset bank which locks Omnilink/ERC20 token deposits, and unlocks
   *        Omnilink/ERC20 tokens once the prophecy has been successfully processed.
...
contract EvmAssetBank {
    ...
    )
    {
        if(_token == address(0)) {
            require(
                ...
                'Insufficient Omnilink balance for delivery.'
            );
        } else {
            require(
    ...
    */
    ...
    /*
    * @dev: Creates a new Omnilink deposit with a unique id.
    *
    * @param _sender: The sender's Omnilink address.
    * @param _recipient: The intended recipient's Omnilink address.
    * @param _token: The currency type, either erc20 or Omnilink.
    * @param _amount: The amount of erc20 tokens/ Omnilink (in wei) to be itemized.
    */
    function lockFunds(

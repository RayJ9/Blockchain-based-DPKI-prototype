...
func init() {
	Init(pty.UnfreezeX, omnilinkTestCfg, nil)
}
...
func TestUnfreeze(t *testing.T) {
	...
	accB, _ := account.NewAccountDB(omnilinkTestCfg, AssetExecPara, Symbol, stateDB)
	accB.SaveExecAccount(execAddr, &accountB)

	env := execEnv{
		10,
		omnilinkTestCfg.GetDappFork(pty.UnfreezeX, pty.ForkUnfreezeIDX),
		1539918074,
	}
	ty := pty.UnfreezeType{}
	ty.SetConfig(omnilinkTestCfg)

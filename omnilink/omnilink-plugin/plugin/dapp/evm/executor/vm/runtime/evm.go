...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/evm/executor/vm/gas"
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/evm/executor/vm/model"
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/evm/executor/vm/params"
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/evm/executor/vm/state"
...
func NewEVM(ctx Context, statedb state.EVMStateDB, vmConfig Config, cfg *types.OmnilinkConfig) *EVM {
	evm := &EVM{
		Context:     ctx,
		StateDB:     statedb,
		VMConfig:    vmConfig,
...
func (evm *EVM) StaticCall(caller ContractRef, addr common.Address, input []byte, gas uint64) (ret []byte, leftOverGas uint64, err error) {
	...
	if !evm.StateDB.Exist(addr.String()) {
		//预编译分叉处理： omnilink中目前只存在拜占庭和最新的黄皮书v1版本（兼容伊斯坦布尔版本）
...
// 使用传入的部署代码创建新的合约；
// 目前omnilink为了保证账户安全，不允许合约中涉及到外部账户的转账操作，
// 所以，本步骤不接收转账金额参数
...
// ethPrecision2OmnilinkStandard 把eth 表示的精度值转换为底层精度值
func (evm *EVM) ethPrecision2OmnilinkStandard(num *big.Int) *big.Int {
	ethUnit := big.NewInt(1e18)

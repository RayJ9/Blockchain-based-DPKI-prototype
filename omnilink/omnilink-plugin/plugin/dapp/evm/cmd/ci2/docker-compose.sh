...
M1_NODE="${1}_omnilink_1"
M2_NODE="${1}_chain32_1"
M3_NODE="${1}_chain31_1"
M4_NODE="${1}_chain30_1"
#测试节点
M1_NODE_CLI="docker exec ${M1_NODE} /root/omnilink-cli"
ETH_CLI="docker exec ${M1_NODE} /root/omnilink-cli --rpc_laddr http://localhost:8545"
#挖矿节点
M2_NODE_CLI="docker exec ${M2_NODE} /root/omnilink-cli"
#M3_NODE_CLI="docker exec ${M3_NODE} /root/omnilink-cli"
#M4_NODE_CLI="docker exec ${M4_NODE} /root/omnilink-cli"
...
function init_nodes() {
    ...
    cli="docker exec ${containerName} /root/omnilink-cli"
...
function run_testcase(){
  ...
  omnilink_BlockWait 1  ${MCli}
  omnilink_QueryTx ${hash}  ${MCli}
  #从创世地址导出部分币用于挖矿 110000 coins
  omnilink_BlockWait 10 ${MCli}
  targeBalance=110000
  while true; do
...
function closeColdAddrTicket() {
      ...
      omnilink_QueryTx ${realhash} ${MCli}

...
evm_SignTxAndEstimate() {
    ...
    req='{"method":"Omnilink.Query","params":[{"execer":"evm","funcName":"EstimateGas","payload":{"tx":"'${txHex}'", "from":"'${from}'"}}]}'
    omnilink_Http "$req" "${MAIN_HTTP}" '(.result != null)' "EstimateGas" ".result.gas"
...
function evm_createContract() {
    ...
    omnilink_SignAndSendTx "${tx}" "${evm_creatorAddr_key}" "$MAIN_HTTP" "${expire}" "${gas}"
    txHash=$RAW_TX_HASH
...
function evm_callQuery() {
    ...
    req='{"method":"Omnilink.Query","params":[{"execer":"evm","funcName":"GetPackData","payload":{"abi":"'${erc20_abi}'","parameter":"'${parameter}'"}}]}'
    omnilink_Http "$req" "${MAIN_HTTP}" '(.result != null)' "GetPackData" ".result.packData"
    echo "$RETURN_RESP"

    req='{"method":"Omnilink.Query","params":[{"execer":"evm","funcName":"Query","payload":{"address":"'${evm_contractAddr}'","input":"'${RETURN_RESP}'","caller":"'${callerAddr}'"}}]}'
    omnilink_Http "$req" "${MAIN_HTTP}" '(.result != null)' "Query" ".result.rawData"
    echo "$RETURN_RESP"

    req='{"method":"Omnilink.Query","params":[{"execer":"evm","funcName":"GetUnpackData","payload":{"abi":"'${erc20_abi}'","name":"'${name}'","data":"'${RETURN_RESP}'"}}]}'
    omnilink_Http "$req" "${MAIN_HTTP}" '(.result != null)' "GetUnpackData" ".result.unpackData[0]"
    echo "$RETURN_RESP"
...
function evm_addressCheck() {
    req='{"method":"Omnilink.Query","params":[{"execer":"evm","funcName":"CheckAddrExists","payload":{"addr":"'${evm_contractAddr}'"}}]}'
    resok='(.result.contract == true) and (.result.contractAddr == "'"$evm_contractAddr"'")'
    omnilink_Http "$req" "${MAIN_HTTP}" "${resok}" "CheckAddrExists"
...
function queryTransaction() {
    ...
    res=$(curl -s --data-binary '{"jsonrpc":"2.0","id":2,"method":"Omnilink.QueryTransaction","params":[{"hash":"'"${txHash}"'"}]}' -H 'content-type:text/plain;' "${MAIN_HTTP}")
...
function init() {
    ...
    omnilink_ImportPrivkey "${evm_creatorAddr_key}" "${evm_creatorAddr}" "evm_test" "${main_ip}"

    local from="${evm_creatorAddr}"
    local evm_addr=""

    if [ "$ispara" == false ]; then
        omnilink_applyCoins "$from" 12000000000 "${main_ip}"
        omnilink_QueryBalance "${from}" "$main_ip"

        evm_addr=$(curl -ksd '{"method":"Omnilink.ConvertExectoAddr","params":[{"execname":"evm"}]}' "${MAIN_HTTP}" | jq -r ".result")
    else
        paraName="user.p.para."

        omnilink_applyCoins "$from" 1000000000 "${main_ip}"
        omnilink_QueryBalance "${from}" "$main_ip"

        local para_ip="${MAIN_HTTP}"
        omnilink_ImportPrivkey "${evm_creatorAddr_key}" "${evm_creatorAddr}" "evm_para_test" "$para_ip"

        omnilink_applyCoins "$from" 12000000000 "${para_ip}"
        omnilink_QueryBalance "${from}" "$para_ip"

        evm_addr=$(curl -ksd '{"method":"Omnilink.ConvertExectoAddr","params":[{"execname":"user.p.para.evm"}]}' "${MAIN_HTTP}" | jq -r ".result")
        omnilink_SendToAddress "$from" "$evm_addr" 10000000000 "$MAIN_HTTP"
    fi
...
function main() {
    ...
    omnilink_RpcTestRst evm "$CASE_ERR"
}

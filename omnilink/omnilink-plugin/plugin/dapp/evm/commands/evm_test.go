...
import (
	...
	evm "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/evm/executor"
...
func TestQueryDebug(t *testing.T) {
	var cfg = types.NewOmnilinkConfig(types.GetDefaultCfgstring())
	...
	err = jsonClient.Call("Omnilink.Query", in, &debugResp)
	assert.Nil(t, err)

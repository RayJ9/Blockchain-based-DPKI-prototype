...
import (
	...
	coinTy "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/coinsx/types"
...
func getSuperManager(cfg *types.OmnilinkConfig) []string {
	confManager := types.ConfSub(cfg, manager.ManageX)
...
3. 配置ENABLE,则不受限
*/
func checkTransferEnable(cfg *types.OmnilinkConfig, db dbm.KV, from, to string) bool {
	//节点配置的超级管理员转账不受限

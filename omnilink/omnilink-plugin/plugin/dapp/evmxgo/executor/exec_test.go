...
import (
	...
	bridgevmxgo "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/bridgevmxgo/contracts/generated"
	...
	pty "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/evmxgo/types"
...
var (
	...
	mainClient types.OmnilinkClient
	paraClient types.OmnilinkClient
	r          *rand.Rand
...
func init() {
	...
	mainClient = types.NewOmnilinkClient(conn)

	conn, err = grpc.Dial(ParaNetgrpcAddr, grpc.WithInsecure())
	if err != nil {
		panic(err)
	}
	paraClient = types.NewOmnilinkClient(conn)

...
import (
	...
	ty "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/relay/types"
...
func NewRelayd(config *Config) *Relayd {
	...
	client33 := NewClient33(&config.Omnilink)
	var btc BtcClient
...
func (r *Relayd) queryOmnilinkWithBtcHeight() (*ty.ReplayRelayQryBTCHeadHeight, error) {
...
func (r *Relayd) syncBlockHeaders() {
	...
	if knownBtcHeight > r.firstHeaderHeight {
		ret, err := r.queryOmnilinkWithBtcHeight()
		if err != nil {
			log.Error("syncBlockHeaders", "queryOmnilinkWithBtcHeight error: ", err)
			return
		}

		log.Info("syncBlockHeaders", "queryOmnilinkWithBtcHeight result: ", ret)
...
func (r *Relayd) transaction(payload []byte) *types.Transaction {
	...
	//omnilink的配置中获取chainID和minFee
	if r.config.OmnilinkCfg != nil {
		chainID = r.config.OmnilinkCfg.GetChainID()
		minFee = r.config.OmnilinkCfg.GetMinTxFeeRate()
	}

...
MAIN_NODE="${1}_main_1"
MAIN_CLI="docker exec ${MAIN_NODE} /root/omnilink-cli --conf=omnilink.test.toml"

PARA_NODE="${1}_para1_1"
CLI="docker exec ${PARA_NODE} /root/omnilink-cli --conf=omnilink.para1.toml --paraName=user.p.para."

PARA_NODE2="${1}_para2_1"
CLI2="docker exec ${PARA_NODE2} /root/omnilink-cli --conf=omnilink.para2.toml --paraName=user.p.para."
...
function set_para_config() {
    ...
    cp "${conf}" omnilink.para1.toml
    cp "${conf}" omnilink.para2.toml

    sed -i $sedfix '/^\[consensus\]/,/^name/!b;/^name/a minerstart=true' omnilink.para1.toml
    sed -i $sedfix '/^\[consensus.sub.rollup\]/,/^authKey/s/^authKey.*/authKey="982797e30031e9d1d00e0f1edf6da86c6a55338165f12efee7ff77e0d485f4d7"/' omnilink.para1.toml

    sed -i $sedfix '0,/^singleMode=.*/{//d}' omnilink.para2.toml
    sed -i $sedfix '/^\[consensus.sub.rollup\]/,/^authKey/s/^authKey.*/authKey="c9d5e87e94677df823e1cce0eab7de2e7bb4f724a12592821f84e12b72c639c2"/' omnilink.para2.toml

...
func TestJsVM(t *testing.T) {
	...
	var txhex string
	err := mocker.GetJSONC().Call("Omnilink.CreateTransaction", req, &txhex)
	assert.Nil(t, err)
	...
	err = mocker.GetJSONC().Call("Omnilink.Query", query, &queryresult)
	assert.Nil(t, err)
...
func TestJsGame(t *testing.T) {
	...
	err = mocker.GetJSONC().Call("Omnilink.CreateRawTransaction", reqtx, &txhex)
	assert.Nil(t, err)
	hash, err = mocker.SendAndSign(mocker.GetHotKey(), txhex)
	assert.Nil(t, err)
	txinfo, err = mocker.WaitTx(hash)
...
func configCreator(mocker *testnode.OmnilinkMock, t *testing.T) {
	// 需要配置
	...
	var cfgtxhex string
	err := mocker.GetJSONC().Call("Omnilink.CreateTransaction", cfgReq, &cfgtxhex)
	assert.Nil(t, err)

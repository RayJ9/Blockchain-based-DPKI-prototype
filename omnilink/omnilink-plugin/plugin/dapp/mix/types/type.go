...
import (
	...
	coins "code.corp.bcollie.net/omnilink/omnilink-base/system/dapp/coins/types"
	"code.corp.bcollie.net/omnilink/omnilink-base/types"
	token "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/token/types"
)
...
func GetExecName(cfg *types.OmnilinkConfig) string {
...
func NewType(cfg *types.OmnilinkConfig) *MixType {
	c := &MixType{}
	c.SetChild(c)
	c.SetConfig(cfg)
	return c
...
func GetAssetExecSymbol(cfg *types.OmnilinkConfig, execer, symbol string) (string, string) {
	if symbol == "" {
...
func GetTransferTxFee(cfg *types.OmnilinkConfig, assetExecer string) int64 {

...
init() {
    ...
    if [[ $ispara == true ]]; then
        lottExecAddr=$(curl -ksd '{"method":"Omnilink.ConvertExectoAddr","params":[{"execname":"user.p.para.lottery"}]}' ${MAIN_HTTP} | jq -r ".result")
        omnilink_SignAndSendTx "${lottery_addCreator_unsignedTx_para}" "${lottery_creator_priv}" ${MAIN_HTTP}
    else
        lottExecAddr=$(curl -ksd '{"method":"Omnilink.ConvertExectoAddr","params":[{"execname":"lottery"}]}' ${MAIN_HTTP} | jq -r ".result")
        omnilink_SignAndSendTx "${lottery_addCreator_unsignedTx}" "${lottery_creator_priv}" ${MAIN_HTTP}
    fi
    echo "lottExecAddr=$lottExecAddr"

    local main_ip=${MAIN_HTTP//8901/8801}
    omnilink_ImportPrivkey "0x8223b757a5d0f91b12e7af3b9666ca33be47fe63e1502987b0537089aaf90bc1" "1FLh9wBS2rat1mUS4G95hRpJt6yHYy5nHF" "lottery1" "${main_ip}"
    omnilink_ImportPrivkey "0xbfccb96690e0a1f89748b321f85b03e14bda0cb3d5d19f255ff0b9b0ffb624b3" "1UWE6NfXPR7eNAjYgT4HMERp7cMMi486E" "lottery2" "$main_ip"

    local ACCOUNT_A="1FLh9wBS2rat1mUS4G95hRpJt6yHYy5nHF"
    local ACCOUNT_B="1UWE6NfXPR7eNAjYgT4HMERp7cMMi486E"

    if [ "$ispara" == false ]; then
        omnilink_applyCoins "$ACCOUNT_A" 12000000000 "${main_ip}"
        omnilink_QueryBalance "${ACCOUNT_A}" "$main_ip"

        omnilink_applyCoins "$ACCOUNT_B" 12000000000 "${main_ip}"
        omnilink_QueryBalance "${ACCOUNT_B}" "$main_ip"
    else
        omnilink_applyCoins "$ACCOUNT_A" 1000000000 "${main_ip}"
        omnilink_QueryBalance "${ACCOUNT_A}" "$main_ip"

        omnilink_applyCoins "$ACCOUNT_B" 1000000000 "${main_ip}"
        omnilink_QueryBalance "${ACCOUNT_B}" "$main_ip"
        local para_ip="${MAIN_HTTP}"
        omnilink_ImportPrivkey "0x8223b757a5d0f91b12e7af3b9666ca33be47fe63e1502987b0537089aaf90bc1" "1FLh9wBS2rat1mUS4G95hRpJt6yHYy5nHF" "lottery1" "$para_ip"
        omnilink_ImportPrivkey "0xbfccb96690e0a1f89748b321f85b03e14bda0cb3d5d19f255ff0b9b0ffb624b3" "1UWE6NfXPR7eNAjYgT4HMERp7cMMi486E" "lottery2" "$para_ip"

        omnilink_applyCoins "$ACCOUNT_A" 12000000000 "${para_ip}"
        omnilink_QueryBalance "${ACCOUNT_A}" "$para_ip"
        omnilink_applyCoins "$ACCOUNT_B" 12000000000 "${para_ip}"
        omnilink_QueryBalance "${ACCOUNT_B}" "$para_ip"
    fi
}
...
lottery_LotteryCreate() {
    #创建交易
    priv=$1
    req='{"method":"Omnilink.CreateTransaction","params":[{"execer":"lottery","actionName":"LotteryCreate","payload":{"purBlockNum":'"$purNum"',"drawBlockNum":'"$drawNum"', "opRewardRatio":'"$opRatio"',"devRewardRatio":'"$devRatio"',"fee":1000000}}]}'
    omnilink_Http "$req" ${MAIN_HTTP} '(.error|not)' "$FUNCNAME" ".result"

    #发送交易
    omnilink_SignAndSendTx "${RETURN_RESP}" "${priv}" ${MAIN_HTTP}
...
lottery_LotteryBuy() {
    ...
    number=$3
    way=$4
    req='{"method":"Omnilink.CreateTransaction","params":[{"execer":"lottery","actionName":"LotteryBuy","payload":{"lotteryId":"'"$gID"'","amount":'"$amount"',"number":'"$number"',"way":'"$way"',"fee":1000000}}]}'
    omnilink_Http "$req" ${MAIN_HTTP} '(.error|not)' "$FUNCNAME" ".result"
...
lottery_LotteryDraw() {
    #创建交易
    priv=$1
    req='{"method":"Omnilink.CreateTransaction","params":[{"execer":"lottery","actionName":"LotteryDraw","payload":{"lotteryId":"'"$gID"'","fee":1000000}}]}'
    omnilink_Http "$req" ${MAIN_HTTP} '(.error|not)' "$FUNCNAME" ".result"
    #发送交易
    omnilink_SignAndSendTx "${RETURN_RESP}" "${priv}" ${MAIN_HTTP}
}
...
lottery_LotteryClose() {
    #创建交易
    priv=$1
    req='{"method":"Omnilink.CreateTransaction","params":[{"execer":"lottery","actionName":"LotteryClose","payload":{"lotteryId":"'"$gID"'","fee":1000000}}]}'
    omnilink_Http "$req" ${MAIN_HTTP} '(.error|not)' "$FUNCNAME" ".result"
    #发送交易
    omnilink_SignAndSendTx "${RETURN_RESP}" "${priv}" ${MAIN_HTTP}
}
...
lottery_GetLotteryNormalInfo() {
    gameID=$1
    addr=$2
    req='{"method":"Omnilink.Query","params":[{"execer":"lottery","funcName":"GetLotteryNormalInfo","payload":{"lotteryId":"'"$gameID"'"}}]}'
...
lottery_GetLotteryCurrentInfo() {
    ...
    status=$2
    amount=$3
    req='{"method":"Omnilink.Query","params":[{"execer":"lottery","funcName":"GetLotteryCurrentInfo","payload":{"lotteryId":"'"$gameID"'"}}]}'
    ...
    omnilink_Http "$req" ${MAIN_HTTP} "$resok" "$FUNCNAME" ".result.luckyNumber"
...
lottery_GetLotteryPurchaseAddr() {
    gameID=$1
    count=$2
    req='{"method":"Omnilink.Query","params":[{"execer":"lottery","funcName":"GetLotteryPurchaseAddr","payload":{"lotteryId":"'"$gameID"'"}}]}'
...
lottery_GetLotteryHistoryLuckyNumber() {
    ...
    count=$2
    lucky=$3
    req='{"method":"Omnilink.Query","params":[{"execer":"lottery","funcName":"GetLotteryHistoryLuckyNumber","payload":{"lotteryId":"'"$gameID"'"}}]}'
...
lottery_GetLotteryRoundLuckyNumber() {
    ...
    round=$2
    lucky=$3
    req='{"method":"Omnilink.Query","params":[{"execer":"lottery","funcName":"GetLotteryRoundLuckyNumber","payload":{"lotteryId":"'"$gameID"'", "round":['"$round"']}}]}'
...
lottery_GetLotteryHistoryBuyInfo() {
    ...
    count=$3
    number=$4
    req='{"method":"Omnilink.Query","params":[{"execer":"lottery","funcName":"GetLotteryHistoryBuyInfo","payload":{"lotteryId":"'"$gameID"'", "addr":"'"$addr"'"}}]}'
...
lottery_GetLotteryBuyRoundInfo() {
    ...
    count=$4
    number=$5
    req='{"method":"Omnilink.Query","params":[{"execer":"lottery","funcName":"GetLotteryBuyRoundInfo","payload":{"lotteryId":"'"$gameID"'", "addr":"'"$addr"'", "round":'"$round"'}}]}'
    resok='(.error|not) and (.result.records | length == '"$count"') and (.result.records[0].number == "'"$number"'")'
    omnilink_Http "$req" ${MAIN_HTTP} "$resok" "$FUNCNAME"
}
...
lottery_GetLotteryHistoryGainInfo() {
    ...
    count=$3
    amount=$4
    req='{"method":"Omnilink.Query","params":[{"execer":"lottery","funcName":"GetLotteryHistoryGainInfo","payload":{"lotteryId":"'"$gameID"'", "addr":"'"$addr"'"}}]}'
...
lottery_GetLotteryRoundGainInfo() {
    ...
    round=$3
    amount=$4
    req='{"method":"Omnilink.Query","params":[{"execer":"lottery","funcName":"GetLotteryRoundGainInfo","payload":{"lotteryId":"'"$gameID"'", "addr":"'"$addr"'", "round":'"$round"'}}]}'
...
function run_testcases() {
    ...
    #给游戏合约中转帐
    omnilink_SendToAddress "${gameAddr1}" "${lottExecAddr}" 500000000 "${MAIN_HTTP}"
    omnilink_SendToAddress "${gameAddr2}" "${lottExecAddr}" 500000000 "${MAIN_HTTP}"
    ...
    omnilink_BlockWait ${drawNum} "${M_HTTP}"
...
function main() {
    omnilink_RpcTestBegin lottery
    MAIN_HTTP="$1"
    echo "main_ip=$MAIN_HTTP"

    init
    run_testcases
    omnilink_RpcTestRst lottery "$CASE_ERR"
}

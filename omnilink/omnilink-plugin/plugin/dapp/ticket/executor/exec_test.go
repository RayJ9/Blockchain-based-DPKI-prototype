...
import (
	...
	executor "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/ticket/executor"
	pty "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/ticket/types"
)
...
func Test_Exec_Bind_Unbind(t *testing.T) {
	omnilinkTestCfg := mock33.GetAPI().GetConfig()
	env := execEnv{
		1539918074,
		10000,
		2,
	...
	bindTx := createBindMiner(t, omnilinkTestCfg, string(Nodes[1]), string(Nodes[0]), priv)
	...
	unbindTx := createBindMiner(t, omnilinkTestCfg, "", string(Nodes[0]), priv)

...
func TestChannelClient_BindMiner(t *testing.T) {
	cfg := types.NewOmnilinkConfig(cfgstring)
...
func TestRPC_CallTestNode(t *testing.T) {
	...
	api.On("Version").Return(&types.VersionInfo{Omnilink: version.GetVersion()}, nil)
	api.On("Close").Return()
	...
	fmt.Println(err)
	assert.Nil(t, err)
	assert.Equal(t, version.GetVersion(), result.Omnilink)

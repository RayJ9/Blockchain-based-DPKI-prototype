...
func getRPCClient(t *testing.T, mocker *testnode.OmnilinkMock) *jsonclient.JSONClient {
...
//创建多重签名账户
func testAccCreateTx(t *testing.T, mocker *testnode.OmnilinkMock, jrpcClient *jsonclient.JSONClient) string {
	...
	rep := &types.Int64{}
	err = jrpcClient.Call("Omnilink.Query", &params, rep)
	assert.Nil(t, err)
	...
	err = jrpcClient.Call("Omnilink.Query", params, rep1)
	assert.Nil(t, err)
	//t.Log(rep1)
	...
	rep2 := &mty.MultiSig{}
	err = jrpcClient.Call("Omnilink.Query", params, rep2)
	assert.Nil(t, err)
	...
	rep3 := &mty.AccAddress{}
	err = jrpcClient.Call("Omnilink.Query", params, rep3)
	assert.Nil(t, err)
...
//多重签名地址转入操作
func testTransferInTx(t *testing.T, mocker *testnode.OmnilinkMock, jrpcClient *jsonclient.JSONClient, multiSigAccAddr string) {
	gen := mocker.GetGenesisKey()
	var params rpctypes.Query4Jrpc
	//send to exec
	...
	var res4 string
	err := jrpcClient.Call("Omnilink.CreateRawTransaction", req3, &res4)
	assert.Nil(t, err)
	tx := getTx(t, res4)
	tx.Sign(types.SECP256K1, gen)
	reply, err := mocker.GetAPI().SendTx(tx)
	...
	err = jrpcClient.Call("Omnilink.Query", params, rep4)
	assert.Nil(t, err)
...
//多重签名地址转出操作 AddrB
func testTransferOutTx(t *testing.T, mocker *testnode.OmnilinkMock, jrpcClient *jsonclient.JSONClient, multiSigAccAddr string) {
	gen := mocker.GetGenesisKey()
	var params rpctypes.Query4Jrpc
	...
	err = jrpcClient.Call("Omnilink.Query", params, rep7)
	assert.Nil(t, err)
	...
	err = jrpcClient.Call("Omnilink.Query", params, rep8)
	assert.Nil(t, err)
...
//owner add AddrE
func testAddOwner(t *testing.T, mocker *testnode.OmnilinkMock, jrpcClient *jsonclient.JSONClient, multiSigAccAddr string) {
	gen := mocker.GetGenesisKey()
	var params rpctypes.Query4Jrpc
	...
	rep10 := &mty.MultiSig{}
	err = jrpcClient.Call("Omnilink.Query", params, rep10)
	assert.Nil(t, err)
	find := false
...
//owner del AddrE
func testDelOwner(t *testing.T, mocker *testnode.OmnilinkMock, jrpcClient *jsonclient.JSONClient, multiSigAccAddr string) {
	gen := mocker.GetGenesisKey()
	var params rpctypes.Query4Jrpc
...
//ModifyOwnerWeight
func testModifyOwnerWeight(t *testing.T, mocker *testnode.OmnilinkMock, jrpcClient *jsonclient.JSONClient, multiSigAccAddr string) {
	gen := mocker.GetGenesisKey()
	var params rpctypes.Query4Jrpc
...
func testReplaceOwner(t *testing.T, mocker *testnode.OmnilinkMock, jrpcClient *jsonclient.JSONClient, multiSigAccAddr string) {
	gen := mocker.GetGenesisKey()
	var params rpctypes.Query4Jrpc
...
func testModifyDailyLimit(t *testing.T, mocker *testnode.OmnilinkMock, jrpcClient *jsonclient.JSONClient, multiSigAccAddr string) {
	gen := mocker.GetGenesisKey()
	var params rpctypes.Query4Jrpc
...
func testAddDailyLimit(t *testing.T, mocker *testnode.OmnilinkMock, jrpcClient *jsonclient.JSONClient, multiSigAccAddr string) {
	gen := mocker.GetGenesisKey()
	var params rpctypes.Query4Jrpc
...
func testModifyRequestWeight(t *testing.T, mocker *testnode.OmnilinkMock, jrpcClient *jsonclient.JSONClient, multiSigAccAddr string) {
	gen := mocker.GetGenesisKey()
	var params rpctypes.Query4Jrpc
...
func testConfirmTx(t *testing.T, mocker *testnode.OmnilinkMock, jrpcClient *jsonclient.JSONClient, multiSigAccAddr string) {
	var params rpctypes.Query4Jrpc
	...
	var res string
	err := jrpcClient.Call("Omnilink.CreateRawTransaction", req, &res)
	assert.Nil(t, err)
	gen := mocker.GetGenesisKey()
	tx := getTx(t, res)
	tx.Sign(types.SECP256K1, gen)
	...
	rep9 := &mty.MultiSig{}
	err = jrpcClient.Call("Omnilink.Query", params, rep9)
	assert.Nil(t, err)
	find := false
	...
	rep11 := &mty.Uint64{}
	err = jrpcClient.Call("Omnilink.Query", params, rep11)
	assert.Nil(t, err)
	//t.Log(rep11)
	txid := rep11.Data - 1
...
func confirmTx(t *testing.T, mocker *testnode.OmnilinkMock, jrpcClient *jsonclient.JSONClient, multiSigAccAddr string, privKey string, txid uint64, confirmOrRevoke bool) {
	//撤销这个交易的确认信息
...
//异常测试，主要是参数的合法性校验
func testAbnormal(t *testing.T, mocker *testnode.OmnilinkMock, jrpcClient *jsonclient.JSONClient) {
...
func testAbnormalCreateTx(t *testing.T, mocker *testnode.OmnilinkMock, jrpcClient *jsonclient.JSONClient, req *mty.MultiSigAccCreate, expecterr error) {
	gen := mocker.GetGenesisKey()
	var res string
	err := jrpcClient.Call("multisig.MultiSigAccCreateTx", req, &res)
	assert.Nil(t, err)

...
func CreateRawTx(actionTy int32, tokenId uint64, amount string, toEthAddress string,
	omnilinkAddr string, accountId uint64, toAccountId uint64) ([]byte, error) {
	var payload []byte
	switch actionTy {
	...
	case zt.TySetVerifierAction:
		verifier := &zt.ZkVerifier{
			Verifiers: strings.Split(omnilinkAddr, ","),
		}
...
func GetDepositMsg(payload *zt.ZkDeposit) *zt.ZkMsg {
	...
	omnilinkAddress := transferStr2Int(payload.OmnilinkAddr, 16)
	pubData = append(pubData, getBigEndBitsWithFixLen(omnilinkAddress, zt.HashBitWidth)...)
...
func GetTransferToNewMsg(payload *zt.ZkTransferToNew) *zt.ZkMsg {
	...
	omnilinkAddress, _ := new(big.Int).SetString(payload.ToLayer2Address, 16)
	pubData = append(pubData, getBigEndBitsWithFixLen(omnilinkAddress, zt.HashBitWidth)...)

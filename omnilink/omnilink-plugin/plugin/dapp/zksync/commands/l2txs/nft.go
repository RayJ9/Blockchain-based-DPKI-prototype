...
func setMintNFT(cmd *cobra.Command, args []string) {
	...
	for i := 0; i < len(ids); i++ {
		...
		if nil != err {
			fmt.Println("mint nft failed to createOmnilinkTx due to err:", err.Error())
			return
		}
		sendTx(rpcLaddr, tx)
	}
...
func transferNFT(cmd *cobra.Command, args []string) {
	...
	for i := 0; i < len(ids); i++ {
		...
		tx, err := createOmnilinkTx(keys[i], getRealExecName(paraName, zksyncTypes.Zksync), action)
		if nil != err {
			fmt.Println("transfer nft failed to createOmnilinkTx due to err:", err.Error())
			return
		}
		sendTx(rpcLaddr, tx)
	}
...
func withdrawNFT(cmd *cobra.Command, args []string) {
	...
	for i := 0; i < len(ids); i++ {
		...
		tx, err := createOmnilinkTx(keys[i], getRealExecName(paraName, zksyncTypes.Zksync), action)
		if nil != err {
			fmt.Println("WithdrawNFT failed to createOmnilinkTx due to err:", err.Error())
			return
		}
		sendTx(rpcLaddr, tx)
	}

...
func sendWithdrawTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "withdraw",
		Short: "send withdraw tx to omnilink",
		Run:   sendWithdraw,
	}
...
func sendWithdrawFlags(cmd *cobra.Command) {
	...
	cmd.Flags().Uint64P("accountID", "a", 0, "L2 account id on omnilink")
...
func sendManyWithdrawFlags(cmd *cobra.Command) {
	...
	cmd.Flags().Uint64P("tokenId", "t", 0, "eth token id")
	_ = cmd.MarkFlagRequired("tokenId")
	cmd.Flags().StringP("accountIDs", "a", "0", "L2 account ids on omnilink, use ',' separate")

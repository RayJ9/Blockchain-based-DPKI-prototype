...
func proxyManyExitFlag(cmd *cobra.Command) {
	...
	cmd.Flags().StringP("proxyIDs", "p", "0", "L2 proxy account ids on omnilink, use ',' separate")
	_ = cmd.MarkFlagRequired("proxyIDs")
	cmd.Flags().StringP("targetIDs", "g", "0", "L2 target account ids on omnilink, use ',' separate")
...
func proxyManyExit(cmd *cobra.Command, args []string) {
	...
	for i := 0; i < len(proxyIds); i++ {
		...
		if nil != err {
			fmt.Println("send ForceExit failed to createOmnilinkTx due to err:", err.Error())
			return
		}
		sendTx(rpcLaddr, tx)
	}

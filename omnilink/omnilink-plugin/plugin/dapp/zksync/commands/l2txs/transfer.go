...
func SendTransferTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "transfer",
		Short: "send transfer tx to omnilink",
		Run:   tranfer,
	}
...
func BatchSendTransferTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "batchtransfer",
		Short: "send transfer tx to omnilink batch",
...
func batchSendTransfer(cmd *cobra.Command, args []string) {
	...
	for i := uint64(0); i < count; i++ {
		tx, err := createOmnilinkTx(privateKey, getRealExecName(paraName, zksyncTypes.Zksync), action)
		if nil != err {
			fmt.Println("send transfer failed to createOmnilinkTx due to err:", err.Error())
			return
		}
...
func SendManyTransferTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "transfer_many",
		Short: "send many transfer tx to omnilink",
		Run:   tranferMany,
	}
...
func sendManyTransferFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringP("toIDs", "d", "0", "to account ids on omnilink, use ',' separate")
...
func SendManyTransferTxFromOneCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "transfer_many_2",
		Short: "from one addr, send many transfer tx to omnilink",

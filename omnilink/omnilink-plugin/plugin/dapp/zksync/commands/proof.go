...
func getFirstRootHashFlag(cmd *cobra.Command) {
	...
	cmd.Flags().StringP("omnilinkAddr", "c", "", "optional layer2 fee addr, hex format,default from config")
}

func getFirstRootHash(cmd *cobra.Command, args []string) {
	rpcLaddr, _ := cmd.Flags().GetString("rpc_laddr")
	eth, _ := cmd.Flags().GetString("ethAddr")
	omnilink, _ := cmd.Flags().GetString("omnilinkAddr")

	var params rpctypes.Query4Jrpc
	params.Execer = zt.Zksync
	req := &types.ReqAddrs{}
	if (len(eth) == 0 && len(omnilink) != 0) || (len(eth) == 0 && len(omnilink) != 0) {
		fmt.Fprintln(os.Stderr, "eth or layer2 addr nil")
		return
	}
	if len(eth) > 0 && len(omnilink) > 0 {
		req = &types.ReqAddrs{Addrs: []string{eth, omnilink}}
	}

...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/zksync/commands/l2txs"
...
func depositFlag(cmd *cobra.Command) {
	...
	cmd.Flags().StringP("omnilinkAddr", "c", "", "deposit omnilinkAddr")
	cmd.MarkFlagRequired("omnilinkAddr")
...
func deposit(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Omnilink.CreateTransaction", params, nil)
...
func contractToTreeFlag(cmd *cobra.Command) {
	...
	cmd.Flags().StringP("amount", "a", "0", "omnilink side decimal amount,default decimal 8, like 1eth fill 1e8")
	cmd.MarkFlagRequired("amount")
...
func transferToNewFlag(cmd *cobra.Command) {
	...
	cmd.MarkFlagRequired("ethAddress")
	cmd.Flags().StringP("omnilinkAddr", "c", "", "transferToNew toOmnilinkAddr")
	cmd.MarkFlagRequired("omnilinkAddr")
}
...
func transferToNew(cmd *cobra.Command, args []string) {
	...
	omnilinkAddrBigInt, err := transferStr2Int(omnilinkAddr, 16)
	if nil != err {
		fmt.Fprintln(os.Stderr, errors.Wrapf(err, "createRawTx"))
		return
	}
	omnilinkAddr = omnilinkAddrBigInt.Text(16)

	paraName, _ := cmd.Flags().GetString("paraName")
	rpcLaddr, _ := cmd.Flags().GetString("rpc_laddr")
	payload, err := wallet.CreateRawTx(zt.TyTransferToNewAction, tokenId, amount, toEthAddress, omnilinkAddr, accountId, 0)
	if err != nil {
		fmt.Fprintln(os.Stderr, errors.Wrapf(err, "createRawTx"))
		return
	}
...
func getOmnilinkAddrCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "l2addr",
		Short: "get omnilink l2 address by privateKey",
		Run:   getOmnilinkAddr,
	}
	getOmnilinkAddrFlag(cmd)
	return cmd
}

func getOmnilinkAddrFlag(cmd *cobra.Command) {
...
func getOmnilinkAddr(cmd *cobra.Command, args []string) {

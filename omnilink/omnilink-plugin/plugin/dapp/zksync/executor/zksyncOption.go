...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-base/system/dapp"
	"code.corp.bcollie.net/omnilink/omnilink-base/types"
	zt "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/zksync/types"
...
func (a *Action) Deposit(payload *zt.ZkDeposit) (*types.Receipt, error) {
	...
	zklog.Info("start zksync deposit", "eth", payload.EthAddress, "omnilink", payload.OmnilinkAddr)
	//只有管理员能操作
	cfg := a.api.GetConfig()
	...
	//转换10进制
	newAddr, ok := zt.HexAddr2Decimal(payload.OmnilinkAddr)
	if !ok {
		return nil, errors.Wrapf(types.ErrInvalidParam, "transfer omnilinkAddr=%s", payload.OmnilinkAddr)
	}
	payload.OmnilinkAddr = newAddr
	...
	leaf, err := GetLeafByOmnilinkAndEthAddress(a.statedb, payload.GetOmnilinkAddr(), payload.GetEthAddress())
	if err != nil {
		return nil, errors.Wrapf(err, "db.GetLeafByOmnilinkAndEthAddress")
	}
	...
	special := &zt.ZkDepositWitnessInfo{
		...
		Layer2Addr:   payload.OmnilinkAddr,
	...
	if leaf == nil {
		...
		if zt.SystemDefaultAcctId == lastAccountID {
			ethFeeAddr, omnilinkFeeAddr := getCfgFeeAddr(cfg)
			kvs4InitAccount, err := NewInitAccount(ethFeeAddr, omnilinkFeeAddr)
			if nil != err {
				return nil, err
			}
			...
			if payload.EthAddress == ethFeeAddr && payload.OmnilinkAddr == omnilinkFeeAddr {
		...
		//accountID, tokenID uint64, amount, ethAddress,  omnilinkAddr string, statedb dbm.KV, leaf *zt.Leaf) ([]*types.KeyValue, *types.ReceiptLog, error)
		createKVS, l2Log, _, err := applyL2AccountCreate(accountID, special.TokenID, special.Amount, payload.EthAddress, payload.OmnilinkAddr, a.statedb, true)
		if nil != err {
...
func (a *Action) ZkWithdraw(payload *zt.ZkWithdraw) (*types.Receipt, error) {
	...
	withdrawReceiptLog := &zt.AccountTokenBalanceReceipt{
		EthAddress:    leaf.EthAddress,
		OmnilinkAddr:  leaf.OmnilinkAddr,
...
func (a *Action) contractToTreeNewProc(payload *zt.ZkContractToTree, token *zt.ZkTokenSymbol) (*types.Receipt, error) {
	...
	if !ok {
		return nil, errors.Wrapf(types.ErrInvalidParam, "transfer omnilinkAddr=%s", payload.ToLayer2Addr)
	}
	...
	toLeaf, err := GetLeafByOmnilinkAndEthAddress(a.statedb, payload.ToLayer2Addr, payload.ToEthAddr)
	if err != nil {
		return nil, errors.Wrapf(err, "db.GetLeafByOmnilinkAndEthAddress")
	}
...
func (a *Action) transferToNewProcess(accountIdFrom uint64, toOmnilinkAddress, toEthAddress, totalAmount, amount string, tokenID uint64) (*types.Receipt, uint64, error) {
	...
	return a.transferToNewInnerProcess(fromLeaf, toOmnilinkAddress, toEthAddress, totalAmount, amount, tokenID)
}

//contract2tree 也支持tree上新创建账户id， 和transfer2new共用
func (a *Action) transferToNewInnerProcess(fromLeaf *zt.Leaf, toOmnilinkAddress, toEthAddress, totalAmount, amount string, tokenID uint64) (*types.Receipt, uint64, error) {
	var kvs []*types.KeyValue
	var logs []*types.ReceiptLog

	toLeaf, err := GetLeafByOmnilinkAndEthAddress(a.statedb, toOmnilinkAddress, toEthAddress)
	if err != nil {
		return nil, 0, errors.Wrapf(err, "db.GetLeafByOmnilinkAndEthAddress")
	}
	if toLeaf != nil {
	...
	toKVs, _, receiptTo, err := applyL2AccountCreate(accountIDNew, tokenID, amount, toEthAddress, toOmnilinkAddress, a.statedb, false)
	if nil != err {
...
func (a *Action) SetPubKey(payload *zt.ZkSetPubKey) (*types.Receipt, error) {
	...
	if payload.PubKeyTy == 0 {
		...
		calcOmnilinkAddr := zt.Byte2Str(hash.Sum(nil))
		if calcOmnilinkAddr != leaf.OmnilinkAddr {
			zklog.Error("SetPubKey", "leaf.OmnilinkAddr", leaf.OmnilinkAddr, "calcOmnilinkAddr", calcOmnilinkAddr)
...
func (a *Action) setTokenSymbol(payload *zt.ZkTokenSymbol) (*types.Receipt, error) {
	...
	//id已经存在，已有id修改symbol或decimal需要SystemTree2ContractAcctId的当前token balance为0的时候,说明没有转出到omnilink资产或者已经全部转回来了

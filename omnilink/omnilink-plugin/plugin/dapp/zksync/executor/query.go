...
func (z *zksync) Query_GetAccountById(in *zt.ZkQueryReq) (types.Message, error) {
	...
	leaf.OmnilinkAddr, ok = zt.DecimalAddr2Hex(leaf.GetOmnilinkAddr(), zt.BTYAddrLen)
	if !ok {
		return nil, errors.Wrapf(types.ErrInvalidParam, "wrong omnilink addr format=%s", leaf.GetOmnilinkAddr())
	}
	return &leaf, nil
}
...
// Query_GetAccountByOmnilink  通过omnilink地址查询account
func (z *zksync) Query_GetAccountByOmnilink(in *zt.ZkQueryReq) (types.Message, error) {
	res := new(zt.ZkQueryResp)
	addr, ok := zt.HexAddr2Decimal(in.GetOmnilinkAddr())
	if !ok {
		return nil, errors.Wrapf(types.ErrInvalidParam, "omnilink addr not hex format,%s", in.GetOmnilinkAddr())
	}
	leaves, err := GetLeafByOmnilinkAddress(z.GetLocalDB(), addr)
	if err != nil {
		return nil, err
	}
	for _, l := range leaves {
...
func (z *zksync) Query_GetTreeInitRoot(in *types.ReqAddrs) (types.Message, error) {
	...
	root := getInitTreeRoot(z.GetAPI().GetConfig(), eth, omnilink)
...
func (z *zksync) Query_GetZkContractAccount(in *zt.ZkQueryReq) (types.Message, error) {
	...
	contractAccount := accountdb.LoadAccount(in.OmnilinkWalletAddr)

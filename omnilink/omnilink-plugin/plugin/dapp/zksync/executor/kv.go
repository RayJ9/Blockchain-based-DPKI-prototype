...
func GetLocalOmnilinkEthPrimaryKey(omnilinkAddr string, ethAddr string) []byte {
	return []byte(fmt.Sprintf("%s-%s", address.FormatAddrKey(omnilinkAddr), address.FormatAddrKey(ethAddr)))
}

func GetOmnilinkEthPrimaryKey(omnilinkAddr string, ethAddr string) []byte {
	return []byte(fmt.Sprintf("%s%s-%s", KeyPrefixStateDB, address.FormatAddrKey(omnilinkAddr),

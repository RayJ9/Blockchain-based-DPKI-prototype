...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-base/common/db/table"
	"code.corp.bcollie.net/omnilink/omnilink-base/types"
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/mix/executor/merkletree"
...
func getCfgFeeAddr(cfg *types.OmnilinkConfig) (string, string) {
	confManager := types.ConfSub(cfg, zt.Zksync)
	ethAddr := confManager.GStr(zt.ZkCfgEthFeeAddr)
	omnilinkAddr := confManager.GStr(zt.ZkCfgLayer2FeeAddr)
	if len(ethAddr) <= 0 || len(omnilinkAddr) <= 0 {
		panic(fmt.Sprintf("zksync not cfg init fee addr, ethAddr=%s,33Addr=%s", ethAddr, omnilinkAddr))
	}
	ethAddrDecimal, _ := zt.HexAddr2Decimal(ethAddr)
	omnilinkAddrDecimal, _ := zt.HexAddr2Decimal(omnilinkAddr)
	return ethAddrDecimal, omnilinkAddrDecimal
}

// 由于ethAddr+omnilinkAddr 唯一确定一个accountId,所以设置初始账户的omnilinkAddr不相同
func getInitAccountLeaf(ethFeeAddr, omnilinkFeeAddr string) []*zt.Leaf {
...
func getInitTreeRoot(cfg *types.OmnilinkConfig, ethAddrDecimal, layer2AddrDecimal string) string {
...
func NewInitAccount(ethFeeAddr, omnilinkFeeAddr string) ([]*types.KeyValue, error) {
	if len(ethFeeAddr) <= 0 || len(omnilinkFeeAddr) <= 0 {
		return nil, errors.New("zksync default fee addr(ethFeeAddr,zkOmnilinkFeeAddr) is nil")
	}
	var kvs []*types.KeyValue
	initLeafAccounts := getInitAccountLeaf(ethFeeAddr, omnilinkFeeAddr)
	...
	for _, leaf := range initLeafAccounts {
		...
		kv = &types.KeyValue{
			Key:   GetOmnilinkEthPrimaryKey(leaf.OmnilinkAddr, leaf.EthAddress),
			Value: types.Encode(leaf),
		}
		kvs = append(kvs, kv)
...
func AddNewLeafOpt(ethAddress string, tokenId, accountId uint64, amount string, omnilinkAddr string) []*types.KeyValue {
	var kvs []*types.KeyValue
...
func applyL2AccountUpdate(accountID, tokenID uint64, amount string, option int32, statedb dbm.KV, leaf *zt.Leaf, makeEncode bool) ([]*types.KeyValue, *types.ReceiptLog, *zt.AccountTokenBalanceReceipt, error) {
	...
	l2Log.OmnilinkAddr = leaf.OmnilinkAddr
...
func applyL2AccountCreate(accountID, tokenID uint64, amount, ethAddress, omnilinkAddr string, statedb dbm.KV, makeEncode bool) ([]*types.KeyValue, *types.ReceiptLog, *zt.AccountTokenBalanceReceipt, error) {
	var kvs []*types.KeyValue
	var log *types.ReceiptLog
	//如果NFTAccountId第一次初始化token，因为缺省初始balance为（SystemNFTTokenId+1),这里add时候默认为+2
	if accountID == zt.SystemNFTAccountId && tokenID == zt.SystemNFTTokenId {
	...
	kvs = append(kvs, AddNewLeafOpt(ethAddress, tokenID, accountID, amount, omnilinkAddr)...)
	...
	l2Log.OmnilinkAddr = omnilinkAddr
	l2Log.TokenId = tokenID
	l2Log.AccountId = accountID
...
func GetLeafByEthAddress(db dbm.KV, ethAddress string) ([]*zt.Leaf, error) {
	...
	for _, row := range rows {
		...
		data.OmnilinkAddr, _ = zt.DecimalAddr2Hex(data.GetOmnilinkAddr(), zt.BTYAddrLen)
		datas = append(datas, data)
	}
	return datas, nil
}

func GetLeafByOmnilinkAddress(db dbm.KV, omnilinkAddr string) ([]*zt.Leaf, error) {
	accountTable := NewAccountTreeTable(db)
	rows, err := accountTable.ListIndex("omnilink_address", []byte(fmt.Sprintf("%s", omnilinkAddr)), nil, 1000, dbm.ListASC)
...
func GetLeafByOmnilinkAndEthAddress(db dbm.KV, omnilinkAddr, ethAddress string) (*zt.Leaf, error) {
	if omnilinkAddr == "" || ethAddress == "" {
		return nil, types.ErrInvalidParam
	}

	val, err := db.Get(GetOmnilinkEthPrimaryKey(omnilinkAddr, ethAddress))
	if err != nil {
		if err.Error() == types.ErrNotFound.Error() {
			return nil, nil
		} else {
...
func getLeafHash(h hash.Hash, leaf *zt.Leaf) []byte {
	...
	h.Write(zt.Str2Byte(leaf.GetOmnilinkAddr()))

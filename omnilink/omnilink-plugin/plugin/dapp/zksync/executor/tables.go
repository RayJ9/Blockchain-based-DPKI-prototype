...
var opt_account_tree = &table.Option{
	...
	Name:    "tree",
	Primary: "address",
	Index:   []string{"omnilink_address", "eth_address"},
}
...
func (r *AccountTreeRow) Get(key string) ([]byte, error) {
	if key == "address" {
		return GetLocalOmnilinkEthPrimaryKey(r.GetOmnilinkAddr(), r.GetEthAddress()), nil
	} else if key == "omnilink_address" {
		return []byte(fmt.Sprintf("%s", address.FormatAddrKey(r.GetOmnilinkAddr()))), nil

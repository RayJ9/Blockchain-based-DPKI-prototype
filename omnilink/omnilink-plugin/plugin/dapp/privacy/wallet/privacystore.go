...
import (
	...
	privacy "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/privacy/crypto"
	privacytypes "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/privacy/types"
...
func newStore(db db.DB, cfg *types.OmnilinkConfig) *privacyStore {
	return &privacyStore{Store: wcom.NewStore(db), OmnilinkConfig: cfg}
}

// privacyStore 隐私交易数据库存储操作类
type privacyStore struct {
	*types.OmnilinkConfig
	*wcom.Store
}
...
func (store *privacyStore) getWalletPrivacyTxDetails(param *privacytypes.ReqPrivacyTransactionList) (*types.WalletTxDetails, error) {
	...
	for _, keyByte := range txKeyBytes {
		...
		if txDetail.GetTx().IsWithdraw(store.OmnilinkConfig.GetCoinExec()) {
			//swap from and to
...
func (store *privacyStore) selectCurrentWalletPrivacyTx(txDetal *types.TransactionDetail, index int32, privacyInfo []addrAndprivacy, newbatch db.Batch) {
	...
	if assetExec == "" {
		assetExec = store.OmnilinkConfig.GetCoinExec()
	}

...
func StaticsCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "statics",
		Short: "statics of lock/burn asset from or to Ethereum and omnilink",
		Run:   ShowStatics,
	}
...
func ShowStaticsFlags(cmd *cobra.Command) {
	...
	cmd.Flags().Int32P("from", "f", 0, "source chain, 0=ethereum, and 1=omnilink")
...
func ShowStatics(cmd *cobra.Command, args []string) {
	...
	if from != 0 && 1 != from {
		fmt.Println("Pls set correct source chain flag, 0=ethereum, and 1=omnilink")
		return
	}

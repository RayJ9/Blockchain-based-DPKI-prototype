...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/dex/utils"
	evmAbi "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/evm/executor/abi"
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/evm/executor/vm/common"
	"github.com/spf13/cobra"
)

/*
./boss4x omnilink offline create -f 1 -k 0x027ca96466c71c7e7c5d73b7e1f43cb889b3bd65ebd2413eefd31c6709c262ae -n 'deploy crossx to omnilink' -r '1N6HstkyLFS8QCeVfdvYxx1xoryXoJtvvZ, [1N6HstkyLFS8QCeVfdvYxx1xoryXoJtvvZ, 155ooMPBTF8QQsGAknkK7ei5D78rwDEFe6, 13zBdQwuyDh7cKN79oT2odkxYuDbgQiXFv, 113ZzVamKfAtGt9dq45fX1mNsEoDiN95HG], [25, 25, 25, 25]' --chainID 33
./boss4x omnilink offline send -f deployCrossX2Omnilink.txt
*/

type DeployOmnilinkConfigInfo struct {
...
func createContractsWithFile(cmd *cobra.Command, _ []string) {
	...
	var deployCfg DeployOmnilinkConfigInfo
	...
	createOmnilinkDeployTxs(txCreateInfo, valsetParameter, multisignAddrs, from)
...
func createOmnilinkDeployTxs(txCreateInfo *utils.TxCreateInfo, valsetParameter, multisignAddrs string, from common.Address) {
	var txs []*utils.OmnilinkOfflineTx
	i := 1
	...
	fmt.Printf("%d: Going to create MulSign2omnilink \n", i)
	i += 1
	// 7
	createMulSign2omnilinkTx, err := createMulSignAndSign(txCreateInfo, from)
	if nil != err {
		fmt.Println("Failed to createMulSign2omnilinkTx due to cause:", err.Error())
		return
	}
	txs = append(txs, createMulSign2omnilinkTx)

	fmt.Printf("%d: Going to save multisign contract config offline account \n", i)
	i += 1
	// 8
	configMultisignOfflineSaveAccountTx, err := configMultisignOfflineSaveAccount(txCreateInfo, createMulSign2omnilinkTx.ContractAddr, bridgeBankTx.ContractAddr)
	if nil != err {
	...
	i += 1
	// 9
	multisignSetupTx, err := multisignSetup(txCreateInfo, multisignAddrs, createMulSign2omnilinkTx.ContractAddr)
	if nil != err {
		fmt.Println("Failed to configMultisignOfflineSaveAccountTx due to cause:", err.Error())
		return
	}
...
func createBridgeRegistryTxAndSign(txCreateInfo *utils.TxCreateInfo, from common.Address, ethereumBridge, valset, bridgeBank, oracle string) (*utils.OmnilinkOfflineTx, error) {
	...
	bridgeRegistryTx := &utils.OmnilinkOfflineTx{
...
func setOracle2EthBridgeTxAndSign(txCreateInfo *utils.TxCreateInfo, ethbridge, oracle string) (*utils.OmnilinkOfflineTx, error) {
...
func setBridgeBank2EthBridgeTxAndSign(txCreateInfo *utils.TxCreateInfo, ethbridge, bridgebank string) (*utils.OmnilinkOfflineTx, error) {
...
func createBridgeBankTxAndSign(txCreateInfo *utils.TxCreateInfo, from common.Address, oracle, ethereumBridge string) (*utils.OmnilinkOfflineTx, error) {
	...
	newContractAddr := common.NewContractAddress(from, txHash).String()
	bridgeBankTx := &utils.OmnilinkOfflineTx{
		ContractAddr:  newContractAddr,
		TxHash:        common.Bytes2Hex(txHash),
		SignedRawTx:   content,
...
func createOracleTxAndSign(txCreateInfo *utils.TxCreateInfo, from common.Address, valset, ethereumBridge string) (*utils.OmnilinkOfflineTx, error) {
	operator := from.String()
	...
	newContractAddr := common.NewContractAddress(from, txHash).String()
	oracleTx := &utils.OmnilinkOfflineTx{
		ContractAddr:  newContractAddr,
		TxHash:        common.Bytes2Hex(txHash),
		SignedRawTx:   content,
...
func createValsetTxAndSign(txCreateInfo *utils.TxCreateInfo, valsetParameter string, from common.Address) (*utils.OmnilinkOfflineTx, error) {
	...
	newContractAddr := common.NewContractAddress(from, txHash).String()
	valsetTx := &utils.OmnilinkOfflineTx{
		ContractAddr:  newContractAddr,
		TxHash:        common.Bytes2Hex(txHash),
		SignedRawTx:   content,
...
func createEthereumBridgeAndSign(txCreateInfo *utils.TxCreateInfo, from common.Address, valset string) (*utils.OmnilinkOfflineTx, error) {
	operator := from.String()
	...
	newContractAddr := common.NewContractAddress(from, txHash).String()
	ethereumBridgeTx := &utils.OmnilinkOfflineTx{
		ContractAddr:  newContractAddr,
		TxHash:        common.Bytes2Hex(txHash),
		SignedRawTx:   content,
...
func createMulSignAndSign(txCreateInfo *utils.TxCreateInfo, from common.Address) (*utils.OmnilinkOfflineTx, error) {
	content, txHash, err := utils.CreateContractAndSign(txCreateInfo, generated.GnosisSafeBin, generated.GnosisSafeABI, "", "mulSign2omnilink")
	if nil != err {
		return nil, err
	}

	newContractAddr := common.NewContractAddress(from, txHash).String()
	mulSign2omnilinkTx := &utils.OmnilinkOfflineTx{
		ContractAddr:  newContractAddr,
		TxHash:        common.Bytes2Hex(txHash),
		SignedRawTx:   content,
		OperationName: "deploy mulSign2omnilink",
		Interval:      time.Second * 5,
	}
	return mulSign2omnilinkTx, nil
}

func configMultisignOfflineSaveAccount(txCreateInfo *utils.TxCreateInfo, multisignContract, bridgebank string) (*utils.OmnilinkOfflineTx, error) {
...
func multisignSetup(txCreateInfo *utils.TxCreateInfo, multisignAddrs string, multisignContract string) (*utils.OmnilinkOfflineTx, error) {
	owners := strings.Split(multisignAddrs, ",")

	BTYAddrOmnilink := ebTypes.BTYAddrOmnilink
	parameter := "setup(["
	...
	parameter += "], "
	parameter += fmt.Sprintf("%d, %s, 0102, %s, %s, 0, %s)", len(owners), BTYAddrOmnilink, BTYAddrOmnilink, BTYAddrOmnilink, BTYAddrOmnilink)

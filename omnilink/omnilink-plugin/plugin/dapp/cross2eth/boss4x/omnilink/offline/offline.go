...
var crossXfileName = "deployCrossX2Omnilink.txt"

func Boss4xOfflineCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "offline",
		Short: "create and sign offline tx to deploy and set cross contracts to omnilink",
	}
	cmd.AddCommand(
		CreateCrossBridgeCmd(),
		CreateContractsWithFileCmd(),
		SendSignTxs2OmnilinkCmd(),
		CreateERC20Cmd(),
		ApproveErc20Cmd(),
...
func SendSignTxs2OmnilinkCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "send",
		Short: "send all the crossX txs to omnilink in serial",
		Run:   sendSignTxs2Omnilink,
	}
	addSendSignTxs2OmnilinkFlags(cmd)
	return cmd
}

func addSendSignTxs2OmnilinkFlags(cmd *cobra.Command) {
	cmd.Flags().StringP("file", "f", "", "signed tx file")
	_ = cmd.MarkFlagRequired("file")
}

func sendSignTxs2Omnilink(cmd *cobra.Command, _ []string) {
	filePath, _ := cmd.Flags().GetString("file")
	url, _ := cmd.Flags().GetString("rpc_laddr")
	utils.SendSignTxs2Omnilink(filePath, url)
}
...
func createOfflineTx(txCreateInfo *utils.TxCreateInfo, para []byte, contractAddr, name string, interval time.Duration) (*utils.OmnilinkOfflineTx, error) {

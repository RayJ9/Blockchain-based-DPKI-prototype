...
func CreateEthBridgeBankRelated(cmd *cobra.Command, _ []string) {
	...
	//step1 omnilinkbridge
	packData, err := deployomnilinkBridgePackData(deployerAddr, valSetAddr)
	if err != nil {
		panic(err.Error())
	}
	omnilinkBridgeAddr := crypto.CreateAddress(deployerAddr, startNonce)
	infos = append(infos, &DeployInfo{PackData: packData, ContractorAddr: omnilinkBridgeAddr, Name: "omnilinkBridge", Nonce: startNonce, To: nil})
	startNonce = startNonce + 1

	//step2 oracle
	packData, err = deployOraclePackData(deployerAddr, valSetAddr, omnilinkBridgeAddr)
	if err != nil {
		panic(err.Error())
	}
	...
	//step3 bridgebank
	packData, err = deployBridgeBankPackData(deployerAddr, omnilinkBridgeAddr, oracleAddr)
	if err != nil {
		panic(err.Error())
	}
	...
	infos = append(infos, &DeployInfo{PackData: packData, ContractorAddr: common.Address{}, Name: "setbridgebank", Nonce: startNonce, To: &omnilinkBridgeAddr})
	startNonce = startNonce + 1
	...
	infos = append(infos, &DeployInfo{PackData: packData, ContractorAddr: common.Address{}, Name: "setoracle", Nonce: startNonce, To: &omnilinkBridgeAddr})
	startNonce = startNonce + 1

	//step6 bridgeRegistry
	packData, err = deployBridgeRegistry(omnilinkBridgeAddr, bridgeBankAddr, oracleAddr, valSetAddr)
	if err != nil {
		panic(err.Error())
	}
...
func deployomnilinkBridgePackData(deployerAddr, valSetAddr common.Address) ([]byte, error) {
	parsed, err := abi.JSON(strings.NewReader(generated.OmnilinkBridgeABI))
	if err != nil {
		return nil, err
	}
	bin := common.FromHex(generated.OmnilinkBridgeBin)
...
func deployBridgeRegistry(omnilinkBridgeAddr, bridgeBankAddr, oracleAddr, valSetAddr common.Address) ([]byte, error) {
	parsed, err := abi.JSON(strings.NewReader(generated.BridgeRegistryABI))
	if err != nil {
		return nil, err
	}
	bin := common.FromHex(generated.BridgeRegistryBin)
	packData, err := parsed.Pack("", omnilinkBridgeAddr, bridgeBankAddr, oracleAddr, valSetAddr)
	if err != nil {
		return nil, err
	}
	return append(bin, packData...), nil

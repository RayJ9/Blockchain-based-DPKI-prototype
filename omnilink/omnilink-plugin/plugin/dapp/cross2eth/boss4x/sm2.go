...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-base/system/crypto/sm2"
...
func decryptWithSm2(cmd *cobra.Command, args []string) {
	...
	//第一步，解密数字信封
	sm2key, err := omnilinkCommon.FromHex(sm2keyStr)
	if nil != err {
		fmt.Println("omnilinkCommon.FromHex failed due to:" + err.Error())
		return
	}
	if 32 != len(sm2key) {
	...
	symmKeyCipher, err := omnilinkCommon.FromHex(symmKeyCipherStr)
	if nil != err {
		fmt.Println("omnilinkCommon.FromHex failed for cipher due to:" + err.Error())
		return
	}
	...
	fmt.Println("The decrypted sm4 key is:"+omnilinkCommon.ToHex(sm4Key), "len:", len(sm4Key))
	...
	cipher, err := omnilinkCommon.FromHex(cipherStr)
	if nil != err {
		fmt.Println("omnilinkCommon.FromHex failed for cipher due to:" + err.Error())
		return
	}
	...
	fmt.Println(omnilinkCommon.ToHex(dst))
}
...
func encryptWithSm2(cmd *cobra.Command, args []string) {
	...
	sm4Key, err := omnilinkCommon.FromHex(symmKeyCipher)
	if nil != err {
		fmt.Println("omnilinkCommon.FromHex failed for cipher due to:" + err.Error())
		return
	}
	...
	privateKeySlice, err := omnilinkCommon.FromHex(privatKeyStr)
	if nil != err {
		fmt.Println("omnilinkCommon.FromHex failed for cipher due to:" + err.Error())
		return
	}

...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/cross2eth/ebrelayer/relayer/ethereum"
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/cross2eth/ebrelayer/relayer/events"
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/cross2eth/ebrelayer/relayer/omnilink"
...
//Manager ...
type Manager struct {
	omnilinkRelayer *omnilink.Relayer4Omnilink
...
//NewRelayerManager ...
//1.验证人的私钥需要通过cli命令行进行导入，且omnilink和ethereum两种不同的验证人需要分别导入
//2.显示或者重新替换原有的私钥首先需要通过passpin进行unlock的操作
func NewRelayerManager(omnilinkRelayer *omnilink.Relayer4Omnilink, ethRelayers map[string]*ethereum.Relayer4Ethereum, db dbm.DB) *Manager {
	l, _ := lru.New(4096)
	manager := &Manager{
		omnilinkRelayer: omnilinkRelayer,
...
func (manager *Manager) SetPassphase(setPasswdReq *relayerTypes.ReqSetPasswd, result *interface{}) error {
	...
	if !utils.IsValidPassWord(setPasswdReq.Passphase) {
		return omnilinkTypes.ErrInvalidPassWord
	}
...
func (manager *Manager) ChangePassphase(setPasswdReq *relayerTypes.ReqChangePasswd, result *interface{}) error {
	...
	if len(manager.passphase) == 0 && manager.encryptFlag == EncryptEnable {
		...
		if !isok {
			...
			return omnilinkTypes.ErrVerifyOldpasswdFail
		}
	}
	...
	err = manager.omnilinkRelayer.StoreAccountWithNewPassphase(setPasswdReq.NewPassphase, setPasswdReq.OldPassphase)
	if err != nil {
		mlog.Error("ChangePassphase", "StoreAccountWithNewPassphase err", err)
		return err
	}
...
func (manager *Manager) Unlock(passphase string, result *interface{}) error {
	...
	if err := manager.omnilinkRelayer.RestorePrivateKeys(passphase); nil != err {
		info := fmt.Sprintf("Failed to RestorePrivateKeys for omnilinkRelayer due to:%s", err.Error())
...
//ImportOmnilinkRelayerPrivateKey 导入omnilinkrelayer验证人的私钥,该私钥实际用于向ethereum提交验证交易时签名使用
func (manager *Manager) ImportOmnilinkRelayerPrivateKey(importKeyReq *relayerTypes.ImportKeyReq, result *interface{}) error {
	manager.mtx.Lock()
	defer manager.mtx.Unlock()
	...
	err := manager.omnilinkRelayer.ImportPrivateKey(manager.passphase, privateKey)
	if err != nil {
		mlog.Error("ImportOmnilinkValidatorPrivateKey", "Failed due to cause:", err.Error())
		return err
	}

	*result = rpctypes.Reply{
		IsOk: true,
		Msg:  "Succeed to import private key for omnilink relayer",
	}
	return nil
}
...
//ShowOmnilinkRelayerValidator 显示在omnilink中以验证人validator身份进行登录的地址
func (manager *Manager) ShowOmnilinkRelayerValidator(param interface{}, result *interface{}) error {
	manager.mtx.Lock()
	defer manager.mtx.Unlock()
	var err error
	*result, err = manager.omnilinkRelayer.GetAccountAddr()
	if nil != err {
		return err
	}
...
func (manager *Manager) ResendOmnilinkEvent(param *relayerTypes.ResendOmnilinkEventReq, result *interface{}) error {
	manager.mtx.Lock()
	defer manager.mtx.Unlock()
	if err := manager.checkPermission(); nil != err {
		return err
	}
	err := manager.omnilinkRelayer.ResendOmnilinkEvent(param.Height)
	if nil != err {
		return err
	}
	*result = rpctypes.Reply{
		IsOk: true,
		Msg:  "Successful to ResendOmnilinkEvent",
	}
	return nil
}
...
func (manager *Manager) SetupOwner4Omnilink(setupMulSign *relayerTypes.SetupMulSign, result *interface{}) error {
	manager.mtx.Lock()
	defer manager.mtx.Unlock()
	if err := manager.checkPermission(); nil != err {
		return err
	}
	mulSign, err := manager.omnilinkRelayer.SetupMulSign(setupMulSign)
	if nil != err {
		return err
	}
	*result = rpctypes.Reply{
...
func (manager *Manager) SafeTransfer4Omnilink(para *relayerTypes.SafeTransfer, result *interface{}) error {
	manager.mtx.Lock()
	defer manager.mtx.Unlock()
	if err := manager.checkPermission(); nil != err {
		return err
	}
	mulSign, err := manager.omnilinkRelayer.SafeTransfer(para)
	if nil != err {
		return err
	}
	*result = rpctypes.Reply{
...
func (manager *Manager) Burn(burn *relayerTypes.Burn, result *interface{}) error {
	...
	txhash, err := ethInt.Burn(burn.OwnerKey, burn.TokenAddr, burn.OmnilinkReceiver, burn.Amount)
	if nil != err {
		return err
	}
	*result = rpctypes.Reply{
...
func (manager *Manager) BurnAsync(burn *relayerTypes.Burn, result *interface{}) error {
	...
	txhash, err := ethInt.BurnAsync(burn.OwnerKey, burn.TokenAddr, burn.OmnilinkReceiver, burn.Amount)
	if nil != err {
		return err
	}
	*result = rpctypes.Reply{
...
// SimBurnFromEth : 模拟从eth销毁资产，提币回到omnilink,使用LockBTY仅为测试使用
...
// SimLockFromEth : 模拟从eth锁住eth/erc20，转移到omnilink
...
func (manager *Manager) BurnAsyncFromOmnilink(burn *relayerTypes.BurnFromOmnilink, result *interface{}) error {
	manager.mtx.Lock()
	defer manager.mtx.Unlock()
	if err := manager.checkPermission(); nil != err {
		return err
	}
	txhash, err := manager.omnilinkRelayer.BurnAsyncFromOmnilink(burn.OwnerKey, burn.TokenAddr, burn.EthereumReceiver, burn.Amount)
	if nil != err {
		return err
	}
	*result = rpctypes.Reply{
...
func (manager *Manager) LockBTYAssetAsync(lockEthErc20Asset *relayerTypes.LockBTY, result *interface{}) error {
	...
	txhash, err := manager.omnilinkRelayer.LockBTYAssetAsync(lockEthErc20Asset.OwnerKey, lockEthErc20Asset.Amount, lockEthErc20Asset.EtherumReceiver)
	if nil != err {
		return err
	}
	*result = rpctypes.Reply{
...
func (manager *Manager) LockEthErc20AssetAsync(lockEthErc20Asset *relayerTypes.LockEthErc20, result *interface{}) error {
	...
	txhash, err := ethInt.LockEthErc20AssetAsync(lockEthErc20Asset.OwnerKey, lockEthErc20Asset.TokenAddr, lockEthErc20Asset.Amount, lockEthErc20Asset.OmnilinkReceiver)
	if nil != err {
		return err
	}
	*result = rpctypes.Reply{
...
func (manager *Manager) LockEthErc20Asset(lockEthErc20Asset *relayerTypes.LockEthErc20, result *interface{}) error {
	...
	txhash, err := ethInt.LockEthErc20Asset(lockEthErc20Asset.OwnerKey, lockEthErc20Asset.TokenAddr, lockEthErc20Asset.Amount, lockEthErc20Asset.OmnilinkReceiver)
	if nil != err {
		return err
	}
	*result = rpctypes.Reply{
...
//ShowBridgeRegistryAddr4omnilink ...
func (manager *Manager) ShowBridgeRegistryAddr4omnilink(para interface{}, result *interface{}) error {
	manager.mtx.Lock()
	defer manager.mtx.Unlock()
	addr, err := manager.omnilinkRelayer.ShowBridgeRegistryAddr()
	if nil != err {
		return err
	}
	*result = relayerTypes.ReplyAddr{
...
func (manager *Manager) ShowTokenAddress(token2show *relayerTypes.TokenAddress, result *interface{}) error {
	...
	if ethINt, ok := manager.ethRelayer[token2show.ChainName]; ok {
		...
		res, err = manager.omnilinkRelayer.ShowTokenAddress(token2show)
		if nil != err {
			return err
		}
	}
...
// ShowTokenStatics ShowEthRelayer2OmnilinkTxs ...
func (manager *Manager) ShowTokenStatics(request *relayerTypes.TokenStaticsRequest, result *interface{}) error {
	...
	if relayerTypes.Source_Chain_Omnilink == request.From {
		...
		*result = resFinal
		...
		res, err := manager.omnilinkRelayer.ShowStatics(request)
		if nil != err {
			return err
		}
		*result = res
...
func (manager *Manager) CreateLockEventManually(createLockEventReq *relayerTypes.CreateLockEventReq, result *interface{}) error {
	...
	omnilinkAddressTo, err := omnilinkAddress.NewBtcAddress(createLockEventReq.To)
	if nil != err {
		return err
	}
...
func (manager *Manager) SetOmnilinkMultiSignAddr(multiSignAddr string, result *interface{}) error {
	manager.mtx.Lock()
	defer manager.mtx.Unlock()
	if err := manager.checkPermission(); nil != err {
		return err
	}
	manager.omnilinkRelayer.SetMultiSignAddr(multiSignAddr)
	*result = rpctypes.Reply{
		IsOk: true,
	}
	return nil
...
func (manager *Manager) GetOmnilinkMultiSignAddr(chainName string, result *interface{}) error {
	manager.mtx.Lock()
	defer manager.mtx.Unlock()
	if err := manager.checkPermission(); nil != err {
		return err
	...
	*result = rpctypes.Reply{
		IsOk: true,
		Msg:  manager.omnilinkRelayer.GetMultiSignAddr(),
	}
	return nil
}
...
func (manager *Manager) WithdrawFromOmnilink(burn *relayerTypes.BurnFromOmnilink, result *interface{}) error {
	manager.mtx.Lock()
	defer manager.mtx.Unlock()
	if err := manager.checkPermission(); nil != err {
		return err
	}
	txhash, err := manager.omnilinkRelayer.WithdrawFromOmnilink(burn.OwnerKey, burn.TokenAddr, burn.EthereumReceiver, burn.Amount)
	if nil != err {
		return err
	}
	*result = rpctypes.Reply{
...
func (manager *Manager) BurnWithIncreaseAsyncFromOmnilink(burn *relayerTypes.BurnFromOmnilink, result *interface{}) error {
	manager.mtx.Lock()
	defer manager.mtx.Unlock()
	if err := manager.checkPermission(); nil != err {
		return err
	}
	txhash, err := manager.omnilinkRelayer.BurnWithIncreaseAsyncFromOmnilink(burn.OwnerKey, burn.TokenAddr, burn.EthereumReceiver, burn.Amount)
	if nil != err {
		return err
	}
	*result = rpctypes.Reply{

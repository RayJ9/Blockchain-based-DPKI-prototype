...
//Burn ...
func Burn(ownerPrivateKeyStr, tokenAddrstr, omnilinkReceiver string, bridgeBank common.Address, amount *big.Int,
	...
	//omnilinkbank 是bridgeBank的基类，所以使用bridgeBank的地址
	...
	receAddr, err := omnilinkAddress.NewBtcAddress(omnilinkReceiver)
	if nil != err {
		txslog.Info("Burn", "Failed to decode omnilink address due to", err.Error())
		return "", err
	}
...
//BurnAsync ...
func BurnAsync(ownerPrivateKeyStr, tokenAddrstr, omnilinkReceiver string, amount *big.Int, bridgeBankIns *generated.BridgeBank,
	...
	receAddr, err := omnilinkAddress.NewBtcAddress(omnilinkReceiver)
	if nil != err {
		txslog.Info("BurnAsync", "Failed to decode omnilink address due to", err.Error())
		return "", err
	}
...
//LockEthErc20Asset ...
func LockEthErc20Asset(ownerPrivateKeyStr, tokenAddrStr, omnilinkReceiver string, amount *big.Int, client ethinterface.EthClientSpec, bridgeBank *generated.BridgeBank, bridgeBankAddr common.Address, addr2TxNonce map[common.Address]*NonceMutex, providerHttp string) (string, error) {
	var prepareDone bool
	txslog.Info("LockEthErc20Asset", "ownerPrivateKeyStr", ownerPrivateKeyStr, "tokenAddrStr", tokenAddrStr, "omnilinkReceiver", omnilinkReceiver, "amount", amount.String())
	ownerPrivateKey, err := crypto.ToECDSA(common.FromHex(ownerPrivateKeyStr))
	if nil != err {
		return "", err
	}
	...
	recvAddr, err := omnilinkAddress.NewBtcAddress(omnilinkReceiver)
	if nil != err {
		txslog.Info("LockEthErc20Asset", "Failed to decode omnilink address due to", err.Error())
		return "", err
	}
...
func LockEthErc20AssetAsync(ownerPrivateKeyStr, tokenAddrStr, omnilinkReceiver string, amount *big.Int, client ethinterface.EthClientSpec, bridgeBank *generated.BridgeBank, addr2TxNonce map[common.Address]*NonceMutex) (string, error) {
	txslog.Info("LockEthErc20AssetAsync", "ownerPrivateKeyStr", ownerPrivateKeyStr, "tokenAddrStr", tokenAddrStr, "omnilinkReceiver", omnilinkReceiver, "amount", amount.String())
	ownerPrivateKey, err := crypto.ToECDSA(common.FromHex(ownerPrivateKeyStr))
	if nil != err {
		return "", err
	}
	...
	recvAddr, err := omnilinkAddress.NewBtcAddress(omnilinkReceiver)
	if nil != err {
		txslog.Info("LockEthErc20AssetAsync", "Failed to decode omnilink address due to", err.Error())
		return "", err
	}

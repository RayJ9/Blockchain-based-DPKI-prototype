...
var (
	omnilinkToEthStaticsPrefix     = "eth-omnilinkToEthStatics"
	omnilinkToEthTxTotalAmount     = "eth-omnilinkToEthTxTotalAmount"
	...
	//Below for relay ack
	omnilinkTxRelayAlreadyPrefix = "eth-omnilinkTxRelayAlready"
	//有待确认
	ethTxIsRelayedUnconfirm = "eth-ethTxIsRelayedUnconfirm"
	//已经中继发送
	ethTxRelayedAlready      = "eth-ethTxRelayedAlready"
	fdTx2OmnilinkTotalAmount = "eth-fdTx2OmnilinkTotalAmount"
	// ethereum burn 已经执行
...
func (ethRelayer *Relayer4Ethereum) setClaimIDExecuteAlready(claimID string, txRelayConfirm *ebTypes.ProphecyProcessed) error {
	...
	data := omnilinkTypes.Encode(txRelayConfirm)
...
func (ethRelayer *Relayer4Ethereum) getClaimIDExecuteAlready(claimID string) (*ebTypes.ProphecyProcessed, error) {
	...
	err = omnilinkTypes.Decode(data, &prophecyProcessed)
...
func omnilinkTxRelayAlreadyKey(chainName, omnilinkTxhash string) []byte {
	return []byte(fmt.Sprintf("%s-chainName-%s-%s", omnilinkTxRelayAlreadyPrefix, chainName, omnilinkTxhash))
}
...
func calcRelayFromOmnilinkKey(chainName string, claimType int32, txindex int64) []byte {
	return []byte(fmt.Sprintf("%s-%s-%d-%012d", chainName, omnilinkToEthStaticsPrefix, claimType, txindex))
}

func calcRelayFromOmnilinkListPrefix(chainName string, claimType int32) []byte {
	return []byte(fmt.Sprintf("%s-%s-%d-", chainName, omnilinkToEthStaticsPrefix, claimType))
}

func (ethRelayer *Relayer4Ethereum) setOmnilinkTxRelayAlreadyInfo(chainName, omnilinkTxhash string, relayTxDetail *ebTypes.RelayTxDetail) error {
	key := omnilinkTxRelayAlreadyKey(chainName, omnilinkTxhash)
	data := omnilinkTypes.Encode(relayTxDetail)
	return ethRelayer.db.SetSync(key, data)
}

func (ethRelayer *Relayer4Ethereum) getOmnilinkTxRelayAlreadyInfo(chainName, omnilinkTxhash string) (*ebTypes.RelayTxDetail, error) {
	key := omnilinkTxRelayAlreadyKey(chainName, omnilinkTxhash)
	data, err := ethRelayer.db.Get(key)
	if nil != err {
		return nil, err
...
func (ethRelayer *Relayer4Ethereum) getStatics(claimType int32, txIndex int64, count int32) ([][]byte, error) {
	keyPrefix := calcRelayFromOmnilinkListPrefix(ethRelayer.name, claimType)

	keyFrom := calcRelayFromOmnilinkKey(ethRelayer.name, claimType, txIndex)
...
func (ethRelayer *Relayer4Ethereum) setEthLockTxUpdateTxIndex(txindex int64, claimType events.ClaimType) error {
	...
	if events.ClaimTypeBurn == claimType {
		...
		return ethRelayer.db.SetSync(key, omnilinkTypes.Encode(txIndexWrapper))
	}
...
func (ethRelayer *Relayer4Ethereum) updateTotalTxAmountFromomnilink(totalIndex int64) error {
	totalTx := &omnilinkTypes.Int64{
		Data: totalIndex,
	}
	key := []byte(fmt.Sprintf("%s-%s", ethRelayer.name, omnilinkToEthTxTotalAmount))
	//更新成功见证的交易数
	return ethRelayer.db.SetSync(key, omnilinkTypes.Encode(totalTx))
}
...
func (ethRelayer *Relayer4Ethereum) setLastestStatics(claimType int32, txIndex int64, data []byte) error {
	key := calcRelayFromOmnilinkKey(ethRelayer.name, claimType, txIndex)
	return ethRelayer.db.SetSync(key, data)
}
...
func (ethRelayer *Relayer4Ethereum) setLogProcHeight(key []byte, height uint64) error {
	...
	return ethRelayer.db.SetSync(key, omnilinkTypes.Encode(data))
}
...
func (ethRelayer *Relayer4Ethereum) getLogProcHeight(key []byte) uint64 {
	...
	err = omnilinkTypes.Decode(value, &height)
	if nil != err {
		return 0
	}
	return height.Data
...
func (ethRelayer *Relayer4Ethereum) setBridgeBankProcessedHeight(height uint64, index uint32) {
	bytes := omnilinkTypes.Encode(&ebTypes.EventLogIndex{
		Height: height,
		Index:  index})
...
func (ethRelayer *Relayer4Ethereum) getLastBridgeBankProcessedHeight() ebTypes.EventLogIndex {
	...
	_ = omnilinkTypes.Decode(data, &logIndex)
	return logIndex
}
...
func (ethRelayer *Relayer4Ethereum) GetLockedTokenAddress(symbol string) (*ebTypes.TokenAddress, error) {
	...
	if err := omnilinkTypes.Decode(data, &token2set); nil != err {
		return nil, err
	}
...
func (ethRelayer *Relayer4Ethereum) RestoreTokenAddress() error {
	...
	for _, data := range datas {
		...
		err := omnilinkTypes.Decode(data, &tokenLocked)
		if nil != err {
			return err
		}
...
func (ethRelayer *Relayer4Ethereum) setWithdrawFee(symbol2Para map[string]*ebTypes.WithdrawPara) error {
	...
	bytes := omnilinkTypes.Encode(withdrawSymbol2Fee)
...
func (ethRelayer *Relayer4Ethereum) restoreWithdrawFee() map[string]*ebTypes.WithdrawPara {
	...
	if err := omnilinkTypes.Decode(bytes, &withdrawSymbol2Para); nil != err {
		result := make(map[string]*ebTypes.WithdrawPara)
		return result
	}
...
func calcWithdrawKey(name, omnilinkSender, symbol string, year, month, day int, nonce int64) []byte {
	return []byte(fmt.Sprintf("%s-%s-%s-%s-%d-%d-%d-%d", name, withdrawTokenPrefix, omnilinkSender, symbol, year, month, day, nonce))
}

func calcWithdrawKeyPrefix(name, omnilinkSender, symbol string, year, month, day int) []byte {
	return []byte(fmt.Sprintf("%s-%s-%s-%s-%d-%d-%d", name, withdrawTokenPrefix, omnilinkSender, symbol, year, month, day))
}
...
func (ethRelayer *Relayer4Ethereum) setWithdraw(withdrawTx *ebTypes.WithdrawTx) error {
	omnilinkSender := withdrawTx.OmnilinkSender
	symbol := withdrawTx.Symbol
	year := withdrawTx.Year
	month := withdrawTx.Month
	day := withdrawTx.Day

	key := calcWithdrawKey(ethRelayer.name, omnilinkSender, symbol, int(year), int(month), int(day), withdrawTx.Nonce)
	bytes := omnilinkTypes.Encode(withdrawTx)
...
func (ethRelayer *Relayer4Ethereum) setWithdrawStatics(withdrawTx *ebTypes.WithdrawTx, omnilinkMsg *events.OmnilinkMsg) error {
	...
	relayerLog.Info("setWithdrawStatics::successful", "txIndex", txIndex, "OmnilinkTxhash", statics.OmnilinkTxhash, "EthereumTxhash", statics.EthereumTxhash, "type", operationType,
		"Symbol", omnilinkMsg.Symbol, "Amount", omnilinkMsg.Amount, "EthereumReceiver", statics.EthereumReceiver, "OmnilinkSender", statics.OmnilinkSender)
	...
	relayerLog.Info("setWithdrawStatics::relayomnilinkToEthereumCheckPonit_2::sendBackAck", "omnilinkTxHash", omnilinkTxHash, "ForwardIndex", omnilinkMsg.ForwardIndex, "FdTimes", omnilinkMsg.ForwardTimes)
	...
	relayerLog.Info("setWithdrawStatics::relayomnilinkToEthereumCheckPonit_3::setRelayFinishFlag", "omnilinkTxHash", omnilinkTxHash, "ForwardIndex", omnilinkMsg.ForwardIndex, "FdTimes", omnilinkMsg.ForwardTimes)

	data := omnilinkTypes.Encode(statics)
	return ethRelayer.setLastestStatics(int32(omnilinkMsg.ClaimType), txIndex, data)
}
...
func (ethRelayer *Relayer4Ethereum) getWithdrawsWithinSameDay(withdrawTx *ebTypes.WithdrawTx) (*big.Int, error) {
	...
	prefix := calcWithdrawKeyPrefix(ethRelayer.name, omnilinkSender, symbol, int(year), int(month), int(day))
	helper := dbm.NewListHelper(ethRelayer.db)
	datas := helper.List(prefix, nil, 100, dbm.ListASC)
	if nil == datas {
	...
	for _, data := range datas {
		...
		err := omnilinkTypes.Decode(data, &info)
		if nil != err {
...
func (ethRelayer *Relayer4Ethereum) updateFdTx2EthTotalAmount(index int64) error {
	...
	//更新成功见证的交易数
	return ethRelayer.db.SetSync([]byte(fdTx2OmnilinkTotalAmount), omnilinkTypes.Encode(totalTx))
}

func (ethRelayer *Relayer4Ethereum) getFdTx2OmnilinkTotalAmount() int64 {
	totalTx, _ := utils.LoadInt64FromDB([]byte(fdTx2OmnilinkTotalAmount), ethRelayer.db)
	return totalTx
}

...
import (
	...
	bridgeRegistry "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/cross2eth/contracts/contracts4eth/generated"
...
const (
	...
	BridgeBank
	// OmnilinkBridge : omnilinkBridge contract
	OmnilinkBridge
)

// String : returns the event type as a string
func (d ContractRegistry) String() string {
	return [...]string{"valset", "oracle", "bridgebank", "omnilinkbridge", "notsupport"}[d-1]
}
...
func GetAddressFromBridgeRegistry(client ethinterface.EthClientSpec, sender, registry common.Address, target ContractRegistry) (address *common.Address, err error) {
	...
	case OmnilinkBridge:
		omnilinkBridgeAddress, err := registryInstance.OmnilinkBridge(&auth)
		if err != nil {
			log.Fatal(err)
		}
		return &omnilinkBridgeAddress, nil
	default:

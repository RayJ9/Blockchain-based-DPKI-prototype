...
import (
	...
	gnosis "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/cross2eth/contracts/gnosis/generated"
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/cross2eth/ebrelayer/relayer/ethereum/ethinterface"
	...
	cross2ethErrors "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/cross2eth/ebrelayer/types"
...
type Relayer4Ethereum struct {
	...
	omnilinkMsgChan          <-chan *events.OmnilinkMsg
	totalTxRelayFromOmnilink int64
...
func StartEthereumRelayer(startPara *EthereumStartPara) *Relayer4Ethereum {
	...
	ethRelayer.totalTxRelayFromOmnilink = ethRelayer.getTotalTxAmount2Eth()
	if 0 == ethRelayer.totalTxRelayFromOmnilink {
		statics := &ebTypes.OmnilinkToEthereumStatics{}
...
func (ethRelayer *Relayer4Ethereum) IsProphecyPending(claimID [32]byte) (bool, error) {
	return ethtxs.IsProphecyPending(claimID, ethRelayer.ethSender, ethRelayer.x2EthContracts.OmnilinkBridge)
}

//Burn ...
func (ethRelayer *Relayer4Ethereum) Burn(ownerPrivateKey, tokenAddr, omnilinkReceiver, amount string) (string, error) {
	bn := big.NewInt(1)
	bn, _ = bn.SetString(utils.TrimZeroAndDot(amount), 10)
	return ethtxs.Burn(ownerPrivateKey, tokenAddr, omnilinkReceiver, ethRelayer.x2EthDeployInfo.BridgeBank.Address, bn,
		ethRelayer.x2EthContracts.BridgeBank, ethRelayer.clientSpec, ethRelayer.Addr2TxNonce, ethRelayer.providerHttp[0])
}

//BurnAsync ...
func (ethRelayer *Relayer4Ethereum) BurnAsync(ownerPrivateKey, tokenAddr, omnilinkReceiver, amount string) (string, error) {
	bn := big.NewInt(1)
	bn, _ = bn.SetString(utils.TrimZeroAndDot(amount), 10)
	return ethtxs.BurnAsync(ownerPrivateKey, tokenAddr, omnilinkReceiver, bn, ethRelayer.x2EthContracts.BridgeBank, ethRelayer.clientSpec, ethRelayer.Addr2TxNonce)
}
...
//LockEthErc20Asset ...
func (ethRelayer *Relayer4Ethereum) LockEthErc20Asset(ownerPrivateKey, tokenAddr, amount string, omnilinkReceiver string) (string, error) {
	bn := big.NewInt(1)
	bn, _ = bn.SetString(utils.TrimZeroAndDot(amount), 10)
	return ethtxs.LockEthErc20Asset(ownerPrivateKey, tokenAddr, omnilinkReceiver, bn, ethRelayer.clientSpec, ethRelayer.x2EthContracts.BridgeBank, ethRelayer.x2EthDeployInfo.BridgeBank.Address, ethRelayer.Addr2TxNonce, ethRelayer.providerHttp[0])
}

//LockEthErc20AssetAsync ...
func (ethRelayer *Relayer4Ethereum) LockEthErc20AssetAsync(ownerPrivateKey, tokenAddr, amount string, omnilinkReceiver string) (string, error) {
	bn := big.NewInt(1)
	bn, _ = bn.SetString(utils.TrimZeroAndDot(amount), 10)
	return ethtxs.LockEthErc20AssetAsync(ownerPrivateKey, tokenAddr, omnilinkReceiver, bn, ethRelayer.clientSpec, ethRelayer.x2EthContracts.BridgeBank, ethRelayer.Addr2TxNonce)
}
...
func (ethRelayer *Relayer4Ethereum) proc() {
	...
	for {
		...
		case omnilinkMsg := <-ethRelayer.omnilinkMsgChan:
			ethRelayer.handleOmnilinkMsg(omnilinkMsg)
...
func (ethRelayer *Relayer4Ethereum) procTxRelayAck(ack *ebTypes.TxRelayAck) {
	...
	//relayEthereum2omnilinkCheckPonit 4:procTxRelayAck
	relayerLog.Info("relayLockBurnToOmnilink::relayEthereum2omnilinkCheckPonit_4::procTxRelayAck", "ethTxhash", ack.TxHash, "ForwardIndex", ack.FdIndex)
}

func (ethRelayer *Relayer4Ethereum) handleOmnilinkMsg(omnilinkMsg *events.OmnilinkMsg) {
	if omnilinkMsg.ClaimType == events.ClaimTypeWithdraw {
		ethRelayer.handleLogWithdraw(omnilinkMsg)
		return
	}

	ethRelayer.handleLogLockBurn(omnilinkMsg)
	return
}
...
func (ethRelayer *Relayer4Ethereum) checkPermissionWithinOneDay(withdrawTx *ebTypes.WithdrawTx) (*big.Int, error) {
	...
	relayerLog.Info("checkPermissionWithinOneDay", "total withdraw already", totalAlready, "OmnilinkSender", withdrawTx.OmnilinkSender,
...
func (ethRelayer *Relayer4Ethereum) remindBalanceNotEnough(addr, symbol, omnilinkTxHash string) {
	ethName := "以太坊"
	...
	relayerLog.Debug("SendRemind", "remindUrl", ethRelayer.remindUrl, "postData:", postData, "omnilinkTxhash", omnilinkTxHash)
	ethRelayer.SendRemind(ethRelayer.remindUrl, postData)
}

func (ethRelayer *Relayer4Ethereum) handleLogWithdraw(omnilinkMsg *events.OmnilinkMsg) {
	...
	withdrawTx := &ebTypes.WithdrawTx{
		OmnilinkSender:   omnilinkMsg.OmnilinkSender.String(),
		EthereumReceiver: omnilinkMsg.EthereumReceiver.String(),
		Symbol:           omnilinkMsg.Symbol,
		TxHashOnOmnilink: common.Bytes2Hex(omnilinkMsg.TxHash),
		Nonce:            omnilinkMsg.Nonce,
		Year:             int32(year),
		Month:            int32(month),
		Day:              int32(day),
	}
	...
	if ethRelayer.checkIsResendOmnilinkMsg(omnilinkMsg) {
		return
	}
	omnilinkTxHash := omnilinkCommon.ToHex(omnilinkMsg.TxHash)
	...
	defer func() {
		if err != nil {
			...
			relayerLog.Error("handleLogWithdraw", "Failed to withdraw due to:", err.Error(), "omnilinkTxhash", omnilinkTxHash)
		}

		err := ethRelayer.setWithdraw(withdrawTx)
		if nil != err {
			relayerLog.Error("handleLogWithdraw", "Failed to setWithdraw due to:", err.Error(), "omnilinkTxhash", omnilinkTxHash)
		}

		err = ethRelayer.setWithdrawStatics(withdrawTx, omnilinkMsg)
		if nil != err {
			relayerLog.Error("handleLogWithdraw", "Failed to setWithdrawStatics due to:", err.Error(), "omnilinkTxhash", omnilinkTxHash)
		}
	}()

	relayerLog.Info("handleLogWithdraw", "Received omnilinkMsg", omnilinkMsg, "tx hash string", omnilinkTxHash)
	withdrawFromOmnilinkTokenInfo, exist := ethRelayer.symbol2LockAddr[omnilinkMsg.Symbol]
	if !exist {
		//因为是withdraw操作，必须从允许lock的token地址中进行查询
		relayerLog.Error("handleLogWithdraw", "Failed to fetch locked Token Info for symbol", omnilinkMsg.Symbol, "omnilinkTxhash", omnilinkTxHash)
		err = errors.New("ErrFetchLockedTokenInfo")
		return
	}

	tokenAddr := common.HexToAddress(withdrawFromOmnilinkTokenInfo.Address)
	//从omnilink进行withdraw回来的token需要根据精度进行相应的缩放
	if 8 != withdrawFromOmnilinkTokenInfo.Decimal {
		dist := math.Abs(float64(withdrawFromOmnilinkTokenInfo.Decimal - 8))
		value, exist := utils.Decimal2value[int(dist)]
		if !exist {
			relayerLog.Error("handleLogWithdraw", "does support for decimal, %d", withdrawFromOmnilinkTokenInfo.Decimal, "omnilinkTxhash", omnilinkTxHash)
			err = errors.New("ErrDecimalNotSupport")
			return
		}

		if withdrawFromOmnilinkTokenInfo.Decimal > 8 {
			omnilinkMsg.Amount.Mul(omnilinkMsg.Amount, big.NewInt(value))
		} else {
			omnilinkMsg.Amount.Div(omnilinkMsg.Amount, big.NewInt(value))
		}
	}
	withdrawTx.Amount = omnilinkMsg.Amount.String()
	relayerLog.Info("handleLogWithdraw", "token address", tokenAddr.String(), "amount", withdrawTx.Amount,
		"Receiver on Ethereum", omnilinkMsg.EthereumReceiver.String(), "omnilinkTxhash", omnilinkTxHash)
	...
	if omnilinkMsg.Amount.Cmp(feeAmount) < 0 {
		relayerLog.Error("handleLogWithdraw", "ErrWithdrawAmountLessThanFee feeAmount", feeAmount.String(), "Withdraw Amount", omnilinkMsg.Amount.String(), "omnilinkTxhash", omnilinkTxHash)
		err = errors.New("ErrWithdrawAmountCan'tPay4Fee")
		return
	}
	amount2transfer := omnilinkMsg.Amount.Sub(omnilinkMsg.Amount, feeAmount)
	value := big.NewInt(0)
	...
	if tokenAddr.String() != ethtxs.EthNullAddr { //判断是否要Pack EVM数据
		...
		intputData, err = ethRelayer.packTransferData(omnilinkMsg.EthereumReceiver, amount2transfer)
		if err != nil {
			relayerLog.Error("handleLogWithdraw", "CallEvmData err", err, "omnilinkTxhash", omnilinkTxHash)
		...
		if err != nil {
			relayerLog.Error("handleLogWithdraw", "callEvmBalanceData err", err, "omnilinkTxhash", omnilinkTxHash)
			err = errors.New("ErrPackBalanceOfData")
			return
		}
	} else {
		//如果tokenAddr为空，则把toAddr设置为用户指定的地址
		toAddr = omnilinkMsg.EthereumReceiver
	...
	if err != nil {
		relayerLog.Error("handleLogWithdraw", "Failed to checkBalanceEnough:", err.Error(), "omnilinkTxhash", omnilinkTxHash)
		err = errors.New("ErrBalanceNotEnough")
		ethRelayer.remindBalanceNotEnough(ethRelayer.ethSender.String(), omnilinkMsg.Symbol, omnilinkTxHash)
		return
	}
	...
	if err != nil {
		relayerLog.Error("handleLogWithdraw", "NewTransferSignTx err", err, "omnilinkTxhash", omnilinkTxHash)
		return
	}
	//交易发送
	...
	if err != nil {
		...
		if err.Error() == core.ErrNonceTooLow.Error() || err.Error() == core.ErrNonceTooHigh.Error() {
			relayerLog.Error("handleLogWithdraw", "sendEthereumTx err", err, "出现 nonce 错误重新构建并发送交易, omnilinkTxhash", omnilinkTxHash)
	...
	relayerLog.Info("handleLogWithdraw", "SendTransaction Hash", ethTxhash, "omnilinkTxhash", omnilinkTxHash)
...
func (ethRelayer *Relayer4Ethereum) checkIsResendOmnilinkMsg(omnilinkMsg *events.OmnilinkMsg) bool {
	if omnilinkMsg.ForwardTimes <= 1 {
		return false
	}
	omnilinkTxHash := omnilinkCommon.ToHex(omnilinkMsg.TxHash)
	relayerLog.Info("checkIsResendOmnilinkMsg", "Received the same omnilinkMsg more than once with times", omnilinkMsg.ForwardTimes, "tx hash string", omnilinkTxHash)
	relayTxDetail, _ := ethRelayer.getOmnilinkTxRelayAlreadyInfo(ethRelayer.name, omnilinkTxHash)
	if nil == relayTxDetail {
		relayerLog.Info("checkIsResendOmnilinkMsg::haven't relay yet")
		return false
	}

	ethRelayer.txRelayAckSendChan <- &ebTypes.TxRelayAck{
		TxHash:  omnilinkTxHash,
		FdIndex: omnilinkMsg.ForwardIndex,
	}
	//relayomnilinkToEthereumCheckPonit 2: send ack
	relayerLog.Info("checkIsResendOmnilinkMsg::relayomnilinkToEthereumCheckPonit_2::sendBackAck", "omnilinkTxHash", omnilinkTxHash, "ForwardIndex", omnilinkMsg.ForwardIndex, "FdTimes", omnilinkMsg.ForwardTimes)

	relayerLog.Info("checkIsResendOmnilinkMsg", "have relay already with tx hash:", relayTxDetail.Txhash)
	return true
}

func (ethRelayer *Relayer4Ethereum) handleLogLockBurn(omnilinkMsg *events.OmnilinkMsg) {
	...
	omnilinkTxHash := omnilinkCommon.ToHex(omnilinkMsg.TxHash)
	relayerLog.Info("handleLogLockBurn", "Received omnilinkMsg", omnilinkMsg, "tx hash string", omnilinkTxHash)
	if ethRelayer.checkIsResendOmnilinkMsg(omnilinkMsg) {
		return
	}

	// Parse the OmnilinkMsg into a ProphecyClaim for relay to Ethereum
	prophecyClaim := ethtxs.OmnilinkMsgToProphecyClaim(*omnilinkMsg)
	var tokenAddr common.Address
	exist := false
	operationType := omnilinkMsg.ClaimType.String()
	if omnilinkMsg.ClaimType == events.ClaimTypeLock {
		...
		burnFromOmnilinkTokenInfo, exist := ethRelayer.symbol2LockAddr[prophecyClaim.Symbol]
		if !exist {
		...
		tokenAddr = common.HexToAddress(burnFromOmnilinkTokenInfo.Address)
		//从omnilink进行withdraw回来的token需要根据精度进行相应的缩放
		if 8 != burnFromOmnilinkTokenInfo.Decimal {
			if burnFromOmnilinkTokenInfo.Decimal > 8 {
				dist := burnFromOmnilinkTokenInfo.Decimal - 8
				value, exist := utils.Decimal2value[int(dist)]
				if !exist {
					panic(fmt.Sprintf("does support for decimal, %d", burnFromOmnilinkTokenInfo.Decimal))
				}
				prophecyClaim.Amount.Mul(prophecyClaim.Amount, big.NewInt(value))
			} else {
				dist := 8 - burnFromOmnilinkTokenInfo.Decimal
				value, exist := utils.Decimal2value[int(dist)]
				if !exist {
					panic(fmt.Sprintf("does support for decimal, %d", burnFromOmnilinkTokenInfo.Decimal))
				}
	...
	if ethRelayer.delayedSend {
		claimID := crypto.Keccak256Hash(prophecyClaim.OmnilinkTxHash, prophecyClaim.OmnilinkSender, prophecyClaim.EthereumReceiver.Bytes(), []byte(prophecyClaim.Symbol), prophecyClaim.Amount.Bytes())
		...
		if nil != err {
			...
			if prophecyProcessed.Valid {
				...
				relayerLog.Info("handleLogLockBurn", "prophecyProcessed Valid with tx hash", omnilinkTxHash)
			}
		}
	}
	...
	//relayomnilinkToEthereumCheckPonit 2: send ack to omnilink relay service
	relayerLog.Info("handleLogLockBurn::relayomnilinkToEthereumCheckPonit_2::sendBackAck", "omnilinkTxHash", omnilinkTxHash, "ForwardIndex", omnilinkMsg.ForwardIndex, "FdTimes", omnilinkMsg.ForwardTimes)

	//保存交易hash，方便查询
	txIndex := atomic.AddInt64(&ethRelayer.totalTxRelayFromOmnilink, 1)
	if err = ethRelayer.updateTotalTxAmountFromomnilink(txIndex); nil != err {
		relayerLog.Error("handleLogLockBurn", "Failed to RelayLockToOmnilink due to:", err.Error())
		return
	}
	statics := &ebTypes.OmnilinkToEthereumStatics{
		EthTxstatus:      ebTypes.Tx_Status_Pending,
		OmnilinkTxhash:   omnilinkTxHash,
		EthereumTxhash:   ethTxhash,
		BurnLockWithdraw: int32(omnilinkMsg.ClaimType),
		OmnilinkSender:   omnilinkMsg.OmnilinkSender.String(),
		EthereumReceiver: omnilinkMsg.EthereumReceiver.String(),
		Symbol:           omnilinkMsg.Symbol,
		Amount:           omnilinkMsg.Amount.String(),
		Nonce:            omnilinkMsg.Nonce,
		TxIndex:          txIndex,
		OperationType:    operationType,
	}
	data := omnilinkTypes.Encode(statics)
	if err = ethRelayer.setLastestStatics(int32(omnilinkMsg.ClaimType), txIndex, data); nil != err {
		relayerLog.Error("handleLogLockBurn", "Failed to RelayLockToOmnilink due to:", err.Error())
		return
	}

	relayTxDetail := &ebTypes.RelayTxDetail{
		ClaimType:      int32(omnilinkMsg.ClaimType),
		TxIndexRelayed: txIndex,
		Txhash:         ethTxhash,
	}

	if err = ethRelayer.setOmnilinkTxRelayAlreadyInfo(ethRelayer.name, omnilinkTxHash, relayTxDetail); nil != err {
		relayerLog.Error("handleLogLockBurn", "Failed to setEthTxRelayAlreadyInfo due to:", err.Error())
		return
	}
	//relayomnilinkToEthereumCheckPonit 3: set flag to send relay tx to ethereum node
	relayerLog.Info("handleLogLockBurn::relayomnilinkToEthereumCheckPonit_3::setRelayFinishFlag", "omnilinkTxHash", omnilinkTxHash, "ForwardIndex", omnilinkMsg.ForwardIndex, "FdTimes", omnilinkMsg.ForwardTimes)

	relayerLog.Info("RelayOracleClaimToEthereum::successful",
		"txIndex", txIndex,
		"OmnilinkTxhash", statics.OmnilinkTxhash,
		"EthereumTxhash", statics.EthereumTxhash,
		"type", operationType,
		"Symbol", omnilinkMsg.Symbol,
		"Amount", omnilinkMsg.Amount,
		"EthereumReceiver", statics.EthereumReceiver,
		"OmnilinkSender", statics.OmnilinkSender)
}
...
func (ethRelayer *Relayer4Ethereum) ResendLockEvent(height uint64, index uint32) (string, error) {
	...
	if 0 == len(logs) {
		...
		return "No event need to be relayed to omnilink", nil
	}
	vLog := *logs[0]
...
func (ethRelayer *Relayer4Ethereum) checkTxRelay2Omnilink() {
	txInfos, err := ethRelayer.getAllTxsUnconfirm()
	if err != nil {
		relayerLog.Error("ethRelayer::checkTxRelay2Omnilink", "Failed to getAllTxsUnconfirm due to", err.Error())
		return
	}
	if 0 == len(txInfos) {
		return
	...
	for _, txInfo := range txInfos {
		...
		if !txInfo.Resend {
			...
			if nil != err {
				relayerLog.Error("ethRelayer::checkTxRelay2Omnilink", "Failed to SetTxIsRelayedconfirm due to", err.Error())
				return
			}
			continue
		}
		...
		if nil != err {
			relayerLog.Error("ethRelayer::checkTxRelay2Omnilink", "Failed to UnpackLogLock due to", err.Error())
			ethRelayer.rwLock.RUnlock()
			return
		}
		ethRelayer.rwLock.RUnlock()
		...
		if nil != err {
			relayerLog.Error("ethRelayer::checkTxRelay2Omnilink", "Failed to GetLockedTokenAddress due to", err.Error(),
		...
		if err != nil {
			ethRelayer.rwLock.RUnlock()
			relayerLog.Error("ethRelayer::checkTxRelay2Omnilink", "Failed to LogLockToEthBridgeClaim due to", err.Error())
			return
		}
		ethRelayer.rwLock.RUnlock()
		...
		//relayEthereum2omnilinkCheckPonit 5:resendClaim
		relayerLog.Info("checkTxRelay2Omnilink::relayEthereum2omnilinkCheckPonit_5::resendClaim", "ethTxhash", txInfo.TxHash, "ForwardIndex", txInfo.FdIndex, "FdTimes", txInfo.FdTimes)
		err = ethRelayer.setTxIsRelayedUnconfirm(ethRelayer.name, txHashStr, txInfo.FdIndex, txInfo)
		if nil != err {
			relayerLog.Error("ethRelayer::checkTxRelay2Omnilink", "Failed to setTxIsRelayedconfirm due to", err.Error())
			return
...
func (ethRelayer *Relayer4Ethereum) procNewHeight() {
	...
	ethRelayer.checkTxRelay2Omnilink()
...
func (ethRelayer *Relayer4Ethereum) storeBridgeBankLogs(vLog types.Log, setBlockNumber bool) {
	//lock,用于捕捉 (ETH/ERC20----->omnilink) 跨链转移
	//burn,用于捕捉 (omnilink token----->omnilink) 实现omnilink资产withdraw操作，之后在omnilink上实现unlock操作
...
// handleLogLockEvent : unpacks a LogLock event, converts it to a ProphecyClaim, and relays a tx to omnilink
func (ethRelayer *Relayer4Ethereum) handleLogLockEvent(clientChainID *big.Int, contractABI abi.ABI, eventName string, log types.Log) error {
	...
	fdIndex := int64(0)
	if !ethRelayer.processWithDraw {
		fdIndex = ethRelayer.getFdTx2OmnilinkTotalAmount() + 1
	...
	//relayEthereum2omnilinkCheckPonit 1:send prophecyClaim to omnilink relay service
	relayerLog.Info("handleLogLockEvent::relayEthereum2omnilinkCheckPonit_1::sendClaim2Omnilink", "ethTxhash", ethTxhash, "ForwardIndex", prophecyClaim.ForwardIndex, "FdTimes", prophecyClaim.ForwardTimes)
	_ = ethRelayer.updateFdTx2EthTotalAmount(fdIndex)
	txRelayConfirm4Omnilink := &ebTypes.TxRelayConfirm4Ethereum{
	...
	return ethRelayer.setTxIsRelayedUnconfirm(ethRelayer.name, ethTxhash, fdIndex, txRelayConfirm4Omnilink)
}
...
// handleLogBurnEvent : unpacks a burn event, converts it to a ProphecyClaim, and relays a tx to omnilink
...
func (ethRelayer *Relayer4Ethereum) ShowStatics(request *ebTypes.TokenStaticsRequest) (*ebTypes.TokenStaticsResponse, error) {
	...
	for _, data := range datas {
		var statics ebTypes.OmnilinkToEthereumStatics

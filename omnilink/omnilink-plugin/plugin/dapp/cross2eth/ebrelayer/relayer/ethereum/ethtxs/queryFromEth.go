...
import (
	...
	bep20 "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/cross2eth/contracts/bep20/generated"
...
//IsProphecyPending ...
func IsProphecyPending(claimID [32]byte, validator common.Address, omnilinkBridge *generated.OmnilinkBridge) (bool, error) {
	opts := &bind.CallOpts{
		Pending: true,
		From:    validator,
		Context: context.Background(),
	...
	// Initialize BridgeRegistry instance
	active, err := omnilinkBridge.IsProphecyClaimActive(opts, claimID)
	if err != nil {
		txslog.Error("IsActiveValidatorFromOmnilinkBridge", "Failed to query IsActiveValidator due to:", err.Error())
		return false, err
	}
	return active, nil
}

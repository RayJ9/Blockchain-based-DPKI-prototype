...
func deployAndInit2Omnilink(rpcLaddr, paraChainName string, para4deploy *DeployPara4Omnilink) (*X2EthDeployResult, error) {
	...
	if nil != err {
		omnilinktxLog.Error("DeployAndInit", "failed to DeployValset due to:", err.Error())
		return nil, err
	}
	{
		...
		for {
			...
			case <-oneSecondtimeout.C:
				...
				if data == "" {
					omnilinktxLog.Info("deployAndInit2Omnilink", "No receipt received yet for Deploy valset tx and continue to wait", "continue")
					continue
				} else if data != "2" {
				...
				omnilinktxLog.Info("deployAndInit2Omnilink", "Succeed to deploy valset with address =", valsetAddr)
...
deployEthereumBridge:
	//x2EthContracts.OmnilinkBridge, deployInfo.OmnilinkBridge, err = DeployOmnilinkBridge(client, para.DeployPrivateKey, para.Deployer, para.Operator, deployInfo.Valset.Address)
	//constructor(
	//	address _operator,
	//	address _valset
	//)
	...
	if nil != err {
		omnilinktxLog.Error("DeployAndInit", "failed to deployEthereumBridge due to:", err.Error())
		return nil, err
	}
	{
		timeout := time.NewTimer(300 * time.Second)
		...
		for {
			...
			case <-oneSecondtimeout.C:
				...
				if data == "" {
					omnilinktxLog.Info("deployAndInit2Omnilink", "No receipt received yet for Deploy EthereumBridge tx and continue to wait", "continue")
					continue
				} else if data != "2" {
				...
				omnilinktxLog.Info("deployAndInit2Omnilink", "Succeed to deploy EthereumBridge with address =", ethereumBridgeAddr)
				goto deployOracle
			}
		}
	}
...
deployOracle:
	...
	//x2EthContracts.Oracle, deployInfo.Oracle, err = DeployOracle(client, para.DeployPrivateKey, para.Deployer, para.Operator, deployInfo.Valset.Address, deployInfo.OmnilinkBridge.Address)
	...
	if nil != err {
		omnilinktxLog.Error("DeployAndInit", "failed to DeployOracle due to:", err.Error())
		return nil, err
	}
	{
		timeout := time.NewTimer(300 * time.Second)
		...
		for {
			...
			case <-oneSecondtimeout.C:
				...
				if data == "" {
					omnilinktxLog.Info("deployAndInit2Omnilink", "No receipt received yet for Deploy Oracle tx and continue to wait", "continue")
					continue
				} else if data != "2" {
				...
				omnilinktxLog.Info("deployAndInit2Omnilink", "Succeed to deploy Oracle with address =", oracleAddr)
...
deployBridgeBank:
	...
	if nil != err {
		omnilinktxLog.Error("DeployAndInit", "failed to DeployBridgeBank due to:", err.Error())
		return nil, err
	}
	{
		timeout := time.NewTimer(300 * time.Second)
		...
		for {
			...
			case <-oneSecondtimeout.C:
				...
				if data == "" {
					omnilinktxLog.Info("deployAndInit2Omnilink", "No receipt received yet for Deploy BridgeBank tx and continue to wait", "continue")
					continue
				} else if data != "2" {
				...
				omnilinktxLog.Info("deployAndInit2Omnilink", "Succeed to deploy BridgeBank with address =", bridgeBankAddr)
...
settingBridgeBank:
	...
	if nil != err {
		omnilinktxLog.Info("setBridgeBank", "Failed to do abi.Pack due to:", err.Error())
	...
	if nil != err {
		omnilinktxLog.Error("DeployAndInit", "failed to settingBridgeBank due to:", err.Error())
		return nil, err
	}
	{
		...
		for {
			...
			case <-oneSecondtimeout.C:
				...
				if data == "" {
					omnilinktxLog.Info("deployAndInit2Omnilink", "No receipt received yet for for setBridgeBank tx and continue to wait", "continue")
					continue
				} else if data != "2" {
					return nil, errors.New("setBridgeBank failed due to" + ", ty = " + data)
				}
				omnilinktxLog.Info("deployAndInit2Omnilink", "Succeed to setBridgeBank ", "Oh yea!!!")
				goto setOracle
			}
		}
	}
...
setOracle:
	...
	if nil != err {
		omnilinktxLog.Error("DeployAndInit", "failed to setOracle due to:", err.Error())
		return nil, err
	}
	{
		...
		for {
			...
			case <-oneSecondtimeout.C:
				...
				if data == "" {
					omnilinktxLog.Info("deployAndInit2Omnilink", "No receipt received yet for for setOracle tx and continue to wait", "continue")
					continue
				} else if data != "2" {
					return nil, errors.New("setOracle failed due to" + ", ty = " + data)
				}
				omnilinktxLog.Info("deployAndInit2Omnilink", "Succeed to setOracle ", "Oh yea!!!")
...
deployBridgeRegistry:
	...
	if nil != err {
		omnilinktxLog.Error("DeployAndInit", "failed to deployBridgeRegistry due to:", err.Error())
		return nil, err
	}
	{
		timeout := time.NewTimer(300 * time.Second)
		...
		for {
			...
			case <-oneSecondtimeout.C:
				...
				if data == "" {
					omnilinktxLog.Info("deployAndInit2Omnilink", "No receipt received yet for for deploy BridgeRegistry tx and continue to wait", "continue")
					continue
				} else if data != "2" {
				...
				omnilinktxLog.Info("deployAndInit2Omnilink", "Succeed to deployBridgeRegistry with address", deployBridgeRegistry.Address.String())
				goto finished
			}
		}
	}
...
func deployMulSign2Omnilink(rpcLaddr, paraChainName, deployer string) (string, error) {
	...
	if nil != err {
		omnilinktxLog.Error("deployMulSign2Omnilink", "failed to deployMulSign due to:", err.Error())
		return "", err
	}
	...
	for {
		...
		case <-oneSecondtimeout.C:
			...
			if data == "" {
				omnilinktxLog.Info("deployMulSign2Omnilink", "No receipt received yet for Deploy MulSign tx and continue to wait", "continue")
				continue
			} else if data != "2" {
			...
			omnilinktxLog.Info("deployMulSign2Omnilink", "Succeed to deploy MulSign with address =", mulSignAddr)
			return mulSignAddr, nil
		}
	}
}

func deployERC20ToOmnilink(rpcLaddr, paraChainName, deployer string, param ebTypes.ERC20Token) (string, error) {
	...
	if nil != err {
		omnilinktxLog.Error("deployERC20ToOmnilink", "failed to deployMulSign due to:", err.Error())
		return "", err
	}
	...
	for {
		...
		case <-oneSecondtimeout.C:
			...
			if data == "" {
				omnilinktxLog.Info("deployERC20ToOmnilink", "No receipt received yet for Deploy erc20 tx and continue to wait", "continue")
				continue
			} else if data != "2" {
			...
			omnilinktxLog.Info("deployERC20ToOmnilink", "Succeed to deploy erc20 with address =", erc20Addr)

...
### 启动 relayer A
#### 完成 ethererum 和 omnilink 相关合约的部署
得到 BridgeRegistryOnOmnilink, BridgeRegistryOnEth, multisignOmnilinkAddr, multisignEthAddr 4个合约地址。
...
|omnilinkBridgeRegistry|部署在 omnilink 的 BridgeRegistry 地址|
|ChainID4Omnilink|omnilink 链的 ID, 默认为 0|
|ChainName|链的名称, 用来区分主链和平行链, 如user.p.xxx., 必须包含最后一个点|
|omnilinkHost|平行链的 host 地址, 默认: http://localhost:8801|
...
# 设置 omnilink 验证私钥
./ebcli_A omnilink import_privatekey -k "${omnilinkValidatorKeya}"
# 设置 ethereum 验证私钥
./ebcli_A ethereum import_privatekey -k "${ethValidatorAddrKeya}"

# 设置 omnilink 多签合约地址
./ebcli_A omnilink multisign set_multiSign -a "${omnilinkMultisignAddr}"
# 设置 ethereum 多签合约地址
...
|----|----|
|pushName|4 个 relayer 不同相同, `sed -i 's/^pushName=.*/pushName="XXX"/g' relayer.toml`|
|omnilinkHost|平行链的 host 地址, 默认: http://localhost:8801, 4 个 relayer 对应 4 个不同 omnilink 平行链地址|
...
|omnilinkHost|平行链的 host 地址, 默认: http://localhost:8801, 选任意一个 omnilink 平行链地址就可以|
|pushHost|relayer 的 host 地址, 默认: http://localhost:20000, omnilinkHost 和本地IP 不同时要换成本地 IP|
|RemindUrl|代理打币地址不够时, 提醒打币发送短信的URL|

#### 首次启动 relayer 进行设置
同上...

#### 设置 omnilink 代理地址, 及手续费设置
```shell
...
# 设置 omnilink 代理地址
./boss4x omnilink offline set_withdraw_proxy -c "${omnilinkBridgeBank}" -a "${omnilinkValidatorsp}" -k "${omnilinkDeployKey}" -n "set_withdraw_proxy:${omnilinkValidatorsp}"
Flags:

...
#### 部署平行链 (数据来源 HZM)
得到 omnilink rcp url, eg: http://35.77.111.58:8901
...
|validatorsAddr[]|普通验证人地址, 3个以上, 需要少量金额, 用于用户从omnilink中提币时手续费, 需要监测, 地址金额不能为空否则提币失败, BNB 建议 0.1 个, 根据需求增加或减少|
...
#### omnilink 部署配置文件
把要部署需要的数据写入 omnilink_ethereum.toml 配置文件
```toml
# 验证人地址, 至少配置3个以上, 即大于等于3个
...
#### omnilink 端所需地址及说明 (数据来源管理员)
|地址|说明|
|----|----|
|operatorAddr|合约部署人, 需要比较多的金额, 用于部署合约时需要的手续费|
|validatorsAddr[]|普通验证人地址, 3个以上, 需要少量金额, 用于用户从omnilink中提币时手续费, 需要监测, 地址金额不能为空否则提币失败, BTY 建议 20 个, 根据需求增加或减少|
...
###  离线部署 omnilink 跨链合约及各操作
#### 基础步骤
* 离线创建交易并签名 `./boss4x omnilink offline create ...`
* 在线发送签名后文件 `./boss4x omnilink offline send -f XXX.txt`

拼凑 boss4x 命令
```shell
./boss4x --rpc_laddr http://${omnilink_ip}:8901 --rpc_laddr_ethereum --paraName user.p.para. --chainID 0
...
```
交易1: 部署合约: Valset
交易2: 部署合约: omnilinkBridge
交易3: 部署合约: Oracle
交易4: 部署合约: BridgeBank
交易5: 在合约omnilinkBridge中设置BridgeBank合约地址
交易6: 在合约omnilinkBridge中设置Oracle合约地址
...
命令:
./boss4x omnilink offline create_file -f 1 -k "${omnilinkDeployKey}" -n "deploy crossx to omnilink" -c "./deploy_omnilink.toml"

执行之后会将交易写入到文件：
  deployCrossX2Omnilink.txt
```
...
```
命令：
./boss4x omnilink offline create_erc20 -s YCC -k ... -o 1N6HstkyLFS8QCeVfdvYxx1xoryXoJtvvZ
...
执行之后会将交易写入到文件：
  deployErc20XXXOmnilink.txt 其中 XXX 为 token 标识
```
...
```
命令：
./boss4x omnilink offline approve_erc20 -a 330000000000 -s 1JmWVu1GEdQYSN1opxS9C39aS4NvG57yTr -c 1998HqVnt4JUirhC9KL5V71xYU8cFRn82c -k ... 

参数说明：  
  -a, --amount float      审批金额
  -s, --approve string    审批地址, omnilink BridgeBank 合约地址
...
```
命令：
./boss4x omnilink offline create_add_lock_list -c 1JmWVu1GEdQYSN1opxS9C39aS4NvG57yTr -k ... -t 1998HqVnt4JUirhC9KL5V71xYU8cFRn82c -s YCC
...
```
命令：
./boss4x omnilink offline create_bridge_token -c 1JmWVu1GEdQYSN1opxS9C39aS4NvG57yTr -k ... -s YCC
...
```
命令：
./omnilink-cli evm query -a 1JmWVu1GEdQYSN1opxS9C39aS4NvG57yTr -c 1N6HstkyLFS8QCeVfdvYxx1xoryXoJtvvZ -b 'getToken2address(YCC)'

参数说明：
  -a, --address string   evm 合约地址,这里是 omnilink BridgeBank 合约地址
...
```
命令：
./boss4x omnilink offline set_offline_token -c 1MaP3rrwiLV1wrxPhDwAfHggtei1ByaKrP -s BTY -m 100000000000 -p 50 -k ...
...
执行之后会将交易写入到文件：
  omnilink_set_offline_token.txt
```
...
```
命令：
./boss4x omnilink offline create_multisign_transfer -a 10 -r 168Sn1DXnLrZHTcAM9stD6t2P49fNuJfJ9 -m 1NFDfEwne4kjuxAZrtYEh4kfSrnGSE7ap
...
```
命令：
./evmxgoboss4x omnilink offline create_add_lock_list -s ETH -t "${omnilinkEthBridgeTokenAddr}" -c "${XgoOmnilinkBridgeBank}" -k "${omnilinkDeployKey}"

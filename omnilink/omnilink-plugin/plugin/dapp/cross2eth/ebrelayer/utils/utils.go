...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/cross2eth/contracts/erc20/generated"
	...
	omnilinkAbi "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/evm/executor/abi"
...
func QueryResult(param, abiData, contract, owner string, client ethinterface.EthClientSpec) (string, error) {
	...
	methodName, params, err := omnilinkAbi.ProcFuncCall(param)
	if err != nil {
		return methodName + " ProcFuncCall fail", err
	}

	// 解析ABI数据结构，获取本次调用的方法对象
	abi_, err := omnilinkAbi.JSON(strings.NewReader(abiData))
	if err != nil {
		log.Error("QueryResult", "JSON fail", err)
		return methodName + " JSON fail", err
	}

	var method omnilinkAbi.Method
	var ok bool
	...
	if len(params) != 0 {
		...
		for i, v := range method.Inputs.NonIndexed() {
			paramVal, err := omnilinkAbi.Str2GoValue(v.Type, params[i])
			if err != nil {

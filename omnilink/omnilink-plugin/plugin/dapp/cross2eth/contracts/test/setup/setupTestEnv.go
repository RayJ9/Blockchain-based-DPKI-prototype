...
func DeployOmnilinkBridge(client ethinterface.EthClientSpec, privateKey *ecdsa.PrivateKey, deployer common.Address, operator, valset common.Address) (*generated.OmnilinkBridge, *ethtxs.DeployResult, error) {
	auth, err := ethtxs.PrepareAuth(client, privateKey, deployer)
	if nil != err {
		return nil, nil, err
	}
...
func DeployOracle(client ethinterface.EthClientSpec, privateKey *ecdsa.PrivateKey, deployer, operator, valset, omnilinkBridge common.Address) (*generated.Oracle, *ethtxs.DeployResult, error) {
	auth, err := ethtxs.PrepareAuth(client, privateKey, deployer)
	if nil != err {
		return nil, nil, err
	}
...
func DeployBridgeBank(client ethinterface.EthClientSpec, privateKey *ecdsa.PrivateKey, deployer, operator, oracle, omnilinkBridge common.Address) (*generated.BridgeBank, *ethtxs.DeployResult, error) {
	auth, err := ethtxs.PrepareAuth(client, privateKey, deployer)
	if nil != err {
		return nil, nil, err
	}
...
func DeployBridgeRegistry(client ethinterface.EthClientSpec, privateKey *ecdsa.PrivateKey, deployer, omnilinkBridgeAddr, bridgeBankAddr, oracleAddr, valsetAddr common.Address) (*generated.BridgeRegistry, *ethtxs.DeployResult, error) {
	auth, err := ethtxs.PrepareAuth(client, privateKey, deployer)
	if nil != err {
		return nil, nil, err
	}
...
func DeployAndInit(client ethinterface.EthClientSpec, para *ethtxs.DeployPara) (*ethtxs.X2EthContracts, *ethtxs.X2EthDeployInfo, error) {
	...
	if nil != err {
		fmt.Println("DeployAndInit", "failed to DeployOmnilinkBridge due to:", err.Error())
		return nil, nil, err
	}
	if isSim {
		sim.Commit()

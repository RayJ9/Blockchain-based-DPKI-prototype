...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/dex/contracts/pancake-farm/src/cakeToken"
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/dex/contracts/pancake-farm/src/masterChef"
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/dex/contracts/pancake-farm/src/syrupBar"
...
func createCakeToken(cmd *cobra.Command, from common.Address) (*utils.OmnilinkOfflineTx, error) {
	...
	cakeTokenTx := &utils.OmnilinkOfflineTx{
...
func createSyrupBar(cmd *cobra.Command, from common.Address, cakeToken string) (*utils.OmnilinkOfflineTx, error) {
	privateKey, _ := cmd.Flags().GetString("key")
	expire, _ := cmd.Flags().GetString("expire")
	note, _ := cmd.Flags().GetString("note")
	fee, _ := cmd.Flags().GetFloat64("fee")
...
func TransferOwnerShip(cmd *cobra.Command, from common.Address, masterChef, contractAddr, operationName string) (*utils.OmnilinkOfflineTx, error) {
	privateKey, _ := cmd.Flags().GetString("key")
	expire, _ := cmd.Flags().GetString("expire")
	note, _ := cmd.Flags().GetString("note")
	fee, _ := cmd.Flags().GetFloat64("fee")
	...
	newContractAddr := common.NewContractAddress(from, txHash).String()
	transferOwnerShipTx := &utils.OmnilinkOfflineTx{
		ContractAddr:  newContractAddr,
		TxHash:        common.Bytes2Hex(txHash),
		SignedRawTx:   content,
...
func createMasterChef(cmd *cobra.Command, args []string) {
	...
	newContractAddr := common.NewContractAddress(from, txHash).String()
	masterChefTx := &utils.OmnilinkOfflineTx{
		ContractAddr:  newContractAddr,
		TxHash:        common.Bytes2Hex(txHash),
		SignedRawTx:   content,
...
func addPool(cmd *cobra.Command, args []string) {
	...
	newContractAddr := common.NewContractAddress(from, txHash).String()
	addPoolTx := &utils.OmnilinkOfflineTx{
		ContractAddr:  newContractAddr,
		TxHash:        common.Bytes2Hex(txHash),
		SignedRawTx:   content,

...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/dex/contracts/pancake-swap-periphery/src/pancakeFactory"
	"code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/dex/contracts/pancake-swap-periphery/src/pancakeRouter"
...
func createERC20Contract(cmd *cobra.Command, args []string) {
	...
	routerTx := &utils.OmnilinkOfflineTx{
...
func createWeth9(cmd *cobra.Command, from common.Address) (*utils.OmnilinkOfflineTx, error) {
	privateKeyStr, _ := cmd.Flags().GetString("key")
	expire, _ := cmd.Flags().GetString("expire")
	note, _ := cmd.Flags().GetString("note")
	fee, _ := cmd.Flags().GetFloat64("fee")
	...
	newContractAddr := common.NewContractAddress(from, txHash).String()
	weth9Tx := &utils.OmnilinkOfflineTx{
		ContractAddr:  newContractAddr,
		TxHash:        common.Bytes2Hex(txHash),
		SignedRawTx:   content,
...
func createFactoryContract(cmd *cobra.Command, from common.Address) (*utils.OmnilinkOfflineTx, error) {
	...
	newContractAddr := common.NewContractAddress(from, txHash).String()
	factoryTx := &utils.OmnilinkOfflineTx{
		ContractAddr:  newContractAddr,
		TxHash:        common.Bytes2Hex(txHash),
		SignedRawTx:   content,

...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-base/system/consensus/solo"
	...
	gty "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/guess/types"
	pty "code.corp.bcollie.net/omnilink/omnilink-plugin/plugin/dapp/norm/types"
)
...
func Init() {
	...
	os.RemoveAll("wallet")
	os.Remove("omnilink.test.toml")

	ioutil.WriteFile("omnilink.test.toml", []byte(config), 0664)
}
...
func initEnvGuess() (queue.Queue, *blockchain.BlockChain, queue.Module, queue.Module, *executor.Executor, queue.Module, queue.Module) {
	flag.Parse()
	omnilinkCfg := types.NewOmnilinkConfig(types.ReadFile("omnilink.test.toml"))
	var q = queue.New("channel")
	q.SetConfig(omnilinkCfg)
	cfg := omnilinkCfg.GetModuleConfig()
	cfg.Log.LogFile = ""
	sub := omnilinkCfg.GetSubConfig()
	chain := blockchain.New(omnilinkCfg)
	chain.SetQueueClient(q.Client())

	exec := executor.New(omnilinkCfg)
	exec.SetQueueClient(q.Client())
	omnilinkCfg.SetMinFee(0)
	s := store.New(omnilinkCfg)
	s.SetQueueClient(q.Client())

	cs := solo.New(cfg.Consensus, sub.Consensus["solo"])
	cs.SetQueueClient(q.Client())

	mem := mempool.New(omnilinkCfg)
	mem.SetQueueClient(q.Client())
	network := p2p.NewP2PMgr(omnilinkCfg)
...
func createConn() error {
	...
	c = types.NewOmnilinkClient(conn)
	return nil
}
...
func prepareTxList(cfg *types.OmnilinkConfig) *types.Transaction {
	var key string
	var value string
	var i int
...
func NormPut(cfg *types.OmnilinkConfig) {
...
func sendTransferTx(cfg *types.OmnilinkConfig, fromKey, to string, amount int64) bool {
...
func sendTransferToExecTx(cfg *types.OmnilinkConfig, fromKey, execName string, amount int64) bool {
	signer := util.HexToPrivkey(fromKey)
	var tx *types.Transaction
	transfer := &cty.CoinsAction{}
...
func sendGuessStartTx(cfg *types.OmnilinkConfig, topic, option, category, privKey string) (bool, []byte) {
...
func sendGuessBetTx(cfg *types.OmnilinkConfig, gameID, option string, betsNum int64, privKey string) (bool, []byte) {
	signer := util.HexToPrivkey(privKey)
	var tx *types.Transaction
	action := &gty.GuessGameAction{}
...
func sendGuessStopTx(cfg *types.OmnilinkConfig, gameID, privKey string) (bool, []byte) {
	signer := util.HexToPrivkey(privKey)
	var tx *types.Transaction
	action := &gty.GuessGameAction{}
...
func sendGuessAbortTx(cfg *types.OmnilinkConfig, gameID, privKey string) (bool, []byte) {
	signer := util.HexToPrivkey(privKey)
	var tx *types.Transaction
	action := &gty.GuessGameAction{}
...
func sendGuessPublishTx(cfg *types.OmnilinkConfig, gameID, result, privKey string) (bool, []byte) {
	signer := util.HexToPrivkey(privKey)
	var tx *types.Transaction
	action := &gty.GuessGameAction{}
...
func queryGuessByIds(gameIDs string) *gty.ReplyGuessGameInfos {
	...
	//ctx := jsonrpc.NewRPCCtx("http://"+types.Conf("config.rpc").GStr("jrpcBindAddr"), "Omnilink.Query", params, &res)
	ctx := jsonrpc.NewRPCCtx("http://127.0.0.1:9801", "Omnilink.Query", params, &res)
	ctx.Run()
	return &res
}

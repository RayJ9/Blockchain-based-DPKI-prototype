...
### pubsub在omnilink中用途
````
pubsub基础功能集成在omnilink/system/p2p/dht/net/pubsub.go中,该功能主要包含订阅topic,向
topic 发送消息，删除主题，获取已经订阅的topic.

目前pubsub功能刷线应用在omnilink平行链模块，主要应用于在平行链内部共识期间的消息广播。
omnilink平行链使用相关功能，需要先通过queue消息处理模块 发送相关消息事件注册自己的模块名与要订阅的topic，
当有相关topic消息过来之后，pubsub就会通过queue 把接收到消息转发给注册的相应模块。
后期计划用pubsub去掉omnilink的tx广播功能。
````
#### 接口说明
...
### pubsub在omnilink中用途
````
中继功能目前还没有实际应用在omnilink中，作为探索开发，为以后omnilink有实际需求后，提供储备。
````

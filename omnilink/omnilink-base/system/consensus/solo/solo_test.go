...
import (
	...
	log "code.corp.bcollie.net/omnilink/omnilink-base/common/log"
	"code.corp.bcollie.net/omnilink/omnilink-base/queue"
	"code.corp.bcollie.net/omnilink/omnilink-base/rpc/grpcclient"
...
func sendTxGrpc(cfg *types.OmnilinkConfig, recvChan <-chan *types.Transaction, batchNum int) {
...
func sendTxDirect(mock33 *testnode.OmnilinkMock, recvChan <-chan *types.Transaction) {
...
func createCoinsTx(cfg *types.OmnilinkConfig, to string, txHeight int64) *types.Transaction {
...
func BenchmarkSolo(b *testing.B) {
	...
	cfg := types.NewOmnilinkConfig(cfgStr)
	cfg.GetModuleConfig().Exec.DisableAddrIndex = true

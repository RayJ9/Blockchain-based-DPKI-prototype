...
import (
	...
	_ "code.corp.bcollie.net/omnilink/omnilink-base/system/consensus/init"
...
func initEnv3() (queue.Queue, queue.Module, queue.Module, *Mempool) {
	cfg := types.NewOmnilinkConfig(types.ReadFile("../../cmd/omnilink/omnilink.test.toml"))
...
func initEnv(size int) (queue.Queue, *Mempool) {
	...
	cfg := types.NewOmnilinkConfig(types.MergeCfg(types.ReadFile("../../cmd/omnilink/omnilink.test.toml"), types.ReadFile("../../cmd/omnilink/omnilink.fork.toml")))
	mcfg := cfg.GetModuleConfig()
	var q = queue.New("channel")
	q.SetConfig(cfg)
	blockchainProcess(q)
...
func initEnv4(size int) (queue.Queue, *Mempool) {
	...
	cfg := types.NewOmnilinkConfig(types.MergeCfg(types.ReadFile("../../cmd/omnilink/omnilink.fork.toml"), types.ReadFile("testdata/omnilink.test.toml")))
	mcfg := cfg.GetModuleConfig()
	var q = queue.New("channel")
	q.SetConfig(cfg)
	blockchainProcess(q)
...
func createTx(cfg *types.OmnilinkConfig, priv crypto.PrivKey, to string, amount int64) *types.Transaction {
	v := &cty.CoinsAction_Transfer{Transfer: &types.AssetsTransfer{Amount: amount}}
	transfer := &cty.CoinsAction{Value: v, Ty: cty.CoinsActionTransfer}

...
func (mem *Mempool) eventProcess() {
	...
	for msg := range mem.client.Recv() {
		...
		case types.EventGetPendingSize:
			mem.eventGetPendingSize(msg)
...
// EventGetPendingSize 获取Mempool内交易数
func (mem *Mempool) eventGetPendingSize(msg *queue.Message) {
	msg.Reply(mem.client.NewMessage("rpc", types.EventReplyPendingSize,
		&types.ReplyPendingSize{PendingSize: int64(len(mem.filterTxList(0, nil, true)))}))
}

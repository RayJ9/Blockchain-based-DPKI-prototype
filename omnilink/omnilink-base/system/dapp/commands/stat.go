...
func totalCoins(cmd *cobra.Command, args []string) {
	...
	if symbol == "bty" {
		...
		err = rpc.Call("Omnilink.Query", params, &res)
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			return
		}
	...
	if actual != "" {
		...
		for count = 100; count == 100; {
			...
			err = rpc.Call("Omnilink.GetTotalCoins", &params, &res)
			if err != nil {
				fmt.Fprintln(os.Stderr, err)
				return
			}
...
func execBalance(cmd *cobra.Command, args []string) {
	...
	for {
		...
		var str string
		err = rpc.Call("Omnilink.GetExecBalance", &reqParam, &str)
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			return
		}
...
func getLastBlock(rpc *jsonclient.JSONClient) (*rpctypes.Header, error) {
	...
	res := &rpctypes.Header{}
	err := rpc.Call("Omnilink.GetLastHeader", nil, &res)
	if err != nil {
		return nil, err
	}
	return res, nil
...
func getBlockHash(height int64, rpc *jsonclient.JSONClient) (string, error) {
	...
	err := rpc.Call("Omnilink.GetBlockHash", &params, &res)
	if err != nil {
		return "", err
	}
	return res.Hash, nil
...
func queryTotalFeeWithHash(blockHash string, rpc *jsonclient.JSONClient) (*types.TotalFee, error) {
	...
	res := &types.TotalFee{}
	err = rpc.Call("Omnilink.QueryTotalFee", &params, &res)
	if err != nil {
		return nil, err
	}
	return res, nil
...
func getBlocks(start, end int64, rpc *jsonclient.JSONClient) (*rpctypes.BlockDetails, error) {
	...
	err := rpc.Call("Omnilink.GetBlocks", params, &res)
	if err != nil {
		return nil, err
	}
	return res, nil

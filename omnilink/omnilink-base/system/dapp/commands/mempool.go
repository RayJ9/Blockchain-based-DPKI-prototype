...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-base/system/dapp/commands/types"
...
func listMempoolTxs(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Omnilink.GetMempool", params, &res)
...
func GetPendingSizeCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "pending_size",
		Short: "Get tx numbers in mempool",
		Run:   pendingSize,
	}
	return cmd
}

func pendingSize(cmd *cobra.Command, args []string) {
	rpcLaddr, _ := cmd.Flags().GetString("rpc_laddr")
	var res rpctypes.ReplyPendingSize
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Omnilink.GetPendingSize", nil, &res)
	ctx.SetResultCb(nil)
	ctx.Run()
}
...
func lastMempoolTxs(cmd *cobra.Command, args []string) {
	rpcLaddr, _ := cmd.Flags().GetString("rpc_laddr")
	var res rpctypes.ReplyTxList
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Omnilink.GetLastMemPool", nil, &res)
...
func properFee(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Omnilink.GetProperFee", nil, &res)
	ctx.SetResultCb(nil)
	ctx.Run()
}

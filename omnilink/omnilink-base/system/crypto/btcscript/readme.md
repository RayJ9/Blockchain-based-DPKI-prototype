...
无法约束到omnilink交易，需要完成交易绑定，相关步骤如下：

1. 根据锁定脚本生成解锁脚本时，需要构造临时的比特币交易btctx
2. btctx只包含一个输入UTXO，且该UTXO的索引哈希为omnilink交易的哈希
3. 这样基于btctx生成的解锁脚本就和omnilink交易相互绑定，保证了omnilink交易不被篡改

#### 交易发送方绑定

锁定脚本和解锁脚本智能约束交易是否正确，
在omnilink交易中，需要在签名结构中指定公钥，即对应交易发送方的公钥，
需要通过脚本限定交易发送方的地址

1. 在比特币脚本签名验证中，签名的数据需要包含锁定脚本和解锁脚本
2. omnilink交易签名公钥设为Hash256(锁定脚本)，即交易的发送方地址由锁定脚本生成
3. 类似于比特币P2SH，采用比特币脚本验证方式，交易的发起地址都是基于锁定脚本哈希计算
4. 在验证时，即需要验证公钥是否和锁定脚本一一对应，即绑定了omnilink交易的发送方地址
...
设有地址A，B，C，在omnilink中完成3:2多重签名大概步骤如下

1. 构造锁定脚本S1， 2 PubkeyA PubkeyB PubkeyC 3 CHECKMULTISIG
2. 生成omnilink多重签名地址X = Pubkey2Addr(Hash256(S1))
3. 在omnilink中，在地址X中存入需要多重签名的资产
...
3. CSV在比特币中需要基于一个开始时间点，在集成到omnilink中，使用1中构造的none延时交易执行时间点作为CSV的开始时间，并进行相对时长验证

...
func Test_ExampleMultiSig(t *testing.T) {
	...
	addr1, err := btcutil.NewAddressPubKey(pk1.SerializeCompressed(), script.OmnilinkBtcParams)
	require.Nil(t, err)

	addr2, err := btcutil.NewAddressPubKey(pk2.SerializeCompressed(), script.OmnilinkBtcParams)
	require.Nil(t, err)

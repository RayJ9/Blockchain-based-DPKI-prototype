...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-base/system/crypto/secp256k1eth/types"
...
func (pubKey PubKeySecp256k1Eth) VerifyBytes(msg []byte, sig crypto.Signature) bool {
	...
	if err == nil && len(action.Note) == 0 || err != nil {
		...
		if amount.Uint64() != action.Amount { //防止自组装的omnilink Tx amount 被篡改

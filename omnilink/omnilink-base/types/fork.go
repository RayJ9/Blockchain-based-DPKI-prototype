...
// IsFork 是否系统 fork高度
func (c *OmnilinkConfig) IsFork(height int64, fork string) bool {
	return c.forks.IsFork(height, fork)
}

// IsDappFork 是否dapp fork高度
func (c *OmnilinkConfig) IsDappFork(height int64, dapp, fork string) bool {
	return c.forks.IsDappFork(height, dapp, fork)
}

// GetDappFork 获取dapp fork高度
func (c *OmnilinkConfig) GetDappFork(dapp, fork string) int64 {
	return c.forks.GetDappFork(dapp, fork)
}

// SetDappFork 设置dapp fork高度
func (c *OmnilinkConfig) SetDappFork(dapp, fork string, height int64) {
...
func (c *OmnilinkConfig) RegisterDappFork(dapp, fork string, height int64) {
	if c.needSetForkZero() {
		height = 0
		if fork == "ForkBlockHash" {
			height = 1
...
// GetFork 获取系统fork高度
func (c *OmnilinkConfig) GetFork(fork string) int64 {
	return c.forks.GetFork(fork)
}

// HasFork 是否有系统fork
func (c *OmnilinkConfig) HasFork(fork string) bool {
	return c.forks.HasFork(fork)
}

// IsEnableFork 是否使能了fork
func (c *OmnilinkConfig) IsEnableFork(height int64, fork string, enable bool) bool {
	if !enable {
		return false
	}
...
func (c *OmnilinkConfig) initForkConfig(forks *ForkList) {
	omnilinkfork := c.forks.GetAll()
	if omnilinkfork == nil {
		panic("omnilink fork not init")
	}
	//开始判断omnilinkfork中的system部分是否已经设置
	s := ""
	for k := range omnilinkfork {

...
func CallExecNewTx(c *OmnilinkConfig, execName, action string, param interface{}) ([]byte, error) {
...
// CallCreateTx 构造交易信息
func CallCreateTx(c *OmnilinkConfig, execName, action string, param Message) ([]byte, error) {
...
func CallCreateTxJSON(c *OmnilinkConfig, execName, action string, param json.RawMessage) ([]byte, error) {
	exec := LoadExecutorType(execName)
	if exec == nil {
...
func CreateFormatTx(c *OmnilinkConfig, execName string, payload []byte) (*Transaction, error) {
...
// FormatTx 格式化tx交易
func FormatTx(c *OmnilinkConfig, execName string, tx *Transaction) (*Transaction, error) {
	//填写nonce,execer,to, fee 等信息, 后面会增加一个修改transaction的函数，会加上execer fee 等的修改
	tx.Nonce = rand.Int63()
...
func FormatTxEncode(c *OmnilinkConfig, execName string, tx *Transaction) ([]byte, error) {
...
type ExecutorType interface {
	...
	// about OmnilinkConfig
	GetConfig() *OmnilinkConfig
	SetConfig(cfg *OmnilinkConfig)
}
...
//GetConfig ...
func (base *ExecTypeBase) GetConfig() *OmnilinkConfig {
	return base.cfg
}

//SetConfig ...
func (base *ExecTypeBase) SetConfig(cfg *OmnilinkConfig) {
	base.cfg = cfg
}

...
func TestConfigMverInit(t *testing.T) {
	cfg := NewOmnilinkConfigNoInit(ReadFile("testdata/local.mvertest.toml"))
	cfg.RegisterDappFork("store-kvmvccmavl", "ForkKvmvccmavl", MaxHeight)
	cfg.DisableCheckFork(true)
	cfg.omnilinkCfgInit(cfg.GetModuleConfig())
...
func getP(cfg *OmnilinkConfig, height int64) *ChainParam {
	initChainBase()
	initChainBityuanV3()
...
func TestInitChainParam(t *testing.T) {
	cfg := NewOmnilinkConfig(MergeCfg(ReadFile("../cmd/omnilink/omnilink.toml"), ReadFile("../cmd/omnilink/omnilink.fork.toml")))

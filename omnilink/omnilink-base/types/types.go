...
// Package types 实现了omnilink基础结构体、接口、常量等的定义
package types
...
import (
	...
	// 注册默认的btc地址模式
	_ "code.corp.bcollie.net/omnilink/omnilink-base/system/address/btc"
	"code.corp.bcollie.net/omnilink/omnilink-base/types/jsonpb"
...
//ExecName  执行器name
func (c *OmnilinkConfig) ExecName(name string) string {
...
//GetParaExec  获取平行链执行
func (c *OmnilinkConfig) GetParaExec(execer []byte) []byte {
	//必须是平行链
	if !c.IsPara() {
		return execer
	}

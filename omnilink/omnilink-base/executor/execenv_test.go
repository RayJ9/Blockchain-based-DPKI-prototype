...
import (
	...
	erpctypes "code.corp.bcollie.net/omnilink/omnilink-base/rpc/ethrpc/types"
...
func TestLoadDriverFork(t *testing.T) {
	...
	new := strings.Replace(str, "Title=\"local\"", "Title=\"omnilink\"", 1)
	exec, _ := initEnv(types.MergeCfg(types.ReadFile("../cmd/omnilink/omnilink.fork.toml"), new))
	...
	//types.SetTitleOnlyForTest("omnilink")
...
func TestProxyExec(t *testing.T) {
	...
	assert.Nil(t, err)
	assembleTx := erpctypes.AssembleOmnilinkTx(signtx, sig, pubkey, cfg)
	//checkSign

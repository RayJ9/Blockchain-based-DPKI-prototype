...
func (q *QueueProtocol) send2MainChain(cfg *types.OmnilinkConfig, tx *types.Transaction) (*types.Reply, error) {
...
func (q *QueueProtocol) GetPendingSize() (*types.ReplyPendingSize, error) {
	msg, err := q.send(mempoolKey, types.EventGetPendingSize, &types.ReqNil{})
	if err != nil {
		log.Error("GetPendingSize", "Error", err.Error())
		return nil, err
	}
	if reply, ok := msg.GetData().(*types.ReplyPendingSize); ok {
		return reply, nil
	}
	return nil, types.ErrTypeAsset
}
...
func (q *QueueProtocol) GetConfig() *types.OmnilinkConfig {
	if q.client == nil {
		panic("client is nil, can not get OmnilinkConfig")
	}
	cfg := q.client.GetConfig()
	if cfg == nil {
		panic("OmnilinkConfig is nil")
	}
	return cfg
}

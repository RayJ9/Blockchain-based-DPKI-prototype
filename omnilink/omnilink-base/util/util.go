...
func GenNoneTxs(cfg *types.OmnilinkConfig, priv crypto.PrivKey, n int64) (txs []*types.Transaction) {
	for i := 0; i < int(n); i++ {
...
func GenCoinsTxs(cfg *types.OmnilinkConfig, priv crypto.PrivKey, n int64) (txs []*types.Transaction) {
	to, _ := Genaddress()
	for i := 0; i < int(n); i++ {
...
func CreateNoneTx(cfg *types.OmnilinkConfig, priv crypto.PrivKey) *types.Transaction {
...
func CreateCoinsTxWithTxHeight(cfg *types.OmnilinkConfig, priv crypto.PrivKey, to string, amount, txHeight int64) *types.Transaction {
...
func CreateNoneTxWithTxHeight(cfg *types.OmnilinkConfig, priv crypto.PrivKey, txHeight int64) *types.Transaction {
...
func CreateTxWithExecer(cfg *types.OmnilinkConfig, priv crypto.PrivKey, execer string) *types.Transaction {
...
func CreateManageTx(cfg *types.OmnilinkConfig, priv crypto.PrivKey, key, op, value string) *types.Transaction {
...
func CreateCoinsTx(cfg *types.OmnilinkConfig, priv crypto.PrivKey, to string, amount int64) *types.Transaction {
...
func createCoinsTx(cfg *types.OmnilinkConfig, to string, amount int64) *types.Transaction {
...
func CreateTxWithTxHeight(cfg *types.OmnilinkConfig, priv crypto.PrivKey, to string, amount, expire int64) *types.Transaction {
	tx := createCoinsTx(cfg, to, amount)
...
func GenTxsTxHeight(cfg *types.OmnilinkConfig, priv crypto.PrivKey, n, height int64) (txs []*types.Transaction) {
	to, _ := Genaddress()
	for i := 0; i < int(n); i++ {
...
func CreateNoneBlock(cfg *types.OmnilinkConfig, priv crypto.PrivKey, n int64) *types.Block {
...
func CreateCoinsBlock(cfg *types.OmnilinkConfig, priv crypto.PrivKey, n int64) *types.Block {
	newblock := &types.Block{}
	newblock.Height = 1
	newblock.BlockTime = types.Now().Unix()
	newblock.ParentHash = zeroHash[:]
...
func ExecBlock(client queue.Client, prevStateRoot []byte, block *types.Block, errReturn, sync, checkblock bool) (*types.BlockDetail, []*types.Transaction, error) {
	...
	// 写数据库失败时需要及时返回错误，防止错误数据被写入localdb中
...
func CreateNewBlock(cfg *types.OmnilinkConfig, parent *types.Block, txs []*types.Transaction) *types.Block {
	newblock := &types.Block{}
...
func ResetDatadir(cfg *types.Config, datadir string) string {
	...
	if len(datadir) >= 6 && datadir[:6] == "$TEMP/" {
		dir, err := ioutil.TempDir("", "omnilinkdatadir-")
		if err != nil {
			panic(err)
		}

...
// Package queue omnilink底层消息队列模块
package queue
...
type Queue interface {
	...
	Client() Client
	Name() string
	SetConfig(cfg *types.OmnilinkConfig)
	GetConfig() *types.OmnilinkConfig
}
...
func New(name string) Queue {
	...
	go func() {
		for {
			select {
			case <-q.done:
				qlog.Info("closing omnilink callback")
				return
			case msg := <-q.callback:
				if msg.callback != nil {
					msg.callback(msg)
...
// GetConfig return the queue OmnilinkConfig
func (q *queue) GetConfig() *types.OmnilinkConfig {
	return q.cfg
}

// Name return the queue name
func (q *queue) SetConfig(cfg *types.OmnilinkConfig) {
	if cfg == nil {
...
func (q *queue) Start() {
	...
	select {
	case <-q.done:
		qlog.Info("closing omnilink done")
		//atomic.StoreInt32(&q.isClose, 1)
		break
	case <-q.interrupt:
		qlog.Info("closing omnilink")
		//atomic.StoreInt32(&q.isClose, 1)
		break
	case s := <-c:

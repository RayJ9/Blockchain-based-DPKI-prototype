...
//RunOmnilinkCli invoke omnilink client
func RunOmnilinkCli(para []string) (string, error) {
...
func SendTxCommand(cmd string) (string, bool) {
	...
	output, err := RunOmnilinkCli(strings.Fields(cmd))
	if err != nil {
...
func GetTxInfo(txHash string) (string, bool) {
	...
	bReady := false
	txInfo, err := RunOmnilinkCli(strings.Fields(fmt.Sprintf("tx query -s %s", txHash)))
...
func CalcUtxoAvailAmount(addr string, txHash string) (float64, error) {
	...
	outStr, err := RunOmnilinkCli(strings.Fields(fmt.Sprintf("privacy showpai -d 1 -a %s", addr)))
...
func CalcUtxoSpendAmount(addr string, txHash string) (float64, error) {
	...
	outStr, err := RunOmnilinkCli(strings.Fields(fmt.Sprintf("privacy showpas -a %s", addr)))

...
import (
	...
	omnilinkType "code.corp.bcollie.net/omnilink/omnilink-base/system/dapp/commands/types"
)
...
//check item handler
//适配autotest早期版本，handlerfunc的参数为json的map形式，后续统一使用omnilink的TxDetailResult结构体结构
...
type CheckHandlerParamType *omnilinkType.TxDetailResult
...
func (pack *BaseCasePack) CheckResult(handlerMap interface{}) (bCheck bool, bSuccess bool) {
	...
	if bReady {
		...
		var txRecp omnilinkType.TxDetailResult
		pack.TxReceipt = txInfo

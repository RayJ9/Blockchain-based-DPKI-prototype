...
### 代码生成
##### 生成基本代码
>使用omnilink-tool，工具使用参考[文档](https://code.corp.bcollie.net/omnilink/omnilink-base/blob/master/cmd/tools/doc/gendapp.md)
```
//本例默认将calculator生成至官方plugin项目dapp目录下
$ cd $GOPATH/src/code.corp.bcollie.net/omnilink/omnilink-base/cmd/tools && go build -o tool
$ ./tool gendapp -n calculator -p doc/calculator.proto
$ cd $GOPATH/src/github.com/33cn/plugin/plugin/dapp/calculator && ls
```

##### 生成pb.go文件
pb.go文件基于protobuf提供的proto-gen-go插件生成，这里protobuf的版本必须和omnilink引用的保持一致，
具体可以查看omnilink项目go.mod文件，github.com/golang/protobuf库的版本
```
//进入生成合约的目录
...
##### rpc说明
>对于构造交易和query类接口可以通过omnilink框架的rpc去调用，
分别是Omnilink.CreateTransaction和Omnilink.Query，上述代码只是示例如何开发rpc接口，
...
func createAdd(cmd *cobra.Command, args []string) {
	...
	omnilinkReq := rpctypes.CreateTxIn{
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Omnilink.CreateTransaction", omnilinkReq, &res)
...
func queryCalcCountCmd() *cobra.Command {
 ...
 func queryCalcCount(cmd *cobra.Command, args []string) {
 	...
 	omnilinkReq := &rpctypes.Query4Jrpc{
 		Execer:   ptypes.CalculatorX,
 		FuncName: "CalcCount",
 		Payload:  types.MustPBToJSON(&req),
 	}
 	...
 	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Omnilink.Query", omnilinkReq, &res)
...
curl -kd '{"method":"Omnilink.CreateTransaction", "params":[{"execer":"calculator", "actionName":"Add", "payload":{"summand":1,"addend":1}}]}' http://localhost:8801
# 通过omnilink-cli构建Add原始交易
./omnilink-cli calculator add -a 1 -s 1

# queryCount接口类似
curl -kd '{"method":"calculator.QueryCalcCount", "params":[{"action":"Add"}]}' http://localhost:8801
./omnilink-cli calculator query_count -a Add
``` 

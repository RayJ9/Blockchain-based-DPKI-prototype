...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-base/cmd/tools/tasks"
)
...
func (up *updateInitStrategy) Run() error {
	mlog.Info("Begin run omnilink update init.go.")
	defer mlog.Info("Run omnilink update init.go finish.")
...
func (up *updateInitStrategy) initMember() error {
	...
	if err != nil || path == "" {
		if len(gopath) > 0 {
			path = filepath.Join(gopath, "/src/code.corp.bcollie.net/omnilink/omnilink-base/plugin/")
		}
	}
	if packname == "" {
	...
	if len(path) == 0 {
		return errors.New("Omnilink Plugin Not Existed")
	}

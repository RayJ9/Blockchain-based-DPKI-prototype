...
// Run：
//dht_crawler -proto "/omnilink-0/kad/1.0.0" -node "/ip4/ip/tcp/port/pid"

// dhtprotoId="/omnilink-0/kad/1.0.0"
var (
	//dht 协议ID，需要根据具体区块链网络进行配置
	dhtProtoID = flag.String("proto", "/omnilink-0/kad/1.0.0", "dht protocol id,such as:/omnilink-0/kad/1.0.0")
	//扫描的引导节点
...
func outputData(host core.Host, filePath, allpeerfile, jsonfile string) {
	...
	f.WriteString("omnilink  peers topology{ \n")
	...
	f.WriteString(fmt.Sprintf("omnilink all peers:%d{ \n", len(allPeers)))

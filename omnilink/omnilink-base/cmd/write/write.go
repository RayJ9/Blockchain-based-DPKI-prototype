...
import (
	...
	coinstypes "code.corp.bcollie.net/omnilink/omnilink-base/system/dapp/coins/types"
...
func main() {
	omnilinkCfg := types.NewOmnilinkConfig(types.GetDefaultCfgstring())
	cfg := initWrite()
...
func scanWrite(cfg *types.OmnilinkConfig) {
	for {
		...
		err = rpc.Call("Omnilink.GetTxByAddr", paramsReqAddr, &replyTxInfos)
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			continue
		}
		...
		for _, hash := range txHashes {
			...
			err = rpc.Call("Omnilink.QueryTransaction", paramsQuery, &transactionDetail)
			if err != nil {
				fmt.Fprintln(os.Stderr, err)
				continue
			}
			...
			var signed string
			rpc.Call("Omnilink.SignRawTx", paramsReqSignRawTx, &signed)
			paramsRaw := rpctypes.RawParm{
				Data: signed,
			}
			var sent string
			rpc.Call("Omnilink.SendTransaction", paramsRaw, &sent)
			f, err := os.OpenFile(heightFile, os.O_RDWR, 0666)
			if err != nil {
				fmt.Fprintln(os.Stderr, err)
				continue

# 命令行自动补全
  ...
  目前在ubuntu下用git 等软件你会发现按tab键有提示或补全的功能， 这个功能依赖一个软件包， 叫bash-completion。bash-completion 提供的能力是bash shell 的可编程补全。而omnilink-cli 功能也越来越多，每加一步都看下帮助， 效率不高。 把这个能力加到omnilink-cli里， 应该能提高不少效率。
...
```
# 是不是很简单
$ complete -W "account block bty trade token" omnilink-cli
linj@linj-TM1701:~$ omnilink-cli t
token  trade  
...
```
linj@linj-TM1701:~$ omnilink-cli account 

declare -a COMP_WORDS='([0]="omnilink-cli" [1]="account" [2]="")'
declare -- COMP_CWORD="2"
declare -- COMP_LINE="omnilink-cli account "
...
# 用 _subcmd 补全 omnilink-cli
# _subcmd 通过当前光标所在的输入参数过滤可选的子命令
complete -F _subcmd omnilink-cli
```

试用一下
```
linj@linj-TM1701:~$ . subcmd.bash  
linj@linj-TM1701:~$ ./omnilink/omnilink-cli 
...
linj@linj-TM1701:~$ ./omnilink/omnilink-cli t
...
```
linj@linj-TM1701:~$ sudo install subcmd.bash  /usr/share/bash-completion/completions/omnilink-cli
# 重新开个窗口就有用了
linj@linj-TM1701:~$ ./omnilink/omnilink-cli 
account   bty       coins     evm       hashlock  mempool   privacy   retrieve  send      ticket    trade     version   
block     close     config    exec      help      net       relay     seed      stat      token     tx        wallet    
...
### 给omnilink-cli 做个 bash-completion


 地址: https://gitlab.33.cn/linj/omnilink-cli-completion
...
account   bty       coins     evm       hashlock  mempool   privacy   retrieve  send      ticket    trade     version   
block     close     config    exec      help      net       relay     seed      stat      token     tx        wallet    
linj@linj-TM1701:~$ ./omnilink/omnilink-cli b
block  bty    
linj@linj-TM1701:~$ ./omnilink/omnilink-cli bty 
priv2priv  priv2pub   pub2priv   send       transfer   txgroup    withdraw   
linj@linj-TM1701:~$ ./omnilink/omnilink-cli bty t
transfer  txgroup   
linj@linj-TM1701:~$ ./omnilink/omnilink-cli bty transfer -

...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-base/rpc/ethrpc/types"
...
func NewEthAPI(cfg *ctypes.OmnilinkConfig, c queue.Client, api client.QueueProtocolAPI) interface{} {
	e := &ethHandler{}
	e.cli.Init(c, api)
	e.cfg = cfg
	e.checkEvmNonce = cfg.GetModuleConfig().RPC.CheckEvmNonce
	...
	//推送用途
	e.grpcCli = ctypes.NewOmnilinkClient(conn)
...
func (e *ethHandler) SendRawTransaction(rawData string) (hexutil.Bytes, error) {
	...
	omnilinkTx := types.AssembleOmnilinkTx(ntx, sig, pubkey, e.cfg)
	reply, err := e.cli.SendTx(omnilinkTx)
	log.Info("SendRawTransaction", "cacuHash", common.Bytes2Hex(omnilinkTx.Hash()),
		"ethHash:", ntx.Hash().String(), "exec", string(omnilinkTx.Execer), "mempool reply:", common.Bytes2Hex(reply.GetMsg()), "err:", err)
	//调整为返回eth 交易哈希，之前是reply.GteMsg() omnilink 哈希
...
func (e *ethHandler) EstimateGas(callMsg *types.CallMsg) (hexutil.Uint64, error) {
	...
	action := &ctypes.EVMContractAction4Omnilink{Amount: amount, GasLimit: 0, GasPrice: 0, Note: "", ContractAddr: callMsg.To}
	if callMsg.Data != nil {

...
import (
	...
	clientMocks "code.corp.bcollie.net/omnilink/omnilink-base/client/mocks"
	"code.corp.bcollie.net/omnilink/omnilink-base/queue"
	etypes "code.corp.bcollie.net/omnilink/omnilink-base/rpc/ethrpc/types"
...
func init() {
	qapi = &clientMocks.QueueProtocolAPI{}
	cfg := ctypes.NewOmnilinkConfig(ctypes.GetDefaultCfgstring())
	q.SetConfig(cfg)
...
func TestEthHandler_SendRawTransaction(t *testing.T) {
	...
	omnilinkTx := etypes.AssembleOmnilinkTx(ntx, sig, pubkey, ethCli.cfg)
	t.Log("checkchainID", ntx.ChainId(), "support chainid", ethCli.evmChainID)
	omnilinkcrypto.Init(ethCli.cfg.GetModuleConfig().Crypto, ethCli.cfg.GetSubConfig().Crypto)
	ok := omnilinkTx.CheckSign(-1)
	require.True(t, ok)

	//测试修改omnilinkTx 数据，期望验签失败
	//修改Nonce
	omnilinkTx.Nonce = rand.New(rand.NewSource(time.Now().UnixNano())).Int63()
	ok = omnilinkTx.CheckSign(-1)
	require.False(t, ok)
	//修改to
	omnilinkTx.To = "0xa06c8907eb1c8a266165f5484c5ea3bd4e0520a8"
	ok = omnilinkTx.CheckSign(-1)
	require.False(t, ok)
	//修改执行器
	omnilinkTx.Execer = []byte("coinsX")
	ok = omnilinkTx.CheckSign(-1)
	require.False(t, ok)
	//修改packdata
	omnilinkTx.Payload = append(omnilinkTx.GetPayload(), uint8(8))
	ok = omnilinkTx.CheckSign(-1)
	require.False(t, ok)
	//修改chainID
	omnilinkTx.ChainID = 666
	ok = omnilinkTx.CheckSign(-1)
	require.False(t, ok)
	qapi.On("Query", mock.Anything).Return("0x12", nil)

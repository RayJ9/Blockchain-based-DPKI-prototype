...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-base/system/address/eth"
	dtypes "code.corp.bcollie.net/omnilink/omnilink-base/system/dapp/coins/types"
...
func paraseOmnilinkTx(itx *ctypes.Transaction, blockHash common.Hash, blockNum int64, index uint64, cfg *ctypes.OmnilinkConfig) *Transaction {
	...
	if err != nil {
		log.Error("paraseOmnilinkTx", "err", err)
		return nil
	}
	...
	if strings.HasSuffix(string(itx.Execer), "evm") {
		var action ctypes.EVMContractAction4Omnilink
		...
		if cfg.IsPara() {
			...
			err := ctypes.Decode(itx.GetPayload(), &action)
			if err != nil {
				log.Error("paraseOmnilinkTx", "decode coinsAction err", err)
				return nil
			}
...
func paraseOmnilinkTxPayload(execer string, payload []byte, blockHash common.Hash, blockNum uint64) *Transaction {
	var note []byte
	if strings.HasSuffix(execer, "evm") {
		var evmaction ctypes.EVMContractAction4Omnilink
...
// TxsToEthTxs omnilink txs format transfer to eth txs format
func TxsToEthTxs(blockHash common.Hash, blockNum int64, ctxs []*ctypes.Transaction, cfg *ctypes.OmnilinkConfig, full bool) (txs []interface{}, fee int64, err error) {
	for index, itx := range ctxs {
		...
		tx := paraseOmnilinkTxPayload(string(itx.GetExecer()), itx.GetPayload(), blockHash, uint64(blockNum))
		//重置交易哈希
		if tx != nil {
			tx.Hash = common.BytesToHash(itx.Hash())
			txs = append(txs, tx)
			...
			tx = paraseOmnilinkTx(itx, blockHash, blockNum, uint64(index), cfg)
			if tx != nil {
				txs = append(txs, tx)
			}
		}
...
// TxDetailsToEthReceipts omnilink txdetails transfer to eth tx receipts
func TxDetailsToEthReceipts(txDetails *ctypes.TransactionDetails, blockHash common.Hash, cfg *ctypes.OmnilinkConfig) (txs Transactions, receipts []*Receipt, err error) {
	for index, detail := range txDetails.GetTxs() {
		if detail.GetTx() == nil {
			continue
		}

		tx := paraseOmnilinkTxPayload(string(detail.GetTx().GetExecer()), detail.GetTx().GetPayload(), blockHash, uint64(detail.Height))
		if tx == nil {
			tx = paraseOmnilinkTx(detail.GetTx(), blockHash, detail.GetHeight(), uint64(index), cfg)
			if tx == nil {
				continue
			}
...
// AssembleOmnilinkTx 通过eth tx 组装omnilink tx 全部走evm 通道
func AssembleOmnilinkTx(etx *etypes.Transaction, sig, pubkey []byte, cfg *ctypes.OmnilinkConfig) *ctypes.Transaction {
	rawData, err := etx.MarshalBinary()
	if err != nil {
		log.Error("AssembleOmnilinkTx", "tx.MarshalBinary err", err.Error())
		return nil
	}
	...
	action := &ctypes.EVMContractAction4Omnilink{
	...
	//全部走Evm 通道，exec=evm
	var omnilinkTx = &ctypes.Transaction{
	...
	//omnilinkTx.SetExpire(cfg, time.Hour*3)
	omnilinkTx.Expire = 0
	omnilinkTx.Nonce = int64(etx.Nonce())
	if cfg.IsPara() {
		omnilinkTx.To = address.ExecAddress(string(omnilinkTx.Execer))
	}
	return omnilinkTx
}

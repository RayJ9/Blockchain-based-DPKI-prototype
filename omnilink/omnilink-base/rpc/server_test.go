...
import (
	...
	"code.corp.bcollie.net/omnilink/omnilink-base/rpc/ethrpc"
	...
	qmocks "code.corp.bcollie.net/omnilink/omnilink-base/queue/mocks"
...
func TestCheckBasicAuth(t *testing.T) {
	...
	r.SetBasicAuth("1212121", "omnilink-mypasswd")
	assert.True(t, checkBasicAuth(r))
	rpcCfg.JrpcUserName = "omnilink-user"
	rpcCfg.JrpcUserPasswd = "omnilink-mypasswd"
	r.SetBasicAuth("", "omnilink-mypasswd")
	assert.False(t, checkBasicAuth(r))
	r.SetBasicAuth("", "")
	assert.False(t, checkBasicAuth(r))
	r.SetBasicAuth("omnilink-user", "")
	assert.False(t, checkBasicAuth(r))
	r.SetBasicAuth("omnilink", "1234")
	assert.False(t, checkBasicAuth(r))
	r.SetBasicAuth("omnilink-user", "omnilink-mypasswd")
	assert.True(t, checkBasicAuth(r))
...
func TestJSONClient_Call(t *testing.T) {
	...
	var result = ""
	err = jsonClient.Call("Omnilink.Version", nil, &result)
	assert.NotNil(t, err)
	...
	err = jsonClient.Call("Omnilink.Version", nil, &nodeVersion)
	assert.Nil(t, err)
	assert.Equal(t, "6.0.2", nodeVersion.Omnilink)

	var isSnyc bool
	err = jsonClient.Call("Omnilink.IsSync", &types.ReqNil{}, &isSnyc)
	assert.Nil(t, err)
	...
	err = jsonClient.Call("Omnilink.GetNetInfo", &types.ReqNil{}, &nodeInfo)
	assert.Nil(t, err)
	...
	err = jsonClient.Call("Omnilink.SignRawTx", &types.ReqSignRawTx{}, &singRet)
	assert.Nil(t, err)
	assert.Equal(t, "123", singRet)

	var fee types.TotalFee
	api.On("LocalGet", mock.Anything).Return(nil, errors.New("error value"))
	err = jsonClient.Call("Omnilink.QueryTotalFee", &types.LocalDBGet{Keys: [][]byte{[]byte("test")}}, &fee)
	assert.NotNil(t, err)

	var retNtp bool
	api.On("IsNtpClockSync", mock.Anything).Return(&types.Reply{IsOk: true, Msg: []byte("yes")}, nil)
	err = jsonClient.Call("Omnilink.IsNtpClockSync", &types.ReqNil{}, &retNtp)
	assert.Nil(t, err)
...
func testCreateTxCoins(t *testing.T, cfg *types.OmnilinkConfig, jsonClient *jsonclient.JSONClient) {
	...
	var res string
	err := jsonClient.Call("Omnilink.CreateRawTransaction", req, &res)
	assert.Nil(t, err)
...
func TestGrpc_Call(t *testing.T) {
	...
	client := types.NewOmnilinkClient(c)

...
func testVersionOK(t *testing.T) {
	reply := &types.VersionInfo{Omnilink: "6.0.2"}
	qapi.On("Version").Return(reply, nil)
	data, err := g.Version(getOkCtx(), nil)
	assert.Nil(t, err, "the error should be nil")
	assert.Equal(t, "6.0.2", data.Omnilink, "reply should be ok")
}
...
func TestGrpc_GetFork(t *testing.T) {
	...
	newstr := strings.Replace(str, "Title=\"local\"", "Title=\"omnilink\"", 1)
	cfg := types.NewOmnilinkConfig(newstr)
	...
	cfg1 := types.NewOmnilinkConfig(types.GetDefaultCfgstring())
	Init(cfg1)
...
func TestGrpc_SubEvent(t *testing.T) {
	c := queue.New("mytest")
	omnilinkCfg := types.NewOmnilinkConfig(types.ReadFile("../cmd/omnilink/omnilink.test.toml"))
	c.SetConfig(omnilinkCfg)

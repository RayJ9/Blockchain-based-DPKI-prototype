...
import (
	...
	rclient "code.corp.bcollie.net/omnilink/omnilink-base/rpc/client"
	...
	_ "code.corp.bcollie.net/omnilink/omnilink-base/rpc/grpcclient" // register grpc multiple resolver
...
// Omnilink  a channel client
type Omnilink struct {
...
func NewGRpcServer(c queue.Client, api client.QueueProtocolAPI) *Grpcserver {
	...
	if rpcCfg.EnableTLS {
		...
		if err != nil {
			panic(fmt.Sprintf("err=%s, if cert.pem not found, run omnilink-cli cert --host=127.0.0.1 to create", err.Error()))
		}
	...
	s.s = server
	types.RegisterOmnilinkServer(server, s.grpc)
...
func NewJSONRPCServer(c queue.Client, api client.QueueProtocolAPI) *JSONRPCServer {
	j := &JSONRPCServer{jrpc: &Omnilink{}}
	j.jrpc.cli.Init(c, api)
	server := rpc.NewServer()
	j.s = server
	err := server.RegisterName("Omnilink", j.jrpc)
	if err != nil {
		return nil
	}
	return j
...
func New(cfg *types.OmnilinkConfig) *RPC {

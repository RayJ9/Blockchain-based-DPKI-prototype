...
import (
	...
	bcMocks "code.corp.bcollie.net/omnilink/omnilink-base/blockchain/mocks"
...
//GetBlockChain :
func (mock *OmnilinkMock) GetBlockChain() *BlockChain {
	return mock.chain
}
...
func Test_PostEVMEvent(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		evmAction := &types.EVMContractAction4Omnilink{
...
func Test_RecoverPush(t *testing.T) {
	...
	//omnilink的push服务重启后，不会将其添加到task中，
	chainAnother := &BlockChain{
		isRecordBlockSequence: true,
		enablePushSubscribe:   true,
	}
...
//init work
func NewOmnilinkMock(cfgpath string, mockapi client.QueueProtocolAPI) *OmnilinkMock {
	cfg := types.NewOmnilinkConfig(types.GetDefaultCfgstring())
	return newWithConfigNoLock(cfg, mockapi)
}

func NewOmnilinkMockWithFlag(cfgpath string, mockapi client.QueueProtocolAPI, isRecordBlockSequence, enablePushSubscribe bool) *OmnilinkMock {
	cfg := types.NewOmnilinkConfig(types.GetDefaultCfgstring())
...
func createBlocks(t *testing.T, mock33 *OmnilinkMock, blockchain *BlockChain, number int64) {
...
func createBlockChain(t *testing.T) (*BlockChain, *OmnilinkMock) {
	mock33 := NewOmnilinkMock("", nil)
...
func createBlockChainWithFalgSet(t *testing.T, isRecordBlockSequence, enablePushSubscribe bool) (*BlockChain, *OmnilinkMock) {
	mock33 := NewOmnilinkMockWithFlag("", nil, isRecordBlockSequence, enablePushSubscribe)

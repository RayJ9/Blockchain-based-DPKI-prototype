...
func TestRollbackblock(t *testing.T) {
	...
	cfg := types.NewOmnilinkConfig(types.MergeCfg(types.ReadFile("../cmd/omnilink/omnilink.fork.toml"), new))
...
func TestNeedRollback(t *testing.T) {
	str := types.GetDefaultCfgstring()
	new := strings.Replace(str, "Title=\"local\"", "Title=\"omnilink\"", 1)
	cfg := types.NewOmnilinkConfig(new)
	cfg.SetDappFork("store-kvmvccmavl", "ForkKvmvccmavl", 20*10000)
	mock33 := testnode.NewWithConfig(cfg, nil)
	chain := mock33.GetBlockChain()
...
func testMockSendTx(t *testing.T, mock33 *testnode.OmnilinkMock) {

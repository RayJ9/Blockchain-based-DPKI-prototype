...
const Web3 = require(path.join(BLOCKCHAIN_ROOT, "dpki-experiment", "node_modules", "web3"));
const solc = require(path.join(BLOCKCHAIN_ROOT, "dpki-experiment", "node_modules", "solc"));
...
const CHAIN_IDS = { main: 4100, sideA: 4101, sideB: 4102 };
...
async function chainDescriptor(name, web3) {
  const [reportedChainId, blockNumber, listening] = await Promise.all([
    web3.eth.getChainId(),
...
async function main() {
  ...
  console.log(`[SUCCESS] Three-chain sidechain initialization completed. Summary: ${path.join(OUTPUT_DIR, "experiment_summary.json")}`);
}

...
$ChainRoot = Resolve-Path $PSScriptRoot
$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$session = Join-Path $ChainRoot "outputs\three_chain_$stamp"
$archiveDir = Join-Path $ChainRoot "archives"
...
$exitCode = 0
try {
    & (Join-Path $ChainRoot "scripts\start-three-chains.ps1") -MeanBlockMs $MeanBlockMs -Clean
    node (Join-Path $ChainRoot "run-sidechain-smoke.js") 2>&1 | Tee-Object -LiteralPath $consoleLog
    $exitCode = $LASTEXITCODE
    if ($exitCode -ne 0) {
        throw "Three-chain smoke experiment failed with exit code $exitCode"
    }
} finally {
    $runtime = Join-Path $ChainRoot "runtime"
    ...
    if (-not $KeepChains) {
        & (Join-Path $ChainRoot "scripts\stop-three-chains.ps1")
    }

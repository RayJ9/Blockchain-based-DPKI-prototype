...
| --- | --- | --- |
| Centralized PKI | `blockchain/platforms/centralized-pki/` | X.509 verification, OCSP, signed assertion |
| Our proposed DPKI | `blockchain/platforms/proposed-dpki/` | X.509, MPT proof, optional on-chain authentication record |
| Multi-CA based DPKI | `blockchain/platforms/multi-ca-dpki/` | 4-of-6 CA responses and threshold evidence |
| Full contract DPKI | `blockchain/platforms/full-contract-dpki/` | certificate/status/assertion checks and result storage in a contract |
...
`blockchain/dpki-experiment`, builds Omnilink from the source under
`omnilink/`, and checks all public entrypoints. Use `-SkipOmnilinkBuild` only
when a compatible local build is already present.
...
Each launcher starts one main chain and two domain sidechains. The main chain
stores service-CA records and sidechain checkpoints. Authentication transactions
are sent to Sidechain A, while Sidechain B is initialized as the remote domain
for cross-domain workflows. The launcher validates all three processes before
running the requested workload.
...
The archive contains the console transcript, logs and configurations from all
three Omnilink chains,
...
## Three-chain sidechain runtime

The multi-ledger runtime is available under
`blockchain/sidechain-three-chain/`. It starts one main chain and two domain
...
authentication. This is an application-level checkpoint experiment rather than
...
- `blockchain/`: three-chain runtime, four-node compatibility runtime, shared DPKI runner, contracts, and four authentication platforms.
...
- `.internal/legacy-simulations/`: compatibility-only analytical and sweep modules used by the public experiment runners.

...
def draw_table(ax: plt.Axes, frame: pd.DataFrame, title: str, font_size: int = 7.4) -> None:
    ax.axis("off")
    ax.text(
        0.0,
        1.03,
...
def main() -> None:
    ...
    fig = plt.figure(figsize=(9.0, 7.7), dpi=300)
    gs = fig.add_gridspec(
        3,
        1,
        height_ratios=[1.35, 1.6, 1.35],
        left=0.02,
        right=0.98,
        top=0.98,
        bottom=0.04,
    ...
    draw_table(fig.add_subplot(gs[0, 0]), MGMT, "Management comparison")
    draw_table(fig.add_subplot(gs[1, 0]), INTRA, "Intra-domain authentication comparison")
    draw_table(fig.add_subplot(gs[2, 0]), CROSS, "Cross-domain authentication comparison")

    for suffix in ("png", "pdf", "eps"):
        fig.savefig(ROOT / f"figure_overhead_table_v4.{suffix}", bbox_inches="tight", pad_inches=0.02)
    plt.close(fig)
